...
function makeEngine() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "miko-experiments-route-"));
...
describe("experiments route", () => {
  ...
  it("stores observation under agent experiment directory and requires agentId for route reads", async () => {
    const { engine } = makeEngine();
    const agentDir = engine.getAgentDir("miko");
    ...
    expect(readCacheSnapshotObservation(agentDir).memoryMdPreview).toContain("This feature is available in English only.");
    ...
    const ok = await (routeFetch as any)(route, "/experiments/memory/cache-snapshot-reflection/observation?agentId=miko");
    expect(ok.status).toBe(200);
    expect(ok.body.observation.memoryMdPreview).toContain("This feature is available in English only.");

    const deleted = await routeFetch(route, "/experiments/memory/cache-snapshot-reflection/observation?agentId=miko", {
      method: "DELETE",
    });
