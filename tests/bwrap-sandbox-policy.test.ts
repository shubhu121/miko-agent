...
describe("Linux bwrap sandbox policy projection", () => {
  ...
  beforeEach(() => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-bwrap-policy-"));
  });
  ...
  it("uses a read-all root mount while keeping writes scoped to explicit roots", () => {
    ...
    const agentSessions = path.join(tempRoot, "miko", "agents", "miko", "sessions");
    const sessionFiles = path.join(tempRoot, "miko", "session-files");
