...
export function createRequestContext(c, engine) {
  ...
  const request = {
    ...
    url: c?.req?.url || "http://miko.local/",
    path: safePathname(c?.req?.url || "http://miko.local/"),
  };
...
function getActiveGrants(engine, authPrincipal) {
  ...
  if (!engine?.mikoHome) return implicit ? [implicit] : [];
  try {
    return [
      ...findActiveGrantsForPrincipal(engine.mikoHome, authPrincipal.principalId),
      ...(implicit ? [implicit] : []),
    ];
  } catch {
