...
function TelegramSecretProbe() {
  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => bridge.setSelectedAgentId('miko')}>switch to miko</button>
    </div>
  );
}
...
function bridgeStatus(overrides: Record<string, unknown> = {}) {
  return {
    ...
    whatsapp: { enabled: false, status: 'disconnected', agentId: 'miko' },
    qq: {
      enabled: false,
      status: 'disconnected',
      appID: 'qq-app',
    ...
    wechat: { enabled: false, status: 'disconnected', token: '', agentId: 'miko' },
...
describe('useBridgeState snapshot hydration', () => {
  beforeEach(() => {
    ...
    mockMikoFetch.mockImplementation((url: string) => {
      if (url === '/api/bridge/status?agentId=miko') {
  ...
  it('uses snapshot bridge status before the status refresh request settles', () => {
    ...
    expect(mockMikoFetch).toHaveBeenCalledWith(
      '/api/bridge/status?agentId=miko',
  ...
  it('keeps saved public ishiki in the settings snapshot for remounts', async () => {
    ...
    mockMikoFetch.mockImplementation((url: string, opts?: RequestInit) => {
      if (url === '/api/bridge/status?agentId=miko') {
        return new Promise<Response>(() => {});
      }
      if (url === '/api/agents/miko/public-ishiki') {
  ...
  it('applies owner status returned by setOwner without waiting for a later refresh', async () => {
    mockMikoFetch.mockImplementation((url: string, opts?: RequestInit) => {
      ...
      if (url === '/api/bridge/owner?agentId=miko') {
        expect(opts).toMatchObject({
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  it('clears the submitted plaintext after save, ignores the returned mask, and tests with saved credentials', async () => {
    ...
    mockMikoFetch.mockImplementation((url: string, opts?: RequestInit) => {
      ...
      if (url === '/api/bridge/config?agentId=miko') {
        expect(opts).toMatchObject({
          method: 'POST',
          body: JSON.stringify({
            platform: 'telegram',
      ...
      if (url === '/api/bridge/test?agentId=miko') {
        expect(opts).toMatchObject({
          method: 'POST',
          body: JSON.stringify({
            platform: 'telegram',
        ...
        return Promise.resolve(new Response(JSON.stringify({ ok: true, info: { username: 'miko_bot' } })));
      }
      throw new Error(`unexpected request: ${url}`);
    });
    ...
    await waitFor(() => {
      expect(mockState.showToast).toHaveBeenCalledWith('settings.bridge.testOk @miko_bot', 'success');
    });
  });
  ...
  it('keeps connection-test state and results owned by the Agent and request generation', async () => {
    let resolveMikoTest!: (response: Response) => void;
    ...
    mockMikoFetch.mockImplementation((url: string) => {
      ...
      if (url === '/api/bridge/test?agentId=miko') {
        return new Promise<Response>((resolve) => { resolveMikoTest = resolve; });
      }
    ...
    await act(async () => {
      resolveMikoTest(new Response(JSON.stringify({
        ok: true,
        info: { username: 'old_miko_bot' },
      })));
    });
  ...
  it('does not reuse a revision after switching away and back while a save is in flight', async () => {
    ...
    render(<TelegramSecretProbe />);
    fireEvent.change(screen.getByTestId('telegram-secret-input'), { target: { value: 'old-miko-secret' } });
    fireEvent.blur(screen.getByTestId('telegram-secret-input'));
    fireEvent.click(screen.getByRole('button', { name: 'switch to mio' }));
    fireEvent.click(screen.getByRole('button', { name: 'switch to miko' }));
    fireEvent.change(screen.getByTestId('telegram-secret-input'), { target: { value: 'new-miko-secret' } });
    ...
    await waitFor(() => {
      expect(screen.getByTestId('telegram-secret-input')).toHaveValue('new-miko-secret');
    });
    expect(screen.getByTestId('telegram-secret-revision')).toHaveTextContent('2');
    expect(screen.getByTestId('telegram-secret-dirty')).toHaveTextContent('true');
  });
  ...
  it('drops plaintext drafts when the selected Agent changes', async () => {
    ...
    render(<TelegramSecretProbe />);
    fireEvent.change(screen.getByTestId('telegram-secret-input'), { target: { value: 'miko-only-secret' } });
    fireEvent.click(screen.getByRole('button', { name: 'switch to mio' }));
