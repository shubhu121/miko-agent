...
describe('initApp bridge indicator', () => {
  ...
  it('treats wechat as a connected bridge when bootstrapping the sidebar dot', async () => {
    ...
    mockMikoFetch
      ...
      .mockResolvedValueOnce(jsonResponse({ agent: 'Miko', user: 'User', avatars: {} }))
  ...
  it('refreshes the HttpOnly device web session before opening WebSocket for a persisted LAN frontend', async () => {
    ...
    (globalThis as Record<string, unknown>).window = {
      ...
      localStorage: {
        getItem: vi.fn((key: string) => key === 'miko-server-connections-v1' ? persistedLanConnectionJson() : null),
        setItem: vi.fn(),
        removeItem: vi.fn(),
      },
      platform: {
    ...
    expect(mockMikoFetch).toHaveBeenNthCalledWith(1, '/api/web-auth/login', expect.objectContaining({
      method: 'POST',
      credentials: 'include',
      body: JSON.stringify({ credential: 'fixture-key' }),
    }));
    expect(mockMikoFetch).toHaveBeenNthCalledWith(2, '/api/server/identity');
    ...
    expect((mockMikoFetch.mock.invocationCallOrder[0] ?? 0)).toBeLessThan(mockConnectWebSocket.mock.invocationCallOrder[0] ?? 0);
  });
  ...
  it('stops startup explicitly when server identity cannot be loaded', async () => {
    ...
    mockMikoFetch.mockRejectedValueOnce(new Error('identity unavailable'));

    const { initApp } = await import('../app-init');
    await initApp();

    expect(mockMikoFetch).toHaveBeenCalledWith('/api/server/identity');
