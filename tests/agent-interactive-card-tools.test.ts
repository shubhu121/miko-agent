...
describe("Agent interactive card tools", () => {
  ...
  it("registers miko_card_guide and show_card in runtime tool snapshots", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "miko-card-tools-"));
    roots.push(root);
    ...
    expect(toolNames).toContain("miko_card_guide");
