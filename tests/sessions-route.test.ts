...
describe("sessions route", () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-sessions-route-"));
  ...
  it("passes workspaceFolders when creating a new session and returns the normalized scope", async () => {
    ...
    const engine = {
      ...
      createSession: vi.fn(async () => ({
        sessionPath: "/tmp/agents/miko/sessions/new.jsonl",
    ...
    expect(hub.eventBus.emit).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        session: expect.objectContaining({
          path: "/tmp/agents/miko/sessions/new.jsonl",
          sessionId: "sess_route_new",
          thinkingLevel: "high",
        }),
      }),
      "/tmp/agents/miko/sessions/new.jsonl",
    );
  });
  ...
  it("resolves workspaceMountId on the server when creating a new session", async () => {
    ...
    const engine = {
      ...
      createSession: vi.fn(async (_sessionMgr, cwd) => {
        engine.cwd = cwd;
        return { sessionPath: "/tmp/agents/miko/sessions/new.jsonl", agentId: "miko" };
      }),
      createSessionForAgent: vi.fn(),
      persistSessionMeta: vi.fn(),
      updateConfig: vi.fn(async (patch) => Object.assign(engine.config, patch)),
  ...
  it("creates a detached session without switching the focused session", async () => {
    ...
    const engine = {
      currentSessionPath: "/tmp/agents/miko/sessions/focused.jsonl",
      currentAgentId: "miko",
      config: {},
      ...
      createDetachedSession: vi.fn(async () => ({
        sessionPath: "/tmp/agents/miko/sessions/quick.jsonl",
    ...
    expect(data).toMatchObject({
      ok: true,
      path: "/tmp/agents/miko/sessions/quick.jsonl",
      sessionId: "sess_quick",
      agentId: "miko",
      currentSessionPath: "/tmp/agents/miko/sessions/focused.jsonl",
      permissionMode: "auto",
    ...
    expect(hub.eventBus.emit).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "session_created",
        session: expect.objectContaining({ path: "/tmp/agents/miko/sessions/quick.jsonl" }),
      }),
      "/tmp/agents/miko/sessions/quick.jsonl",
    );
  });
  ...
  it("adds a session authorized folder through an explicit session route", async () => {
    const { createSessionsRoute } = await import("../server/routes/sessions.ts");
    const app = new Hono();
    const sessionPath = path.join(tmpDir, "agents", "miko", "sessions", "new.jsonl");
    ...
    const engine = {
      ...
      agentIdFromSessionPath: vi.fn(() => "miko"),
  ...
  it("assigns a new session to the requested project before broadcasting it", async () => {
    ...
    const engine = {
      ...
      planMode: false,
      memoryModelUnavailableReason: null,
      normalizeSessionProjectAssignmentId: vi.fn(() => "project-miko"),
      createSession: vi.fn(async () => ({ sessionPath: "/tmp/agents/miko/sessions/new.jsonl", agentId: "miko" })),
      createSessionForAgent: vi.fn(),
    ...
    const res = await app.request("/api/sessions/new", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ cwd: "/tmp/work", projectId: "project-miko" }),
    });
    const data = await res.json();

    expect(res.status).toBe(200);
    expect(engine.normalizeSessionProjectAssignmentId).toHaveBeenCalledWith("project-miko");
    expect(engine.setSessionProjectAssignment).toHaveBeenCalledWith({
      sessionPath: "/tmp/agents/miko/sessions/new.jsonl",
      projectId: "project-miko",
    });
    expect(data.projectId).toBe("project-miko");
    expect(hub.eventBus.emit).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "session_created",
        session: expect.objectContaining({ projectId: "project-miko" }),
      }),
      "/tmp/agents/miko/sessions/new.jsonl",
    );
  });
  ...
  it("includes pinnedAt in the session list response", async () => {
    ...
    const engine = {
      listSessions: vi.fn(async () => [{
        path: "/tmp/agents/miko/sessions/a.jsonl",
        title: "Pinned thread",
        firstMessage: "hello",
  ...
  it("includes explicit projectId in the session list response", async () => {
    ...
    expect(res.status).toBe(200);
    expect(data[0].projectId).toBe("project-miko");
  });
  ...
  it("searches sessions without exposing the cached full transcript", async () => {
    ...
    const sessions = [
      ...
      {
        path: "/tmp/agents/miko/sessions/content.jsonl",
        title: "This feature is available in English only.",
        firstMessage: "hello",
    ...
    expect(contentData.results).toEqual([
      expect.objectContaining({
        path: "/tmp/agents/miko/sessions/content.jsonl",
        matchKind: "content",
        snippet: expect.stringContaining("This feature is available in English only."),
      }),
    ]);
  ...
  it("keeps cold-start session permission mode from the session list projection", async () => {
    ...
    const app = new Hono();
    const session = {
      path: "/tmp/agents/miko/sessions/auto.jsonl",
  ...
  it("creates a primary-agent continuation from a deleted-agent session", async () => {
    ...
    const newPath = path.join(agentsDir, "miko", "sessions", "new.jsonl");
    ...
    expect(hub.eventBus.emit).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "session_created",
        session: expect.objectContaining({ path: newPath, agentId: "miko" }),
      }),
      newPath,
    );
  });
  ...
  it("includes summary presence in the session list response", async () => {
    ...
    const summaryManager = {
      getSummary: vi.fn((sessionId) => (
        sessionId === "has-summary"
          ? { session_id: sessionId, summary: "This feature is available in English only." }
          : null
      )),
    };
    ...
    const engine = {
      ...
      listSessions: vi.fn(async () => [
        {
          path: "/tmp/agents/miko/sessions/has-summary.jsonl",
          title: "Has summary",
          firstMessage: "hello",
          modified: new Date("2026-04-29T07:00:00.000Z"),
          messageCount: 2,
    ...
    expect(data.map((s) => [s.path, s.hasSummary])).toEqual([
      ["/tmp/agents/miko/sessions/has-summary.jsonl", true],
      ["/tmp/agents/miko/sessions/no-summary.jsonl", false],
    ]);
  ...
  it("replays the latest user message through the branch-aware action", async () => {
    const { createSessionsRoute } = await import("../server/routes/sessions.ts");
    const app = new Hono();
    const sessionPath = path.join(tmpDir, "agents", "miko", "sessions", "a.jsonl");
    fs.mkdirSync(path.dirname(sessionPath), { recursive: true });
  ...
  it("returns a session summary through an explicit route", async () => {
    const { createSessionsRoute } = await import("../server/routes/sessions.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/with-summary.jsonl";
    const summaryManager = {
      getSummary: vi.fn(() => ({
    ...
    const agentIdFromSessionPath = vi.fn((_sp: any) => "miko");
    const engine = {
      agentsDir: "/tmp/agents",
  ...
  it("returns an empty summary state when the session has no summary", async () => {
    ...
    const res = await app.request("/api/sessions/summary?path=%2Ftmp%2Fagents%2Fmiko%2Fsessions%2Fempty.jsonl");
    const data = await res.json();
  ...
  it("pins and unpins sessions through an explicit route", async () => {
    ...
    const pinRes = await app.request("/api/sessions/pin", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ path: "/tmp/agents/miko/sessions/a.jsonl", pinned: true }),
    });
    ...
    expect(engine.setSessionPinned).toHaveBeenCalledWith({
      ...
      sessionPath: "/tmp/agents/miko/sessions/a.jsonl",
    }, true);
    ...
    const unpinRes = await app.request("/api/sessions/pin", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ path: "/tmp/agents/miko/sessions/a.jsonl", pinned: false }),
    });
  ...
  it("pins sessions by sessionId and rejects stale locator paths", async () => {
    ...
    const app = new Hono();
    const pinnedAt = "2026-04-29T08:00:00.000Z";
    const currentPath = "/tmp/agents/miko/sessions/current.jsonl";
    ...
    const mismatch = await app.request("/api/sessions/pin", {
      ...
      body: JSON.stringify({
        sessionId: "sess_route_pin",
        path: "/tmp/agents/miko/sessions/stale.jsonl",
        pinned: true,
      }),
    });
  ...
  it("clears pinned state before archiving a session", async () => {
    ...
    const app = new Hono();
    const agentsDir = path.join(tmpDir, "agents");
    const sessionPath = path.join(agentsDir, "miko", "sessions", "a.jsonl");
    fs.mkdirSync(path.dirname(sessionPath), { recursive: true });
    fs.writeFileSync(sessionPath, "{}\n", "utf-8");
  ...
  it("marks current todos completed and removed through an explicit session route", async () => {
    ...
    const app = new Hono();
    const agentsDir = path.join(tmpDir, "agents");
    const sessionDir = path.join(agentsDir, "miko", "sessions");
  ...
  it("infers subagent agent identity from child sessionPath when history details are missing", async () => {
    ...
    vi.mocked(msgUtils.loadSessionHistoryMessages).mockResolvedValueOnce([
      { role: "assistant", content: "parent says hi" },
      {
        ...
        details: {
          taskId: "subagent-1",
          task: "do work",
          sessionPath: "/tmp/agents/miko/subagent-sessions/child.jsonl",
          streamStatus: "done",
        },
      },
    ]);
    ...
    const engine = {
      ...
      getAgent: vi.fn((id) => (id === "miko" ? { agentName: "Miko" } : null)),
    };
    ...
    expect(data.blocks[0]).toMatchObject({
      ...
      agentId: "miko",
      agentName: "Miko",
      streamKey: "/tmp/agents/miko/subagent-sessions/child.jsonl",
    });
  });
  ...
  it("hydrates legacy path-only subagent blocks with sessionId and current manifest locator", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const legacyChildPath = "/tmp/agents/miko/subagent-sessions/child-old.jsonl";
    const movedChildPath = "/tmp/agents/miko/subagent-sessions/archive/child-new.jsonl";
  ...
  it("restores deferred subagent result as an interlude block before the following assistant reply", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/subagent-result.jsonl";
    ...
    const engine = {
      ...
      agentIdFromSessionPath: vi.fn(() => "miko"),
      resolveSessionOwnership: vi.fn(() => ({ agentId: "miko", source: "path", agentDeleted: false })),
      getAgent: vi.fn((id) => (id === "miko" ? { agentName: "This feature is available in English only." } : null)),
      deferredResults: {
        query: vi.fn(() => ({
          status: "resolved",
    ...
    expect(interlude.text).not.toContain("This feature is available in English only.");
  });
  ...
  it("restores repeated deferred result occurrences for the same task as distinct interludes", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/subagent-result-repeat.jsonl";
  ...
  it("exposes JSONL source order for messages and deferred interludes during restore", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/source-order.jsonl";

    vi.mocked(msgUtils.extractTextContent)
      .mockReturnValueOnce({ text: "This feature is available in English only.", images: [], thinking: "", toolUses: [] })
      .mockReturnValueOnce({ text: "This feature is available in English only.", images: [], thinking: "", toolUses: [] })
      .mockReturnValueOnce({ text: "This feature is available in English only.", images: [], thinking: "", toolUses: [] });
    vi.mocked(msgUtils.loadSessionHistoryMessages).mockResolvedValueOnce([
    ...
    expect(data.messages.map((m) => [m.content, m.sourceIndex])).toEqual([
      ["This feature is available in English only.", 0],
      ["This feature is available in English only.", 3],
      ["This feature is available in English only.", 5],
    ]);
  ...
  it("restores consumed turn input ledger records as durable interludes before their assistant reply", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/turn-input-consumption.jsonl";
    ...
    vi.mocked(msgUtils.loadSessionHistoryMessages).mockResolvedValueOnce([
      { role: "assistant", content: "before delivery" },
      {
        ...
        id: "custom-task-a",
        customType: "miko-background-result",
        content: "<miko-background-result task-id=\"task-a\" status=\"success\" type=\"subagent\">\ndone\n</miko-background-result>",
        display: false,
      ...
      { role: "assistant", id: "assistant-task-a", content: "This feature is available in English only." },
    ]);
  ...
  it("keeps deferred subagent interlude hidden in history until an assistant reply exists", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/subagent-result-pending-reply.jsonl";
  ...
  it("does not attach a hidden deferred interlude across a later user message", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/subagent-result-cross-user.jsonl";
  ...
  it("loads session messages by sessionId and treats path as a legacy locator", async () => {
    ...
    const engine = {
      agentsDir: "/tmp/agents",
      currentSessionPath: "/tmp/agents/miko/sessions/focus.jsonl",
      getSessionManifest: vi.fn((sessionId) => (
    ...
    const res = await app.request(
      `/api/sessions/messages?sessionId=sess_messages&path=${encodeURIComponent("/tmp/agents/miko/sessions/stale.jsonl")}`,
    );
    ...
    const engine = {
      ...
      subagentRuns: {
        query: vi.fn((id) => id === "workflow-1"
          ? { taskId: "workflow-1", status: "resolved", summary: "This feature is available in English only.", completedAt: "2026-05-31T08:26:49.160Z" }
          : null),
      },
    };
  ...
  it("does not expose legacy reminder blocks in REST history projections", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const raw = "[miko_reminder at 2026-07-05 14:05]\n- Plugin demo loaded\n[/miko_reminder]\n\nhello";
  ...
  it("strips Bridge internal time tags from loadMessages history for bridge sessions", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const bridgePath = "/tmp/agents/miko/sessions/bridge/owner/tg.jsonl";
  ...
  it("hydrates only the requested display window for long session history", async () => {
    ...
    const engine = {
      agentsDir: "/tmp/agents",
      currentSessionPath: "/tmp/agents/miko/sessions/long.jsonl",
      deferredResults: null,
    };
  ...
  it("refreshes session file lifecycle metadata when rebuilding history blocks", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/main.jsonl";
  ...
  it("preserves repeated stage_files cards for the same SessionFile in history", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/repeated-stage.jsonl";
  ...
  it("hydrates every legacy file block without fileId from the session file sidecar by path", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/legacy.jsonl";
  ...
  it("restores completed image generation as a session file block and suppresses the iframe card of a since-retired plugin", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/media-restore.jsonl";
    ...
    vi.mocked(msgUtils.loadSessionHistoryMessages).mockResolvedValueOnce([
      ...
      },
      {
        role: "custom",
        customType: "miko-background-result",
        content: `<miko-background-result task-id="task-img" status="success" type="image-generation">\n${resultBody}\n</miko-background-result>`,
        display: false,
      },
    ]);
  ...
  it("restores plugin_card blocks from extension custom messages", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/plugin-card.jsonl";
  ...
  it("normalizes plugin chat surface cards from extension custom messages", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/plugin-chat-surface.jsonl";
        ...
        details: {
          card: {
            ...
            sessionRef: {
              ...
              sessionPath: "/tmp/agents/miko/sessions/stale.jsonl",
            },
            title: "Tavern run",
          },
        },
    ...
    const engine = {
      ...
      getSessionManifest: vi.fn((sessionId) => sessionId === "sess_tavern"
        ? {
          sessionId,
          currentLocator: { path: "/tmp/agents/miko/sessions/tavern-current.jsonl" },
    ...
    expect(data.blocks).toEqual([{
      ...
      card: {
        ...
        type: "chat.surface",
        sessionId: "sess_tavern",
        sessionPath: "/tmp/agents/miko/sessions/tavern-current.jsonl",
        sessionRef: {
          sessionId: "sess_tavern",
          sessionPath: "/tmp/agents/miko/sessions/tavern-current.jsonl",
        },
  ...
  it("restores completed image generation from a non-context deferred result record", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/legacy-ledger.jsonl";
  ...
  it("prefers explicit executor metadata over owner-path inference", async () => {
    ...
    const engine = {
      ...
      getAgent: vi.fn((id) => {
        if (id === "miko") return { agentName: "Miko" };
  ...
  it("marks running subagent block done only after deferred store resolves", async () => {
    ...
    expect(res.status).toBe(200);
    expect(msgUtils.loadLatestAssistantSummaryFromSessionFile).toHaveBeenCalledWith("/tmp/agents/miko/subagent-sessions/child.jsonl");
    expect(data.blocks[0]).toMatchObject({
      type: "subagent",
      streamStatus: "done",
  ...
  it("deduplicates subagent summary reads by sessionId while resolving stale paths to the current locator", async () => {
    ...
    const msgUtils = await import("../core/message-utils.ts");
    const app = new Hono();
    const staleChildPath = "/tmp/agents/miko/subagent-sessions/stale-child.jsonl";
    const currentChildPath = "/tmp/agents/miko/subagent-sessions/current-child.jsonl";
  ...
  it("hydrates running subagent block from durable run store when deferred delivery state is gone", async () => {
    ...
    const engine = {
      ...
      subagentRuns: {
        query: vi.fn(() => ({
          taskId: "subagent-1",
          parentSessionPath: "/tmp/agents/miko/sessions/parent.jsonl",
          childSessionPath: "/tmp/agents/miko/subagent-sessions/child.jsonl",
          status: "resolved",
  ...
  it("exposes structured browser session states and returns refreshed states after close", async () => {
    const { createSessionsRoute } = await import("../server/routes/sessions.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/browser.jsonl";
    const states = {
      [sessionPath]: {
  ...
  it("defaults the session list revision to null when the projection has none", async () => {
    ...
    const engine = {
      listSessions: vi.fn(async () => [{
        path: "/tmp/agents/miko/sessions/in-memory.jsonl",
        title: null,
        firstMessage: "",
        modified: new Date("2026-06-10T07:00:00.000Z"),
        messageCount: 0,
  ...
  it("returns a null messages revision when the session file cannot be stat-ed", async () => {
    const { createSessionsRoute } = await import("../server/routes/sessions.ts");
    const app = new Hono();
    const sessionPath = "/tmp/agents/miko/sessions/raced-away.jsonl";
