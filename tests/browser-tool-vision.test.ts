...
describe("browser screenshot vision adaptation", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-browser-shot-"));
  ...
  it("returns browser status details when a session becomes unavailable", async () => {
    const fatalError = new Error("This feature is available in English only.");
    ...
    expect(result.details).toEqual(expect.objectContaining({
      ...
      error: expect.stringContaining("This feature is available in English only."),
    }));
