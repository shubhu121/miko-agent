...
export const DEFERRED_RESULT_MESSAGE_TYPE = "miko-background-result";
export const DEFERRED_RESULT_RECORD_TYPE = "miko-deferred-result";
...
function formatResultNotification(taskId, result, meta) {
  ...
  return `<miko-background-result task-id="${escapeXml(taskId)}" status="success" type="${type}">\n${body}\n</miko-background-result>`;
}

function formatFailNotification(taskId, reason, meta) {
  const type = escapeXml(meta?.type || "background-task");
  return `<miko-background-result task-id="${escapeXml(taskId)}" status="failed" type="${type}">\n${escapeXml(reason)}\n</miko-background-result>`;
}

function formatAbortNotification(taskId, reason, meta) {
  const type = escapeXml(meta?.type || "background-task");
  return `<miko-background-result task-id="${escapeXml(taskId)}" status="aborted" type="${type}">\n${escapeXml(reason || "task was stopped")}\n</miko-background-result>`;
}
...
export function parseDeferredResultNotification(content) {
  ...
  const match = /^<miko-background-result\b([^>]*)>([\s\S]*?)<\/miko-background-result>$/.exec(text);
