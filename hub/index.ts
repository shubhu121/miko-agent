...
export class Hub {
  ...
  async send(text, opts: any = {}) {
    ...
    if (
      o.images?.length
      && this._engine.mikoHome
      && !o.inboundFiles?.length
      && !hasDisplayImageAttachments(o.displayMessage)
    ) {
      const attachDir = path.join(this._engine.mikoHome, "attachments");
  ...
  initSchedulers() {
    ...
    // SchedulerEnglish onlyheartbeat + cronEnglish only
