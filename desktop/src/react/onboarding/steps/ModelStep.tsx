...
import type { AddedModelEntry, AddedModelObject, DiscoveredModel, MikoFetch } from '../onboarding-actions';
...
export function ModelStep({
  preview, mikoFetch, providerName, providerUrl, providerApi, apiKey,
  goToStep, showError,
}: ModelStepProps) {
  ...
  useEffect(() => {
    const doLoad = async () => {
      ...
      try {
        const result = await loadModelsAction({ mikoFetch, providerName, providerUrl, providerApi, apiKey });
        if (result.error) {
  ...
  const onNext = useCallback(async () => {
    ...
    try {
      await saveModelAction({
        mikoFetch, selectedModel, providerName,
  ...
  }, [preview, canContinue, mikoFetch, selectedModel, providerName, addedModels, selectedUtility, selectedUtilityLarge, goToStep, showError]);
