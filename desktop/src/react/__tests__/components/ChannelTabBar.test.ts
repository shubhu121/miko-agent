...
describe('ChannelTabBar switchTab', () => {
  ...
  it('keeps the right workspace companion state independent from tab switches', () => {
    localStorage.setItem('miko-jian-plugin:miko-hyperframes', 'closed');
    localStorage.setItem('miko-jian-plugin:other-plugin', 'open');

    switchTab('plugin:miko-hyperframes');

    expect(useStore.getState().currentTab).toBe('plugin:miko-hyperframes');
