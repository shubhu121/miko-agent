...
describe("createWechatAdapter", () => {
  ...
  it("persists context tokens so scheduled WeChat replies survive adapter restart", async () => {
    const mikoHome = fs.mkdtempSync(path.join(os.tmpdir(), "miko-wechat-context-"));
  ...
  it("prunes expired persisted WeChat context tokens on restart", async () => {
    const mikoHome = fs.mkdtempSync(path.join(os.tmpdir(), "miko-wechat-context-expired-"));
    vi.setSystemTime(new Date("2026-06-04T08:00:00.000Z"));
    let getUpdatesCount = 0;
    const fetchMock = vi.fn(async (url) => {
      const requestUrl = String(url);
    ...
    await expect(restarted.sendReply("user-1", "too late")).rejects.toThrow("This feature is available in English only.");
