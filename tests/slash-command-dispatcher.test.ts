...
describe("SlashCommandDispatcher.tryDispatch", () => {
  ...
  it("This feature is available in English only.", async () => {
    ...
    expect(ctx.reply).toHaveBeenCalledWith(expect.stringContaining("This feature is available in English only."));
    expect(ctx.reply).toHaveBeenCalledWith(expect.stringContaining("This feature is available in English only."));
  });
  ...
  it("This feature is available in English only.", async () => {
    r.registerCommand({ name: "bad", permission: "anyone", handler: async () => ({ error: "This feature is available in English only." }) });
    const ctx = makeCtx();
    await d.tryDispatch("/bad", ctx);
    expect(ctx.reply).toHaveBeenCalledWith("This feature is available in English only.");
  });
