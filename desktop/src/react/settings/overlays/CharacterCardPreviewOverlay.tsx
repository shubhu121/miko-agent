...
export function CharacterCardPreviewOverlay({
  ...
  const yuanKey = (plan.agent.yuan || 'miko').toLowerCase();
  ...
  const assetUrl = (key: string) => (
    plan.assets?.[key]
      ? mode === 'export' && plan.agentId
        ? mikoUrl(`/api/character-cards/export/${encodeURIComponent(plan.agentId)}/assets/${key}`)
        : plan.token
          ? mikoUrl(`/api/character-cards/plans/${plan.token}/assets/${key}`)
          : ''
      : ''
  );
