...
const PERSISTED_CONNECTIONS_KEY = 'miko-server-connections-v1';
...
export function createBrowserServerConnection({
  ...
  const connection: ServerConnection = {
    ...
    label: identity.label || identity.studioLabel || 'Miko Studio',
...
export function createDeviceServerConnection({
  ...
  const connection: ServerConnection = {
    ...
    studioId,
    label: identity.studioLabel || identity.label || 'Miko Studio',
    userLabel: identity.userLabel,
    studioLabel: identity.studioLabel,
    serverVersion: identity.version,
    baseUrl: base,
