...
describe("SessionManifestStore", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-session-manifest-"));
    nextId = 1;
    nowIndex = 0;
  ...
  it("changes lifecycle classification atomically and preserves deleted locator history", () => {
    ...
    const desktopPath = path.join(tmpDir, "agents", "miko", "sessions", "promoted.jsonl");
  ...
  it("keeps the app-facing locator path when a session is reached through a symlinked directory", () => {
    ...
    const logicalSessionsDir = path.join(tmpDir, "agents", "miko", "sessions");
  ...
  it("This feature is available in English only.", () => {
    ...
    const b = store.createForPath({ sessionPath: pathB, ownerAgentId: "miko", domain: "desktop", kind: "chat" });

    expect(store.backfillOwnerAgentId(a.sessionId, "carol").ownerAgentId).toBe("carol");
    expect(store.backfillOwnerAgentId(b.sessionId, "carol").ownerAgentId).toBe("miko"); 
    expect(store.backfillOwnerAgentId(a.sessionId, "").ownerAgentId).toBe("carol");     
  });
  ...
  it("repairs resolver-on-demand defaults but preserves its receipt", () => {
    ...
    const repaired = store.repairLegacyScanMetadata(manifest.sessionId, {
      ...
      domain: "bridge",
      kind: "bridge_owner",
      provenance: { createdBy: "bridge", bridgeSessionKey: "tg_dm_owner@miko" },
      migration: {
        source: "legacy_scan",
        legacySessionFileName: path.basename(sessionPath),
