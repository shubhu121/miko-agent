...
describe("browser search extractors", () => {
  ...
  it("builds browser search URLs without API keys", () => {
    expect(buildBrowserSearchUrl("bing_browser", "miko search", 3)).toBe(
      "https://www.bing.com/search?q=miko+search&count=3",
    );
    expect(buildBrowserSearchUrl("google_browser", "miko search", 3)).toBe(
      "https://www.google.com/search?q=miko+search&num=3",
    );
    expect(buildBrowserSearchUrl("duckduckgo_browser", "miko search", 3)).toBe(
      "https://duckduckgo.com/?q=miko+search&kl=wt-wt",
    );
  });

  it("builds Bing browser searches with an explicit Chinese market contract", () => {
    expect(buildBrowserSearchUrl("bing_browser", "This feature is available in English only.", 3, { locale: "zh-CN" })).toBe(
  ...
  it("extracts Bing results into a Tavily-like shape", () => {
    ...
    `, "https://www.bing.com/search?q=miko");
  ...
  it("unwraps Google result redirect URLs", () => {
    ...
    `, "https://www.google.com/search?q=miko");
  ...
  it("distinguishes an extractor drift from a genuine Bing no-results page", () => {
    ...
    const noResults = extract("bing_browser", "This feature is available in English only.", "https://www.bing.com/search?q=%E4%B8%8D%E5%AD%98%E5%9C%A8");
