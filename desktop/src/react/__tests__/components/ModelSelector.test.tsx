...
describe('ModelSelector', () => {
  ...
  it('maps a server streaming-switch rejection to the same explicit warning', async () => {
    ...
    vi.mocked(mikoFetch).mockResolvedValueOnce(jsonResponse({
    ...
    expect(mikoFetch).toHaveBeenCalledWith('/api/models/switch', expect.objectContaining({
  ...
  it('applies the selected model thinking default while preparing a new session', async () => {
    ...
    expect(mikoFetch).toHaveBeenCalledWith('/api/models/set', expect.objectContaining({
      method: 'POST',
