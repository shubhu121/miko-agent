...
interface MarkdownCoverElement extends HTMLElement {
  __mikoMarkdownCoverCleanup?: () => void;
}
...
class MarkdownCoverWidget extends WidgetType {
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    if (!src) {
      ...
      wrapper.textContent = "This feature is available in English only.";
      return wrapper;
    }
    ...
    const openMenu = (event: MouseEvent) => {
      ...
      menu.appendChild(makeMenuButton("This feature is available in English only.", () => {
        void saveMarkdownCoverImage(imagePath);
      }));
      menu.appendChild(makeMenuButton("This feature is available in English only.", () => {
        void regenerateMarkdownCoverWithPrompt(this.markdownFilePath);
      }));
      const deleteButton = makeMenuButton("This feature is available in English only.", () => deleteCover(view));
    ...
    wrapper.__mikoMarkdownCoverCleanup = () => {
      closeMenu();
      window.removeEventListener('pointermove', onMove);
      window.removeEventListener('pointerup', finishDrag);
      window.removeEventListener('pointercancel', finishDrag);
  ...
  destroy(dom: HTMLElement): void {
    (dom as MarkdownCoverElement).__mikoMarkdownCoverCleanup?.();
  }
}
