...
export class StudioCronService {
  ...
  /**
   * @param {object} opts
   * @param {string} opts.mikoHome
   * @param {string} opts.agentsDir
   * @param {() => string} opts.getStudioId
   */
  constructor({ mikoHome, agentsDir, getStudioId }) {
    if (!mikoHome) throw new Error("StudioCronService requires mikoHome");
  ...
  _getStore() {
    ...
    if (!this._store || this._storeStudioId !== studioId) {
      const deskDir = path.join(this._mikoHome, "studios", studioId, "desk");
  ...
  _importLegacyJobs(store, studioId) {
    const studioRunsDir = path.join(this._mikoHome, "studios", studioId, "desk", "cron-runs");
