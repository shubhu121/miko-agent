...
const SESSION_FILE_SORT_KEY = 'miko-session-file-sort';
...
const BRIDGE_PLATFORMS: BridgePlatform[] = ['telegram', 'whatsapp'];
...
function bridgeTargetLabel(target: BridgeSendTarget): string {
  const base = `${target.agentName || target.agentId}English only${target.platformLabel}`;
    ...
    try {
      ...
      await Promise.all(bridgeAgents.map(async agent => {
        await Promise.all(BRIDGE_PLATFORMS.map(async platform => {
          const res = await mikoFetch(`/api/bridge/sessions?platform=${encodeURIComponent(platform)}&agentId=${encodeURIComponent(agent.id)}`);
  ...
  const sendFilesToBridge = useCallback(async (filesToSend: FileRef[], target: BridgeSendTarget) => {
    ...
    try {
      for (const file of sendableFiles) {
        const res = await mikoFetch(`/api/bridge/send-media?agentId=${encodeURIComponent(target.agentId)}`, {
          method: 'POST',
