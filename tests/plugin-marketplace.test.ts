...
describe("PluginMarketplace", () => {
  it("loads local marketplace entries and resolves source/readme paths", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-marketplace-"));
    try {
  ...
  it("uses the official Miko Plugins marketplace URL by default", async () => {
  ...
  it("resolves readmePath relative to URL marketplaces", async () => {
    const marketplace = new PluginMarketplace({
      indexUrl: "https://raw.githubusercontent.com/shubhu121/miko-plugins/main/marketplace.json",
      fetchImpl: async (url) => {
        ...
        expect(url).toBe("https://raw.githubusercontent.com/shubhu121/miko-plugins/main/plugins/demo/README.md");
    ...
    expect(data.plugins[0].readmeUrl).toBe("https://raw.githubusercontent.com/shubhu121/miko-plugins/main/plugins/demo/README.md");
