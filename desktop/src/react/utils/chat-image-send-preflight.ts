...
export interface ChatImageModel {
  ...
  compat?: {
    mikoVideoInput?: boolean;
    mikoAudioInput?: boolean;
    audioTransport?: string;
    mikoAudioTransport?: string;
  } | null;
}
...
export function getModelVideoInputMode(model: ChatImageModel | null | undefined): ModelVideoInputMode {
  const explicitVideo = model?.video === true || model?.compat?.mikoVideoInput === true;
...
export function getModelAudioInputMode(model: ChatImageModel | null | undefined): ModelAudioInputMode {
  ...
  const explicitAudio = model.audio === true || model.compat?.mikoAudioInput === true;
  const transport = model.audioTransport || model.compat?.audioTransport || model.compat?.mikoAudioTransport;
  if (explicitAudio) {
    if (model.audioTransportSupported === false || transport === 'unsupported' || transport === 'none') {
      return 'no-native-audio';
    }
    if (model.audioTransportSupported === true || transport === 'openai-input-audio') {
      return 'native-audio';
    }
