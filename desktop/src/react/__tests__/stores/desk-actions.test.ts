...
const mockMikoFetch = vi.fn();

vi.mock('../../hooks/use-miko-fetch', () => ({
  mikoFetch: mockMikoFetch,
}));
...
describe('desk-actions workspace roots', () => {
  beforeEach(() => {
    ...
    mockMikoFetch.mockReset();
    mockMikoFetch.mockImplementation(async (url: string) => {
  ...
  it('loads the selected/home folder when no explicit override is passed', async () => {
    ...
    expect(mockMikoFetch).toHaveBeenNthCalledWith(
      1,
  ...
  it('keeps local workspace history on permission errors', async () => {
    ...
    mockMikoFetch.mockResolvedValueOnce(jsonStatusResponse({
    ...
    expect(mockMikoFetch).toHaveBeenCalledTimes(1);
  });
  ...
  it('keeps the visible mounted workspace files when a refresh fails', async () => {
    ...
    mockMikoFetch.mockResolvedValueOnce(jsonResponse({ error: 'workspace_not_found' }));
  ...
  it('removes recent workspaces locally and persists the history deletion', async () => {
    ...
    mockMikoFetch.mockResolvedValueOnce(jsonResponse({ cwd_history: ['/workspace/Novel'] }));
    ...
    expect(mockMikoFetch).toHaveBeenCalledWith(
      '/api/config/workspaces/recent',
      expect.objectContaining({
        method: 'DELETE',
  ...
  it('removes a Studio workspace mount and clears the selected mount when it was active', async () => {
    ...
    mockMikoFetch.mockImplementation(async (url: string, opts?: RequestInit) => {
  ...
  it('clears recent workspace history through the server API', async () => {
    ...
    mockMikoFetch.mockResolvedValueOnce(jsonResponse({ cwd_history: [] }));
  ...
  it('persists the selected workspace before refreshing the visible desk root', async () => {
    ...
    expect(useStore.getState().deskFiles).toEqual([]);
    expect(mockMikoFetch).toHaveBeenCalledTimes(2);
    expect(mockMikoFetch).toHaveBeenNthCalledWith(
      1,
  ...
  it('persists preview panel layout as part of the workspace UI state', async () => {
    ...
    expect(mockMikoFetch).toHaveBeenCalledWith('/api/preferences/workspace-ui-state', expect.objectContaining({
      method: 'PUT',
    }));
    const [, requestInit] = mockMikoFetch.mock.calls.at(-1) || [];
  ...
  it('persists the workspace snapshot captured before switching roots', async () => {
    ...
    try {
      ...
      const putCall = mockMikoFetch.mock.calls.find(([url, init]) =>
  ...
  it('restores persisted workspace preview state from the backend when memory has no entry', async () => {
    mockMikoFetch.mockResolvedValueOnce(jsonResponse({
      state: {
    ...
    expect(mockMikoFetch).toHaveBeenCalledWith('/api/preferences/workspace-ui-state?workspace=%2Fworkspace&surface=electron');
    expect(useStore.getState().deskCurrentPath).toBe('');
  ...
  it('renames a tree item by explicit parent subdir and updates that tree cache', async () => {
    ...
    expect(ok).toBe(true);
    expect(mockMikoFetch).toHaveBeenCalledWith('/api/desk/files', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({
        action: 'rename',
        dir: '/workspace',
  ...
  it('trashes tree items through the platform trash API and refreshes affected parents', async () => {
    ...
    expect(mockMikoFetch).toHaveBeenCalledWith('/api/desk/files?dir=%2Fworkspace&subdir=notes');
  ...
  it('uses the mobile surface bucket for persisted workspace state in the PWA shell', async () => {
    ...
    expect(mockMikoFetch).toHaveBeenCalledWith('/api/preferences/workspace-ui-state?workspace=%2Fworkspace&surface=pwa');
  ...
  it('safe-deletes tree items through the mobile workbench route in the PWA shell', async () => {
    ...
    expect(ok).toBe(true);
    expect(mockMikoFetch).toHaveBeenCalledWith('/api/workbench/actions', expect.objectContaining({
      method: 'POST',
  ...
  it('uploads browser File objects through the workbench upload route', async () => {
    ...
    expect(ok).toBe(true);
    const call = mockMikoFetch.mock.calls[0];
  ...
  it('searches workspace files against the active desk root', async () => {
    ...
    expect(mockMikoFetch).toHaveBeenCalledWith('/api/desk/search-files?dir=%2Fworkspace&q=Desk');
  ...
  it('jumps to a search result by expanding ancestors and selecting the real tree path', async () => {
    ...
    expect(mockMikoFetch).toHaveBeenNthCalledWith(1, '/api/desk/files?dir=%2Fworkspace&subdir=src');
    expect(mockMikoFetch).toHaveBeenNthCalledWith(2, '/api/desk/files?dir=%2Fworkspace&subdir=src%2Fcomponents');
  ...
  it('reveals an absolute workspace directory by expanding tree paths without replacing the desk root', async () => {
    ...
    mockMikoFetch
      ...
      .mockResolvedValueOnce(jsonResponse({
        files: [{ name: 'miko.png', isDir: false }],
        basePath: '/workspace',
      }));
    ...
    expect(ok).toBe(true);
    expect(mockMikoFetch).toHaveBeenNthCalledWith(1, '/api/desk/files?dir=%2Fworkspace');
    expect(mockMikoFetch).toHaveBeenNthCalledWith(2, '/api/desk/files?dir=%2Fworkspace&subdir=OH-Works');
    expect(mockMikoFetch).toHaveBeenNthCalledWith(3, '/api/desk/files?dir=%2Fworkspace&subdir=OH-Works%2FScreenshots');
    ...
    expect(useStore.getState().deskTreeFilesByPath).toMatchObject({
      ...
      'OH-Works/Screenshots': [{ name: 'miko.png', isDir: false }],
    });
  ...
  it('does not reveal a directory outside the active desk root', async () => {
    ...
    expect(mockMikoFetch).not.toHaveBeenCalled();
