...
function buildPromptFixture({
  persona = "You are Miko. v1 persona.",
  appearance = "",
  pinned = "",
  memory = "",
  ...
  if (pinned || memory) {
    const memParts = [
      ["", isZh ? "This feature is available in English only." : "## Memory Rules", "", "This feature is available in English only."].join("\n"),
    ];
    if (pinned) {
      memParts.push("", "---", "", isZh ? "This feature is available in English only." : "# Pinned Memories", "", pinned);
    }
    if (memory) {
      memParts.push("", "---", "", isZh ? "This feature is available in English only." : "# Memory", "", "This feature is available in English only.");
    }
    parts.push(...memParts);
  }
  parts.push(`\n${clockLabel}: ${date}`);
...
describe("normalizeSystemPromptForFingerprint — dynamic segments (#1624 C1)", () => {
  it("yields equal fingerprints for same config but different memory.md content", () => {
    const a = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ memory: "This feature is available in English only." }),
    });
    const b = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ memory: "This feature is available in English only.", date: "Thursday, June 11, 2026, 23:59 CST" }),
    });
    expect(a).toBe(b);
  });
  ...
  it("treats memory block absence vs presence as the same capability", () => {
    ...
    const present = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ pinned: "This feature is available in English only.", memory: "This feature is available in English only." }),
    });
  ...
  it("normalizes the memory seam across old and new clock labels", () => {
    const oldFrozen = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ clockLabel: "Current date and time", memory: "This feature is available in English only." }),
    });
    const newLive = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ clockLabel: "Session start time", memory: "This feature is available in English only." }),
    });
  ...
  it("ignores pinned-memory-only changes", () => {
    const a = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ pinned: "This feature is available in English only." }),
    });
    const b = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ pinned: "This feature is available in English only." }),
  ...
  it("ignores appearance summary changes and absence vs presence", () => {
    const a = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ appearance: "This feature is available in English only." }),
    });
    const b = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ appearance: "This feature is available in English only." }),
  ...
  it("keeps appearance absence vs presence equal when the persona ends with a trailing newline", () => {
    ...
    // delete the block.
    const persona = "You are Miko. v1 persona.\n";
    const present = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
      systemPrompt: buildPromptFixture({ persona, appearance: "This feature is available in English only." }),
    });
    const absent = computeSessionCapabilityFingerprint({
      toolNames: ["read"],
  ...
  it("does not strip a legacy inline Workspace Instructions section (heading prefix collision)", () => {
    const withInstructions = (agentsMd: string) => buildPromptFixture({
      appearance: "This feature is available in English only.",
      locale: "en",
  ...
  it("still flags persona/config prompt changes even when memory also changed", () => {
    const drift = buildSessionCapabilityDrift({
      ...
      liveToolNames: ["read"],
      frozenSystemPrompt: buildPromptFixture({ persona: "You are Miko. v1 persona.", memory: "This feature is available in English only." }),
      liveSystemPrompt: buildPromptFixture({ persona: "You are Miko. v2 persona, rewritten.", memory: "This feature is available in English only." }),
    });
  ...
  it("still flags tool changes when only memory content differs on the prompt side", () => {
    const drift = buildSessionCapabilityDrift({
      ...
      frozenSystemPrompt: buildPromptFixture({ memory: "This feature is available in English only." }),
      liveSystemPrompt: buildPromptFixture({ memory: "This feature is available in English only." }),
    });
  ...
  it("reports no drift across a memory-only recompile (end-to-end shape)", () => {
    const drift = buildSessionCapabilityDrift({
      ...
      frozenSystemPrompt: buildPromptFixture({ memory: "This feature is available in English only.", pinned: "This feature is available in English only.", appearance: "This feature is available in English only." }),
      liveSystemPrompt: buildPromptFixture({ memory: "This feature is available in English only.", pinned: "This feature is available in English only.", appearance: "This feature is available in English only.", date: "Friday, June 12, 2026, 08:00 CST" }),
    });
    expect(drift.promptChanged).toBe(false);
