...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "miko-provider-media-"));
});
...
describe("ProviderRegistry media capabilities", () => {
  ...
  it("exposes OAuth GPT Image 2 and projects Miko-owned chat defaults through the runtime auth alias", () => {
    const registry = new ProviderRegistry(tmpHome);
    registry.reload();
  ...
  it("retains the last runtime snapshot as stale when a later refresh fails", async () => {
    ...
    registry.registerRuntimeMediaCapabilitySource("runtime-cli", {
      refresh: async () => {
        if (shouldFail) throw new Error("CLI was upgraded while Miko was running");
        return {
          media: {
            imageGeneration: {
    ...
    expect(registry.getRuntimeMediaCapabilityState("runtime-cli")).toMatchObject({
      status: "stale",
      error: expect.objectContaining({ message: "CLI was upgraded while Miko was running" }),
    });
