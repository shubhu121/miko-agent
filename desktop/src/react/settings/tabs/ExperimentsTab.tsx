...
function SwitchButton({
  ...
  return (
    <button
      ...
      className={`miko-toggle${checked ? ' on' : ''}${disabled ? ' disabled' : ''}`}
      disabled={disabled}
      onClick={onClick}
    />
  );
...
function CacheSnapshotExperiment({ experiment, onValueChange }: {
  ...
  const loadObservation = async () => {
    ...
    const res = await mikoFetch(observationUrl);
  ...
  const clearObservation = async () => {
    if (!observationUrl) return;
    await mikoFetch(observationUrl, { method: 'DELETE' });
    setObservation(null);
  };
...
export function ExperimentsTab() {
  ...
  const updateExperimentValue = async (id: string, value: unknown) => {
    const res = await mikoFetch(`/api/experiments/${id}`, {
      method: 'PATCH',
