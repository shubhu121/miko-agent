...
function displayTitle(block: SessionConfirmationBlock) {
  if (block.kind === 'computer_app_approval') {
    ...
    return `Allow Miko to control ${appName}`;
  }
  return block.title;
}
...
export function SessionConfirmationPrompt({ block, exiting = false }: SessionConfirmationPromptProps) {
  ...
  const confirmLabel = block.actions?.confirmLabel || window.t?.('common.approve') || "This feature is available in English only.";
  const rejectLabel = block.actions?.rejectLabel || window.t?.('common.reject') || "This feature is available in English only.";
  ...
  const submit = useCallback(async (action: ConfirmationAction) => {
    ...
    try {
      await mikoFetch(`/api/confirm/${block.confirmId}`, {
        method: 'POST',
  ...
  const disableAskForConversation = useCallback(async () => {
    ...
    try {
      const res = await mikoFetch('/api/session-permission-mode', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      window.dispatchEvent(new CustomEvent('miko-inline-notice', {
        detail: {
          text: textWithFallback('input.accessModeLocked', "This feature is available in English only."),
          type: 'error',
        },
      }));
  ...
  const menu = menuOpen && typeof document !== 'undefined'
    ? createPortal(
      ...
      >
        ...
        >
          {textWithFallback('input.noAskThisConversation', "This feature is available in English only.")}
        </button>
      </div>,
      document.body,
    )
      ...
      {pending ? (
            ...
            <div className={styles['session-confirmation-confirm-wrap']} ref={menuAnchorRef}>
              <div className={styles['session-confirmation-split']}>
                ...
                <button
                  ...
                  aria-label={textWithFallback('input.confirmMoreOptions', "This feature is available in English only.")}
        ...
        <div className={styles['session-confirmation-resolved']}>
          {block.status === 'confirmed'
            ? (window.t?.('common.approved') || "This feature is available in English only.")
            : (window.t?.('common.rejected') || "This feature is available in English only.")}
        </div>
      )}
    </div>
  );
