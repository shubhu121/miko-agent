...
const helperSourceDir = path.join(
  process.cwd(),
  "desktop/native/MikoComputerUseHelper/Sources/MikoComputerUseHelper",
);

describe("Miko computer-use helper cursor integration", () => {
  ...
  it("applies Miko runtime cursor overrides inside the helper process", () => {
    const main = fs.readFileSync(path.join(helperSourceDir, "main.swift"), "utf8");

    expect(main).toContain("MIKO_AGENT_CURSOR_CONFIG_JSON");
    expect(main).toContain("applyMikoCursorRuntimeConfig");
