...
describe("classifyExecutionCwd", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-exec-cwd-"));
  });
...
describe("assertExecutionCwd", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-exec-cwd-assert-"));
  });
  ...
  it("throws MIKO_EXEC_CWD_MISSING with the cwd in the message for a missing directory", () => {
    const gone = path.join(tmpDir, "gone");
    let caught: any;
    try {
    ...
    expect(caught.code).toBe("MIKO_EXEC_CWD_MISSING");
  ...
  it("throws MIKO_EXEC_CWD_NOT_DIRECTORY for a file path", () => {
    const file = path.join(tmpDir, "file.txt");
    fs.writeFileSync(file, "x");
    expect(() => assertExecutionCwd(file)).toThrowError(
      expect.objectContaining({ code: "MIKO_EXEC_CWD_NOT_DIRECTORY" }),
    );
  });

  it("throws MIKO_EXEC_CWD_INVALID for empty input", () => {
    expect(() => assertExecutionCwd("")).toThrowError(
      expect.objectContaining({ code: "MIKO_EXEC_CWD_INVALID" }),
    );
  });

  it("throws MIKO_EXEC_CWD_RELATIVE for relative paths", () => {
    expect(() => assertExecutionCwd("rel/dir")).toThrowError(
      expect.objectContaining({ code: "MIKO_EXEC_CWD_RELATIVE" }),
    );
  });
