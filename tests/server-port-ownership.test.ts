...
describe("server transport ownership", () => {
  it("binds the server endpoint before first-run, engine init, plugins, and schedulers", () => {
    ...
    expect(bindIndex).toBeLessThan(source.indexOf("new MikoEngine("));
  ...
  it("exits on port conflict before first-run or engine initialization", async () => {
    ...
    const mikoHome = fs.mkdtempSync(path.join(os.tmpdir(), "miko-port-conflict-test-"));
    const child = spawn(process.execPath, ["server/bootstrap.ts"], {
      cwd: root,
      env: {
        ...process.env,
        MIKO_HOME: mikoHome,
        MIKO_PORT: String(port),
        MIKO_ROOT: root,
    ...
    try {
      ...
      expect(stdout + stderr).not.toContain("MikoEngine");
    } finally {
      blocker.close();
