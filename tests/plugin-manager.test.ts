...
const tmpHome = path.join(os.tmpdir(), "miko-pm-test-" + Date.now());
  ...
  it("passes runtime scope into lifecycle and tool contexts", async () => {
    ...
    fs.writeFileSync(path.join(dir, "index.js"), `
      export default class ScopePlugin {
        async onload() {
          globalThis.__mikoScopePluginLifecycle = {
    ...
    expect(globalThis.__mikoScopePluginLifecycle).toEqual({
      serverId: "server_plugin",
      serverNodeId: "node_plugin",
      userId: "user_plugin",
      studioId: "studio_plugin",
    ...
    delete globalThis.__mikoScopePluginLifecycle;
  });
...
describe("addTool (dynamic registration)", () => {
  ...
  it("preserves dynamic plugin tool sessionPermission metadata", async () => {
    ...
    expect(tool.sessionPermission.describeSideEffect({ query: "miko" })).toMatchObject({
      kind: "external_api",
      ruleId: "mcp-remote-query",
    });
