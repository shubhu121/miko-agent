...
describe("Agent yuan repair state", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-yuan-repair-"));
    agentsDir = path.join(tmpDir, "agents");
    productDir = path.join(tmpDir, "product");
    userDir = path.join(tmpDir, "user");
    fs.mkdirSync(path.join(productDir, "yuan"), { recursive: true });
    fs.mkdirSync(userDir, { recursive: true });
    fs.writeFileSync(path.join(productDir, "yuan", "miko.md"), "valid yuan\n", "utf-8");
  });
  ...
  it("leaves a valid persisted yuan healthy", () => {
    writeAgentConfig(path.join(agentsDir, "healthy"), "miko");
    const agent = new Agent({
      id: "healthy",
      agentsDir,
      productDir,
