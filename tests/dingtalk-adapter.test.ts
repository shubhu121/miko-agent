...
describe("DingTalk bridge adapter", () => {
  ...
  it("normalizes DingTalk private text callbacks and acknowledges the stream message", async () => {
    ...
    expect(onMessage).toHaveBeenCalledWith(expect.objectContaining({
      ...
      chatId: "manager1234",
      userId: "manager1234",
      sessionKey: "dt_dm_manager1234@miko",
      text: "hello DingTalk!",
      senderName: "Alice",
      displayName: "Alice",
  ...
  it("uses conversation-scoped session keys for group callbacks", async () => {
    ...
    expect(onMessage).toHaveBeenCalledWith(expect.objectContaining({
      chatId: "cid-group",
      userId: "staff-bob",
      sessionKey: "dt_group_cid-group@miko",
      text: "group hello",
      isGroup: true,
    }));
  ...
  it("sends private and group replies through DingTalk robot REST APIs", async () => {
    ...
    expect(JSON.parse(fetchMock.mock.calls[2][1].body)).toEqual({
      ...
      msgParam: JSON.stringify({ title: "Miko", text: "direct hello" }),
    });
    expect(fetchMock).toHaveBeenNthCalledWith(
      4,
    ...
    expect(JSON.parse(fetchMock.mock.calls[3][1].body)).toEqual({
      ...
      msgKey: "sampleMarkdown",
      msgParam: JSON.stringify({ title: "Miko", text: "group hello" }),
    });
  ...
  it("splits long replies before sending DingTalk markdown payloads", async () => {
    ...
    await adapter.sendReply("manager1234", "This feature is available in English only.".repeat(5000), { targetScope: "dm" });
    ...
    expect(first.title).toBe("Miko 1/2");
    expect(second.title).toBe("Miko 2/2");
    expect(first.text + second.text).toBe("This feature is available in English only.".repeat(5000));
