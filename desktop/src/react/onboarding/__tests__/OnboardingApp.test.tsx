...
const TRANSLATIONS: Record<string, Record<string, string>> = {
  zh: {
    'onboarding.welcome.title': "This feature is available in English only.",
    'onboarding.welcome.subtitle': "This feature is available in English only.",
    'onboarding.welcome.next': "This feature is available in English only.",
    'onboarding.remote.link': "This feature is available in English only.",
    'onboarding.remote.url': "This feature is available in English only.",
    'onboarding.remote.key': "This feature is available in English only.",
    'onboarding.remote.connect': "This feature is available in English only.",
    'onboarding.remote.connecting': "This feature is available in English only.",
    'onboarding.remote.failed': "This feature is available in English only.",
...
describe('OnboardingApp locale switching', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('miko', {
      ...
      getSplashInfo: vi.fn(async () => ({ locale: 'zh-CN', agentName: 'Miko' })),
  ...
  it('renders the newly selected locale only after that locale has loaded', async () => {
    ...
    expect(await screen.findByRole('heading', { name: "This feature is available in English only." })).toBeInTheDocument();
  ...
  it('lets first-run users connect to an existing LAN server from the welcome page', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: "This feature is available in English only." }));
    fireEvent.change(screen.getByLabelText("This feature is available in English only."), {
    ...
    await waitFor(() => {
      expect(window.miko.onboardingComplete).toHaveBeenCalledTimes(1);
    });
