...
import { mikoFetch, mikoUrl } from './api';
...
export async function loadAvatars() {
  ...
  const store = useSettingsStore.getState();
  try {
    const res = await mikoFetch('/api/health');
    const data = await res.json();
...
export async function loadSettingsConfig() {
  ...
  try {
    ...
    const [configRes, identityRes, ishikiRes, publicIshikiRes, userProfileRes, pinnedRes, globalModelsRes] =
      await Promise.all([
        mikoFetch(`${agentBase}/config`, { signal: controller.signal }),
        mikoFetch(`${agentBase}/identity`, { signal: controller.signal }),
        mikoFetch(`${agentBase}/ishiki`, { signal: controller.signal }),
        mikoFetch(`${agentBase}/public-ishiki`, { signal: controller.signal }),
        mikoFetch('/api/user-profile', { signal: controller.signal }),
        mikoFetch(`${agentBase}/pinned`, { signal: controller.signal }),
        mikoFetch('/api/preferences/models', { signal: controller.signal }),
      ]);
    ...
    if (config.experience?.enabled === true) {
      const experienceRes = await mikoFetch(`${agentBase}/experience`, { signal: controller.signal });
...
export async function loadSettingsSnapshot(options: { retainSameKeyData?: boolean } = {}) {
  ...
  try {
    const res = await mikoFetch(`/api/settings/snapshot?agentId=${encodeURIComponent(agentId)}`, {
...
export async function loadPluginSettings() {
  ...
  try {
    const [settingsRes, tabsRes] = await Promise.all([
      mikoFetch('/api/plugins/settings'),
      mikoFetch('/api/plugins/settings-tabs'),
    ]);
...
export async function switchToAgent(agentId: string) {
  const store = useSettingsStore.getState();
  try {
    const res = await mikoFetch('/api/agents/switch', {
      method: 'POST',
...
export async function setPrimaryAgent(agentId: string) {
  const store = useSettingsStore.getState();
  try {
    const res = await mikoFetch('/api/agents/primary', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: agentId }),
    });
