...
describe("buildWin32SandboxHelperArgs", () => {
  it("projects the helper contract as write roots instead of read ACL grants", () => {
    const args = buildWin32SandboxHelperArgs({
      cwd: "C:\\work",
      timeoutMs: 5000,
      executable: "C:\\Miko\\resources\\git\\bin\\bash.exe",
      args: ["-lc", "curl https://example.com"],
      grants: {
        readPaths: ["C:\\outside\\brief.md"],
        optionalReadPaths: ["C:\\Users\\Miko"],
        writePaths: ["C:\\work"],
        optionalWritePaths: ["C:\\Users\\Miko\\.miko\\.ephemeral"],
        denyReadPaths: ["C:\\Users\\Miko\\.miko\\auth.json"],
    ...
    expect(args).toEqual([
      ...
      "5000",
      "--",
      "C:\\Miko\\resources\\git\\bin\\bash.exe",
      "-lc",
  ...
  it("filters only exact terminal records across stderr chunk boundaries and flushes other bytes unchanged", () => {
    ...
    filter.push(Buffer.from([
      "warning one\r\n",
      "prefix miko-win-sandbox: terminal-v1 is ordinary stderr\n",
      'miko-win-sandbox: terminal-v1 status="tim',
    ].join("")));
    filter.push(Buffer.from([
  ...
  it("keeps required ACL failures diagnosable with numeric Win32 codes", () => {
    const helperSource = fs.readFileSync(
      path.join(process.cwd(), "desktop", "native", "MikoWindowsSandboxHelper", "main.cpp"),
      "utf-8",
    );
