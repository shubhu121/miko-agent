...
describe("Hub media routing", () => {
  it("preserves clientMessageId when routing desktop session messages", async () => {
    ...
    await hub.send("hello", {
      sessionId: "sess_chat",
      sessionPath: "/agents/miko/sessions/chat.jsonl",
  ...
  it("preserves native audio fields when routing desktop session messages", async () => {
    ...
    await hub.send("listen", {
      sessionPath: "/agents/miko/sessions/audio.jsonl",
      audios,
      audioAttachmentPaths: ["/tmp/miko/voice.wav"],
      displayMessage: {
        text: "listen",
        attachments: [{ path: "/tmp/miko/voice.wav", name: "voice.wav", mimeType: "audio/wav" }],
      },
    });
