...
const { MikoEngine } = await import("../core/engine.ts");

describe("MikoEngine.buildTools session external sandbox grants", () => {
  let tempRoot;
  ...
  it("passes active session external files as read-only sandbox inputs", () => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-engine-sandbox-files-"));
    const mikoHome = path.join(tempRoot, "miko-home");
    ...
    const managedFile = path.join(mikoHome, "session-files", "cache", "shot.png");
    ...
    engine.getAgent = vi.fn(() => ({ id: "miko", agentDir, tools: [] }));
    ...
    engine._agentMgr = { agent: { id: "miko", agentDir, tools: [] } };
  ...
  it("passes the sandbox network preference as a dynamic sandbox option", () => {
    ...
    try {
      tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-engine-sandbox-network-"));
      const mikoHome = path.join(tempRoot, "miko-home");
      const agentDir = path.join(mikoHome, "agents", "miko");
      const workspace = path.join(tempRoot, "workspace");
      fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("keeps Windows restricted-token command sandbox networking enabled at the tool boundary", () => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-engine-win32-sandbox-network-"));
    const originalPlatform = process.platform;
  ...
  it("rejects disabling sandbox networking through MikoEngine on Windows", () => {
    const originalPlatform = process.platform;
    Object.defineProperty(process, "platform", { value: "win32", configurable: true });
  ...
  it("defaults sandbox networking to enabled when the preference has not been written yet", () => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-engine-sandbox-network-default-"));
    const mikoHome = path.join(tempRoot, "miko-home");
    const agentDir = path.join(mikoHome, "agents", "miko");
    const workspace = path.join(tempRoot, "workspace");
    fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("builds file tools through ResourceIO without a runtime switch", () => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-engine-resource-io-tools-"));
    const mikoHome = path.join(tempRoot, "miko-home");
    const agentDir = path.join(mikoHome, "agents", "miko");
    const workspace = path.join(tempRoot, "workspace");
    fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("includes inherited parent session files in read-only sandbox inputs", () => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-engine-sandbox-parent-files-"));
    const mikoHome = path.join(tempRoot, "miko-home");
    const agentDir = path.join(mikoHome, "agents", "miko");
    const workspace = path.join(tempRoot, "workspace");
