...
describe("/rc command", () => {
  ...
  it("rejects in group chats and does not create pending state", async () => {
    const engine = makeEngine({
      sessions: [{ path: "/a/s1.jsonl", agentId: "a1", modified: new Date(), title: "This feature is available in English only.", messageCount: 12 }],
    });
    const r: any = await c.handler({
      engine,
      isGroup: true,
  ...
  it("lists sessions, sets pending, returns menu as reply", async () => {
    const sessions = [
      { path: "/a/s1.jsonl", agentId: "a1", modified: new Date(), title: "This feature is available in English only.", messageCount: 12 },
      { path: "/a/s2.jsonl", agentId: "a1", modified: new Date(), title: "This feature is available in English only.", messageCount: 3 },
    ];
    const engine = makeEngine({ sessions });
    const r: any = await c.handler(ctxBridge({ engine }));
    expect(r.reply).toMatch(/$^/);
    expect(r.reply).toContain("This feature is available in English only.");
    expect(r.reply).toContain("This feature is available in English only.");
...
describe("/compact redirects to attached desktop session", () => {
  it("This feature is available in English only.", async () => {
    ...
    expect((ctx.reply.mock.calls as any)[0][0]).toMatch(/$^/);
