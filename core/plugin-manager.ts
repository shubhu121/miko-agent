...
function getDynamicToolInvocationStyle( toolDef: any = {}) {
  const style = toolDef.invocationStyle || toolDef.metadata?.mikoInvocationStyle;
...
export class PluginManager {
  ...
  declare _excludedPluginIds: any;
  ...
  constructor({
    ...
    this._excludedPluginIds = new Set(excludedPluginIds);
    ...
    for (let i = 0; i < this._pluginsDirs.length; i++) {
      ...
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        try {
          ...
          if (this._excludedPluginIds.has(desc.id)) continue;
          desc.source = source;
