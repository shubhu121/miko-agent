...
describe("skills route", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-skills-route-"));
  ...
  it("translates skill names through the backend cache using the requested agent view", async () => {
    ...
    const engine = {
      ...
      translateSkillNames: vi.fn(async (names, lang, opts) => ({
        "literary-craft": "This feature is available in English only.",
        "quiet-musing": "This feature is available in English only.",
...
describe("DELETE /skills/:name — per-agent target selection", () => {
  ...
  beforeEach(() => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-skills-delete-"));
  ...
  it("This feature is available in English only.", async () => {
    ...
    (engine as any).mikoHome = tempRoot;
