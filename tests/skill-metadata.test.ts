...
function makeTmpRoot() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-skill-metadata-"));
  tmpRoots.push(dir);
  return dir;
}
...
describe("SkillManager metadata scanning", () => {
  ...
  it("resource-loader skills can opt out of default enablement through SKILL.md metadata", () => {
    const root = makeTmpRoot();
    const skillDir = path.join(root, "skills", "miko-plugin-creator");
    const skillFile = path.join(skillDir, "SKILL.md");
    fs.mkdirSync(skillDir, { recursive: true });
    fs.writeFileSync(skillFile, [
      "---",
      "name: miko-plugin-creator",
      "description: Create Miko plugins.",
      "metadata:",
      "  default-enabled: false",
      "---",
      "",
    ...
    manager.init(
      { getSkills: () => ({ skills: [{ name: "miko-plugin-creator", source: "user", filePath: skillFile }], diagnostics: [] }) },
      new Map(),
      new Set(),
    );
