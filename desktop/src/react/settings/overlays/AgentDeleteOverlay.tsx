...
export function AgentDeleteOverlay() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('miko-show-agent-delete', handler);
    return () => window.removeEventListener('miko-show-agent-delete', handler);
  }, []);
  ...
  const confirmDelete = async () => {
    ...
    setError('');
    try {
      const res = await mikoFetch(`/api/agents/${targetId}`, { method: 'DELETE' });
