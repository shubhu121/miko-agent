...
describe("createFeishuAdapter", () => {
      ...
      message: {
        ...
        content: JSON.stringify({
          zh_cn: {
            ...
            content: [
              [
                { tag: "text", text: "This feature is available in English only." },
                { tag: "at", user_name: "This feature is available in English only.", user_id: "ou_ming" },
                { tag: "a", text: "This feature is available in English only.", href: "https://example.com" },
                { tag: "md", text: "This feature is available in English only." },
              ],
              [
  ...
  it("drops only self bot/app echoes while keeping other bot messages if Feishu delivers them", async () => {
    ...
    expect(onMessage).toHaveBeenCalledWith(expect.objectContaining({
      text: "other bot",
      userId: "ou_other_bot",
      sessionKey: "fs_group_oc_group@miko",
    }));
  });
  ...
  it("renders explicit Feishu at tokens as post at elements", async () => {
    ...
    await adapter.sendReply("oc_chat", "This feature is available in English only.");
  ...
  it("declares Feishu CardKit rich streaming and updates markdown content by sequence", async () => {
    ...
    expect(state).toEqual({
      ...
      elementId: "miko_stream_markdown",
      sequence: 5,
    });
    ...
    expect(JSON.parse(createCardPayload.data.data)).toMatchObject({
      ...
      body: {
        elements: [{
          tag: "markdown",
          element_id: "miko_stream_markdown",
          content: "first",
        }],
      },
    });
    ...
    expect(mockCardElementContent).toHaveBeenNthCalledWith(1, {
      path: { card_id: "card_stream_001", element_id: "miko_stream_markdown" },
      data: {
        content: "second",
        sequence: 3,
      },
  ...
  it("moves Feishu streams from post to interactive card when markdown tables appear", async () => {
    ...
    expect(mockMessageUpdate).toHaveBeenNthCalledWith(1, {
      ...
      data: {
        msg_type: "post",
        content: markdownPostContent("This feature is available in English only."),
      },
    });
    expect(mockMessageCreate).toHaveBeenNthCalledWith(2, {
      params: { receive_id_type: "chat_id" },
