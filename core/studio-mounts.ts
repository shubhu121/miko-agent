...
const STORAGE_PROVIDERS = ["local_fs", "webdav", "s3", "google_drive", "miko_cloud"];
...
export function ensureStudioMountRegistry(mikoHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(mikoHome, STUDIO_MOUNTS_FILE);
...
export function loadStudioMountRegistry(mikoHome) {
  ensureStudioMountRegistry(mikoHome);
  const registry = readJsonRequired(path.join(mikoHome, STUDIO_MOUNTS_FILE), STUDIO_MOUNTS_FILE);
  validateStudioMountRegistry(registry);
  return registry;
}

export function upsertStudioMount(mikoHome, mount, { now = new Date().toISOString() } = {}) {
  const registry = loadStudioMountRegistry(mikoHome);
  ...
  validateStudioMountRegistry(registry);
  writeJsonAtomic(path.join(mikoHome, STUDIO_MOUNTS_FILE), registry);
...
export function listStudioMountsForStudio(mikoHome, hostStudioId) {
