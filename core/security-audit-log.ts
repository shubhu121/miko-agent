...
export function securityAuditLogPath(mikoHome) {
  if (!mikoHome) throw new Error("mikoHome required");
  return path.join(mikoHome, "logs", SECURITY_AUDIT_LOG_FILE);
}

export function appendSecurityAuditEvent(mikoHome, event, {
  now = new Date().toISOString(),
  eventId = `sec_${crypto.randomUUID()}`,
} = {}) {
  if (!mikoHome) return null;
  const record = normalizeAuditEvent(event, { now, eventId });
  const filePath = securityAuditLogPath(mikoHome);
  try {
