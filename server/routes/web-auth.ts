...
export function createWebAuthRoute({
  ...
  route.post("/web-auth/login", async (c) => {
    ...
    const issued = createWebSession(mikoHome, {
      principal,
  ...
  route.post("/web-auth/logout", async (c) => {
    revokeWebSession(mikoHome, c.req.header("cookie"), { now: now() });
...
function authenticatePasswordLogin(c, {
  ...
  const verified = verifyLocalAccountPassword(mikoHome, { username, password });
