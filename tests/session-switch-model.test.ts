...
const sessionPath = `${agentsDir}/miko/sessions/session.jsonl`;
const missingSessionPath = `${agentsDir}/miko/sessions/missing.jsonl`;
...
describe("SessionCoordinator.switchSessionModel", () => {
  ...
  it("reports per-session model switch state through a public query", () => {
    const coord = new SessionCoordinator({
      agentsDir,
      getAgent: () => ({ sessionDir: `${agentsDir}/miko/sessions` }),
  ...
  it("passes model-switch lifecycle options through _compactWithModel", async () => {
    ...
    const sessionPath = path.join(agentsDir, "miko", "sessions", "compact.jsonl");
  ...
  it("emits lifecycle and session_compact events for model-switch hard truncation", async () => {
    ...
    const sessionPath = path.join(agentsDir, "miko", "sessions", "truncate.jsonl");
