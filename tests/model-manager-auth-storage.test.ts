...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-model-manager-auth-"));
});
...
describe("ModelManager AuthStorage ownership", () => {
  it("registers Grok OAuth with Pi and exposes subscription models only when logged in", async () => {
    ...
    const manager = new ModelManager({ mikoHome: tmpDir });
    manager.init();
  ...
  it("builds the Miko-owned Codex default catalog before ModelRegistry and exposes it only when OAuth is logged in", async () => {
    writeAddedModels({});
    writeAuth({
      "openai-codex": {
        type: "oauth",
  ...
  it.each([
    ...
    const restarted = new ModelManager({ mikoHome: tmpDir });
    restarted.init();
  ...
  it("injects added-models API keys as runtime overrides before Pi SDK env resolution", async () => {
    ...
    try {
      ...
      const projected = JSON.parse(fs.readFileSync(path.join(tmpDir, "models.json"), "utf-8"));
      expect(projected.providers.deepseek.apiKey).toBe("miko-runtime-api-key:deepseek");
    } finally {
  ...
  it("re-applies Miko provider model metadata after Pi SDK availability filtering", async () => {
    writeAuth({});
    writeAddedModels({
      "local-max": {
