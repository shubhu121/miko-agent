...
function mktemp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "miko-channel-test-"));
}
...
describe("ChannelManager", () => {
  ...
  describe("repairChannelCursorProjection", () => {
    it("adds missing agent cursor entries from channel membership without changing channel data", async () => {
      writeChannelMd(channelsDir, "ch_team", ["miko", "yui", "ghost"]);
      fs.mkdirSync(path.join(agentsDir, "miko"), { recursive: true });
      fs.writeFileSync(path.join(agentsDir, "miko", "config.yaml"), "agent:\n  name: Miko\n", "utf-8");
      fs.writeFileSync(path.join(agentsDir, "miko", "channels.md"), "This feature is available in English only.", "utf-8");
      ...
      expect(readBookmarks(path.join(agentsDir, "miko", "channels.md")).get("ch_team")).toBe("never");
      ...
      expect(readMembers(channelsDir, "ch_team")).toEqual(["miko", "yui", "ghost"]);
    });
  });
