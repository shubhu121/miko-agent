---
name: miko-plugin-creator
description: Create Miko plugin scaffolds and guide users through beginner or developer plugin planning, capability checks, manifest setup, runtime tools, WebView/iframe UI, Session/Agent APIs, model/media APIs, SDK templates, and install-ready plugin directories. Use when Miko/Codex needs to explain what Miko plugins can do, help a user describe a plugin idea, check whether the SDK supports it, or generate/update a Miko plugin with @miko/plugin-runtime, @miko/plugin-sdk, and @miko/plugin-components.
...
Use this skill for Miko application plugins, not Codex `.codex-plugin` bundles.

## First Contact

On first use, give a map, not an encyclopedia. Explain what Miko plugins can add, ask what the user wants to build, and invite follow-up questions. Expand details only after the user asks or after the chosen scaffold needs them.
...
- If memory is unavailable, disabled, or uncertain, ask: `Which path fits you? A. I need a guided plan. B. I want SDK and API details.`

Beginner mode tone: encouraging, concrete, and guided. Say that the user can describe the feature in plain language, Miko will help turn it into a plugin plan and scaffold, and Miko can answer questions at any step. Ask:

1. What should the plugin help people do in Miko?
2. Does it need to call an external API, work with files, add an Agent tool, or show a user interface?
3. Is this a personal prototype, a development plugin, or a distributable plugin?
...
After delivering a plugin, explain the concrete situation it helps with, such as reducing repeated steps, making an external service available inside Miko, turning a manual workflow into an Agent-callable tool, or giving a recurring task a stable UI. Use grounded wording such as `This removes a repeated manual step` or `The Agent can now call this workflow when needed`. Avoid inflated praise.

## Capability Map

Miko plugins can provide:

- Agent-callable tools and slash-style actions.
- Skills, agents, and knowledge that guide model behavior.
- WebView/iframe pages, widgets, and cards using Miko theme and host capabilities.
...
- Session and Agent control through `@miko/plugin-runtime`: create/list/update/send/abort/history sessions, subscribe to session events, create/read/update plugin-owned agents, and hide plugin-private resources from the main Miko UI.
...
Miko provides install/enable/reload, per-agent skill toggles, manifest capability checks, iframe host messaging, theme tokens, toast/clipboard/external host APIs, EventBus, data directories, and SDK packages.
...
```bash
node skills2set/miko-plugin-creator/scripts/check_env.mjs --capability scaffold
```

Behavior:

- The preflight itself is JavaScript and uses only Node built-ins.
- It finds Python through `MIKO_PLUGIN_CREATOR_PYTHON`, `python3`, `python`, or Windows `py -3`.
...
1. Find the Miko repo root. Prefer the current workspace if it contains `PLUGIN_SDK.md`, `PLUGINS.md`, and `packages/plugin-runtime`.
...
3. Pick a template:
   ...
   - `guided-react`: React/Vite/SDK starter with shared Miko components and a gentler README.
...
5. Pick the target location:
   - Built-in plugin shipped with Miko: `plugins/<plugin-id>`.
   - Example or template plugin: `examples/plugins/<plugin-id>`.
   - User-installed plugin: the directory reported by `/api/plugins/settings` or `${MIKO_HOME}/plugins`.
6. Generate the scaffold with the bundled script, then adjust names, descriptions, tools, routes, capabilities, and UI to the user's request.
   - When converting an existing website or single-page app into a Miko iframe plugin, rewrite same-plugin browser `fetch('/api/...')` calls to `miko.api.fetch(...)`, move static files under `assets/`, and use `miko.assets.url(...)` for browser-side asset references.
...
```bash
python3 skills2set/miko-plugin-creator/scripts/create_miko_plugin.py "My Plugin" --path examples/plugins --audience beginner --template direct
```

Developer React starter:

```bash
python3 skills2set/miko-plugin-creator/scripts/create_miko_plugin.py "My Plugin" --path examples/plugins --audience developer --template professional-react --sdk-mode workspace
```
...
```bash
python3 skills2set/miko-plugin-creator/scripts/create_miko_plugin.py "Jimeng Provider" --path examples/plugins --kind provider --audience developer
```
...
- React templates may use `@miko/plugin-runtime`, `@miko/plugin-sdk`, and `@miko/plugin-components`.
- Static iframe resources belong under `assets/` and should be referenced with `miko.assets.url(path)` from browser code or the official `/api/plugins/{pluginId}/assets/...` path from the route shell. This includes CSS, JS, images, fonts, JSON, wasm, and browser-playable videos such as MP4/WebM/MOV. Do not inline large assets as a workaround.
...
- Dev authority is not a manifest permission. Miko grants it from the remembered dev install slot under `${MIKO_HOME}/plugins-dev/`, and Agent dev tools are hidden until the user enables the dev tools setting.
...
- Iframe browser code must not call third-party APIs directly. It should call a same-plugin route with `miko.api.fetch(...)`; the route reads config/secrets server-side and calls `ctx.network.fetch(...)`.
...
- Browser iframe code may call `miko.resources.open()`, `miko.resources.pick()`, or `miko.resources.requestAccess()` as host-mediated requests only. It must not read or write filesystem content directly; server-side plugin routes and tools should use `ctx.resources` for actual resource operations.
- If iframe code calls `miko.resources.open()`, `miko.resources.pick()`, or `miko.resources.requestAccess()`, declare the matching `resource.open`, `resource.pick`, or `resource.requestAccess` entry under `ui.hostCapabilities`. Do not request resource host grants in templates that do not use those calls.
...
- `createSession()` creates a detached Miko session and does not switch the main UI focus. Use `visibility: "plugin_private"` and `ownerPluginId` for plugin-only sessions or Tavern-style parallel chat surfaces.
- Use `createChatSurfaceCard(ctx, session.sessionRef ?? session, options)` when a plugin wants to display its own private session in chat. Do not hand-build path-only `chat.surface` payloads; the helper requires `sessionId` / `sessionRef` and Miko verifies plugin ownership before rendering.
...
- After full-access plugin install, enable, or reload, Miko rebinds extension runners for idle sessions. Busy sessions pick up the change on the next safe rebuild, so do not promise that an in-flight reply will use freshly edited extension code.
- Declare only the iframe host capabilities actually used.
- EventBus handlers should return `MIKO_BUS_SKIP` for payloads that do not belong to them.
...
- Marketplace metadata lives in the `OH-Plugins` repository, not inside `project-miko`.
...
- For a single release, root `version`, `compatibility`, and `distribution` remain valid; Miko normalizes them into a single version entry.
- Miko selects the highest SemVer version compatible with the current app and exposes update, reinstall, incompatible, and downgrade states to the UI.
...
- Default React plugin UI to `MikoThemeProvider mode="inherit"` so it follows the host theme.
- Use `mode="miko"` for a named Miko theme, and `mode="custom"` only for explicit token overrides.
- Route shells should read `miko-theme` and `miko-css` query params, include the theme CSS link when present, and escape values inserted into HTML attributes.
...
- Direct templates include `miko.api.fetch()` for plugin route calls; preserve that helper when simplifying generated browser code.
...
- Live data APIs belong behind plugin routes that call `ctx.network.fetch()`. The browser page calls `miko.api.fetch('api/...')`, receives sanitized JSON, and renders it locally.
- First paint must not auto-call LLM APIs. Trigger model calls only from an explicit user action, then call a plugin backend route with `miko.api.fetch(...)`; the backend route may use `sampleText()` or other runtime helpers.
