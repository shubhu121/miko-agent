...
export function AccessTab() {
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      const res = await mikoFetch('/api/access/summary');
  ...
  const qrUrl = useMemo(() => {
    ...
    const query = summary?.network.actualPort
      ...
      : '';
    return mikoUrl(`/api/access/mobile-qr.svg${query}`);
  ...
  const saveNetworkSettings = useCallback(async (nextMode: AccessMode, nextPort: string) => {
    ...
    try {
      const res = await mikoFetch('/api/access/network', {
        method: 'PUT',
  ...
  const generateMobileKey = useCallback(async () => {
    ...
    try {
      const res = await mikoFetch('/api/access/mobile-credentials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const generateDesktopKey = useCallback(async () => {
    ...
    try {
      const res = await mikoFetch('/api/access/desktop-credentials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const saveAccount = useCallback(async () => {
    try {
      const res = await mikoFetch('/api/access/account/profile', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const savePassword = useCallback(async () => {
    try {
      const res = await mikoFetch('/api/access/account/password', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const clearPassword = useCallback(async () => {
    try {
      const res = await mikoFetch('/api/access/account/password', { method: 'DELETE' });
      const data = await res.json();
      setSummary(prev => prev ? { ...prev, account: data.account } : prev);
      setPasswordDraft('');
  ...
  const revokeDevice = useCallback(async (deviceId: string) => {
    try {
      await mikoFetch(`/api/devices/${encodeURIComponent(deviceId)}/revoke`, { method: 'POST' });
      await loadSummary();
  ...
  const revokeCredential = useCallback(async (credentialId: string) => {
    try {
      await mikoFetch(`/api/devices/credentials/${encodeURIComponent(credentialId)}/revoke`, { method: 'POST' });
      await loadSummary();
