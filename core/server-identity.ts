...
export function loadServerIdentity(mikoHome) {
  const serverNode = readRequiredIdentityJson(path.join(mikoHome, SERVER_NODE_FILE), SERVER_NODE_FILE);
  const users = readRequiredIdentityJson(path.join(mikoHome, USERS_FILE), USERS_FILE);
  const studios = readRequiredStudioRegistry(mikoHome);
...
export function ensureLocalIdentityRegistries(mikoHome) {
  const serverNodePath = path.join(mikoHome, SERVER_NODE_FILE);
  const usersPath = path.join(mikoHome, USERS_FILE);
  const studiosPath = path.join(mikoHome, STUDIOS_FILE);
  const legacySpacesPath = path.join(mikoHome, LEGACY_SPACES_FILE);
  ...
  const foundationRegistries = ensureRemoteAccessFoundationRegistries(mikoHome, { now });
...
export function ensureRemoteAccessFoundationRegistries(mikoHome, { now = new Date().toISOString() } = {}) {
  return {
    created: [
      ...ensureDeviceAccessRegistries(mikoHome, { now }).created,
      ...ensureServerNetworkConfig(mikoHome, { now }).created,
      ...ensureStudioMountRegistry(mikoHome, { now }).created,
      ...ensureSecurityRegistries(mikoHome, { now }).created,
    ],
  };
}

function ensureSecurityRegistries(mikoHome, { now }) {
  const created = [];
  const grantPath = path.join(mikoHome, "security", "grants.json");
  const leasePath = path.join(mikoHome, "security", "execution-leases.json");
  const hadGrant = fs.existsSync(grantPath);
  const hadLease = fs.existsSync(leasePath);
  ensureGrantRegistry(mikoHome, { now });
...
function readRequiredStudioRegistry(mikoHome) {
  const studiosPath = path.join(mikoHome, STUDIOS_FILE);
