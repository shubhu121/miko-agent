...
describe("resource-io route", () => {
  ...
  it("returns sanitized ResourceIO denial errors", async () => {
    const resourceIO = {
      write: vi.fn(async () => {
        const err: any = new Error("Denied /tmp/miko-fixture/private/repo/.git/config");
    ...
    const res = await app.request("/api/resource-io/write", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        resource: { kind: "local-file", path: "/tmp/miko-fixture/private/repo/.git/config" },
        content: "bad",
      }),
    });
    ...
    expect(JSON.stringify(body)).not.toContain("/tmp/miko-fixture");
  });
});
