...
describe("Windows sandbox helper build script", () => {
  ...
  it("links the Win32 libraries required by restricted tokens, ACL APIs, and private desktops", () => {
    const command = buildWindowsSandboxCompileCommand({
      source: "C:\\repo\\desktop\\native\\MikoWindowsSandboxHelper\\main.cpp",
      output: "C:\\repo\\dist-sandbox\\win-x64\\miko-win-sandbox.exe",
    });
  ...
  it("uses restricted-token APIs instead of AppContainer launch APIs", () => {
    const source = fs.readFileSync(
      path.resolve(__dirname, "../desktop/native/MikoWindowsSandboxHelper/main.cpp"),
      "utf8"
    );
  ...
  it("uses ordinary Miko write SIDs while retaining legacy capability ACL cleanup", () => {
    const source = fs.readFileSync(
      path.resolve(__dirname, "../desktop/native/MikoWindowsSandboxHelper/main.cpp"),
      "utf8"
    );
    ...
    expect(source).toContain("--cleanup-miko-write-acl");
    expect(source).toContain("miko-write-acl-cleaned");
  });
  ...
  it("logs structured launch diagnostics when CreateProcessAsUserW fails", () => {
    ...
    expect(source).toContain("miko-win-sandbox: launch-failure");
  ...
  it("keeps Windows dev startup fast when the native helper is current", () => {
    ...
    const target = path.join(rootDir, "dist-sandbox", "win-x64", "miko-win-sandbox.exe");
    const build = vi.fn();
  ...
  it("builds the Windows dev helper when missing or stale, and is a no-op elsewhere", () => {
    ...
    const result = ensureWindowsSandboxHelper({
      ...
      platform: "win32",
      arch: "x64",
      existsSync: (candidate: string) => !candidate.endsWith("miko-win-sandbox.exe"),
      statSync: () => ({ mtimeMs: 10 }),
      build,
    });
