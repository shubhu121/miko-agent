...
export class ModelManager {
  ...
  constructor({ mikoHome }) {
    ...
    this.providerRegistry = new ProviderRegistry(mikoHome);
    this.executionRouter = null;
  }

  
  init() {
    this._authStorage = AuthStorage.create(path.join(this._mikoHome, "auth.json"));
    ...
    this._modelRegistry = createModelRegistry(
      this._authStorage,
      path.join(this._mikoHome, "models.json"),
    );
  ...
  get modelsJsonPath() { return path.join(this._mikoHome, "models.json"); }
  get authJsonPath() { return path.join(this._mikoHome, "auth.json"); }
  ...
  /** Convert Miko-visible thinking levels to the Pi SDK session contract. */
