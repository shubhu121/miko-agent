...
function initialBridgePlatform(): BridgePlatform {
  const saved = localStorage.getItem('miko_bridge_tab');
  return isBridgePlatform(saved) ? saved : 'telegram';
}
...
export function BridgePanel() {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      const res = await mikoFetch(`/api/bridge/status${query}`);
  ...
  const loadPlatformData = useCallback(async (plat: BridgePlatform) => {
    ...
    try {
      ...
      const [statusRes, sessionsRes] = await Promise.all([
        mikoFetch(`/api/bridge/status${snapshotId ? `?agentId=${encodeURIComponent(snapshotId)}` : ''}`),
        mikoFetch(`/api/bridge/sessions?platform=${plat}${agentQuery}`),
      ]);
      if (bridgeAgentIdRef.current !== snapshotId) return; // stale
  ...
  const switchTab = useCallback((plat: BridgePlatform) => {
    ...
    setCurrentSessionPath(null);
    setChatOpen(false);
    localStorage.setItem('miko_bridge_tab', plat);
  ...
  const resetSession = useCallback(async () => {
    ...
    try {
      const agentQuery = snapshotId ? `?agentId=${encodeURIComponent(snapshotId)}` : '';
      await mikoFetch(`/api/bridge/sessions/${encodeURIComponent(currentKey)}/reset${agentQuery}`, { method: 'POST' });
      if (bridgeAgentIdRef.current !== snapshotId) return; // stale
