...
export function ActivityPanel() {
  ...
  const promoteActivity = useCallback(async (activityId: string) => {
    try {
      const res = await mikoFetch(`/api/desk/activities/${activityId}/promote`, { method: 'POST' });
        ...
        <div id="activityListView" style={FLEX_COLUMN_STYLE}>
          <div className={fp.floatingPanelHeader}>
              ...
              <div className={fp.activityHbToggle}>
                <span className="miko-toggle-label">{t('activity.heartbeat')}</span>
                <button
                  className={'miko-toggle' + (hbEnabled ? ' on' : '')}
...
function ActivityCard({
  ...
  useEffect(() => {
    ...
    void mikoFetch(`/api/desk/activities/${a.id}/session`)
      .then(res => res.json())
      .then(data => {
        if (cancelled) return;
