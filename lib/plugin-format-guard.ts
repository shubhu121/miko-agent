...
function createOpenClawIssue({ marker, id, name, version }) {
  return {
    ...
    name,
    version,
    message: `This package looks like an OpenClaw plugin (${marker}). Miko cannot install OpenClaw plugin zips directly. Use a Miko plugin package with manifest.json, or port the plugin to Miko's plugin SDK.`,
  };
}
