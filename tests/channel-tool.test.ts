...
function mktemp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "miko-channel-tool-test-"));
}
...
describe("channel tool membership contract", () => {
  ...
  it("lists joined channels with ids and display names", async () => {
    ...
    expect(result.details).toMatchObject({
      action: "list",
      channels: [
        expect.objectContaining({ id, name: "This feature is available in English only.", members: ["alice", "bob"] }),
      ],
    });
  ...
  it("resolves a unique display name for read and post", async () => {
    ...
    const readResult = await tool.execute("call-4", {
      action: "read",
      channel: "This feature is available in English only.",
    });
