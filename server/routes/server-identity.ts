...
export function createServerIdentityRoute({ mikoHome, appVersion = "?", getRuntimeContext }: { mikoHome?: string; appVersion?: string; getRuntimeContext?: () => any } = {}) {
  ...
  route.get("/server/identity", (c) => {
    try {
      const runtimeContext = typeof getRuntimeContext === "function"
        ? getRuntimeContext()
        : createServerRuntimeContext({ mikoHome, appVersion });
