...
export function resourceTicketKeyPath(mikoHome: any) {
  assertNonEmpty(mikoHome, "mikoHome");
  return path.join(mikoHome, "security", RESOURCE_TICKET_KEY_FILE);
}

function signBody(mikoHome: any, body: any) {
  return crypto
...
function readOrCreateTicketKey(mikoHome: any) {
  const filePath = resourceTicketKeyPath(mikoHome);
  try {
