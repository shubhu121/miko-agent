...
function normalizeThrownToolError(err: any, maxOutputTokens: number) {
  if (err?.mikoCommandBlockedResult) {
    return firstText(err.mikoCommandBlockedResult);
  }
...
function getTempFilePath() {
  ...
  return join(tmpdir(), `miko-exec-command-${id}.log`);
}
...
export async function runExecCommandDirect({
  ...
  try {
    ...
    collector.close();
    if (err?.mikoCommandBlockedResult) {
      return mergeExecDetails(err.mikoCommandBlockedResult, {
        ...execDetails,
        ok: false,
        transportError: false,
