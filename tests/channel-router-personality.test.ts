...
describe("ChannelRouter._executeCheck phone delivery", () => {
  it("does not run utility gating or disk personality fallback before the phone session", async () => {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "miko-channel-router-personality-"));
    ...
    const mockAgent = {
      ...
      personality: "This feature is available in English only.",
    };
    ...
    const router = new ChannelRouter({
      hub: {
        engine: {
          ...
          channelsDir,
          userDir,
          agents: new Map([["miko", mockAgent]]),
          getAgent: (id) => (id === "miko" ? mockAgent : null),
