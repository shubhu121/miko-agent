...
const MARKER_FILE = ".miko-sandbox-runtime.json";
...
function ensureCachedRuntimeRoot({ sourceRoot, primaryPath, mikoHome, kind }) {
  if (!mikoHome) {
    throw new Error("[win32-sandbox] MIKO_HOME is required to prepare sandbox runtime cache.");
  }
...
export function sandboxRuntimeCacheRoot(mikoHome) {
  if (!mikoHome) throw new Error("[win32-sandbox] MIKO_HOME is required for sandbox runtime cache.");
  return joinRuntimePath(mikoHome, ".ephemeral", CACHE_DIR);
}

export function prepareSandboxRuntime(runtimeInfo, { mikoHome, kind }) {
  ...
  const targetRoot = ensureCachedRuntimeRoot({ sourceRoot, primaryPath, mikoHome, kind });
