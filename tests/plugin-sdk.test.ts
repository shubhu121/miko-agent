...
import { createMikoPluginSdk, MikoPluginError } from '@miko/plugin-sdk';
...
describe('plugin SDK', () => {
  ...
  it('rejects host request errors with a typed MikoPluginError', async () => {
    const parentWindow = makeParentWindow();
    const sdk = createMikoPluginSdk({
      parentWindow,
      targetWindow: window,
    ...
    await expect(pending).rejects.toMatchObject({
      name: 'MikoPluginError',
      code: 'CAPABILITY_DENIED',
      message: 'Capability denied.',
      details: { capability: 'external.open' },
    } satisfies Partial<MikoPluginError>);
  });
  ...
  it('resolves plugin asset URLs from the current iframe route', () => {
    const parentWindow = makeParentWindow();
    const targetWindow = makeTargetWindow('https://miko.example/api/plugins/demo-plugin/page?miko-host-origin=https%3A%2F%2Fmiko.example');
    const sdk = createMikoPluginSdk({
      parentWindow,
      targetWindow,
      targetOrigin: 'https://miko.example',
    });

    expect(sdk.assets.url('dist/app.js')).toBe('https://miko.example/api/plugins/demo-plugin/assets/dist/app.js');
    expect(sdk.assets.url('/images/logo.svg')).toBe('https://miko.example/api/plugins/demo-plugin/assets/images/logo.svg');
  });

  it('resolves plugin API URLs from the current iframe route', () => {
    const sdk = createMikoPluginSdk({
      parentWindow: makeParentWindow(),
      targetWindow: makeTargetWindow('https://miko.example/api/plugins/demo-plugin/page?pluginSurfaceSession=session-1'),
      targetOrigin: 'https://miko.example',
    });

    expect(sdk.api.url('api/translate')).toBe('https://miko.example/api/plugins/demo-plugin/api/translate');
    expect(sdk.api.url('/api/translate?mode=fast')).toBe('https://miko.example/api/plugins/demo-plugin/api/translate?mode=fast');
  });
  ...
  it('adds the plugin surface session when fetching the current plugin API', async () => {
    ...
    const targetWindow = {
      ...makeTargetWindow('https://miko.example/api/plugins/demo-plugin/page?pluginSurfaceSession=session-1'),
      fetch: fetchMock,
    } as unknown as Window;
    const sdk = createMikoPluginSdk({
      parentWindow: makeParentWindow(),
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'https://miko.example/api/plugins/demo-plugin/api/translate',
  ...
  it('rejects unsafe plugin API inputs and missing surface sessions', () => {
    const sdk = createMikoPluginSdk({
      parentWindow: makeParentWindow(),
      targetWindow: makeTargetWindow('https://miko.example/api/plugins/demo/page?pluginSurfaceSession=session-1'),
      targetOrigin: 'https://miko.example',
    });
    ...
    const missingSessionSdk = createMikoPluginSdk({
      parentWindow: makeParentWindow(),
      targetWindow: makeTargetWindow('https://miko.example/api/plugins/demo/page'),
      targetOrigin: 'https://miko.example',
    });
