...
function interludeText({ receiverName, source, status }) {
  const receiver = compactLabel(receiverName, "Miko");
  const keyMap = {
...
export function buildDeferredResultInterludeBlock(event, { receiverName = "Miko", meta = null } = {}) {
...
export function resolveDeferredReceiverName(engine, sessionPath) {
  ...
  return agent?.agentName || engine?.agentName || "Miko";
}
