...
describe("LocalFsProvider", () => {
  ...
  function makeProvider(check = vi.fn(() => ({ allowed: true }))) {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-resource-local-fs-"));
...
function makeProviderWithPolicy() {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-resource-local-fs-policy-"));
  const cwd = path.join(tempRoot, "workspace");
  const agentDir = path.join(tempRoot, "miko-home", "agents", "miko");
