...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "miko-server-runtime-context-"));
}
...
describe("server runtime context", () => {
  ...
  it("creates a frozen local runtime context from stable identity registries", async () => {
    ...
    const context = createServerRuntimeContext({ mikoHome: tmpDir, appVersion: "2.3.4" });
  ...
  it("fails explicitly when identity registries are invalid", async () => {
    ...
    expect(() => createServerRuntimeContext({ mikoHome: tmpDir }))
