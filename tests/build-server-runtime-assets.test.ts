...
describe("server runtime assets", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-server-assets-"));
    ...
    fs.writeFileSync(path.join(assetsDir, "character-cards", "yuan-miko-card-back.png"), "card-back\n");
    fs.writeFileSync(path.join(assetsDir, "character-cards", "yuan-miko-emblem.png"), "emblem\n");
    ...
    fs.writeFileSync(path.join(rendererDir, "lib", "theme.js"), "window.MikoTheme = {}", "utf-8");
  ...
  it("copies character-card fallback assets into the bundled server root", () => {
    ...
    expect(fs.readFileSync(
      path.join(outDir, "desktop", "src", "assets", "character-cards", "yuan-miko-card-back.png"),
      "utf-8",
    )).toBe("card-back\n");
    expect(fs.readFileSync(
      path.join(outDir, "desktop", "src", "assets", "character-cards", "yuan-miko-emblem.png"),
      "utf-8",
    )).toBe("emblem\n");
    expect(fs.readFileSync(
    ...
    expect(copied).toEqual(expect.arrayContaining([
      path.join("desktop", "src", "assets", "Miko.png"),
