...
describe("plugin config schema", () => {
  it("normalizes fields and materializes global defaults", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-plugin-config-"));
    try {
  ...
  it("keeps scoped values in their own keyed containers", () => {
    ...
    try {
      ...
      store.set("agentMode", "strict", { scope: "per-agent", agentId: "miko" });

      expect(store.get("agentMode", { scope: "per-agent", agentId: "miko" })).toBe("strict");
      expect(store.getState()).toMatchObject({
        agents: {
          miko: { agentMode: "strict" },
        },
      });
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
