...
}: { mikoHome?: string; pluginId?: string; pluginDir?: string; version?: string; maxBackups?: number } = {}) {
  if (!mikoHome || !pluginId || !pluginDir || !fs.existsSync(pluginDir)) return null;
  const backupRoot = path.join(mikoHome, "plugin-backups", safeSegment(pluginId));
