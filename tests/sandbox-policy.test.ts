...
describe("sandbox workspace roots", () => {
  ...
  beforeEach(() => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-sandbox-roots-"));
  });
  ...
  it("grants full access to explicit extra workspace folders and read-only access to ordinary external paths", () => {
    const agentDir = path.join(tempRoot, "agents", "miko");
    ...
    for (const dir of [agentDir, mikoHome, primary, extra, sibling]) {
  ...
  it("lets agents read session files but blocks writing runtime copies", () => {
    ...
    const sessionFile = path.join(mikoHome, "session-files", "abc123", "SKILL.md");
    ...
    expect(policy.writablePaths).not.toContain(path.join(mikoHome, "session-files"));
  ...
  it("honors read-all semantics for non-secret MIKO_HOME paths while keeping writes scoped", () => {
    const agentDir = path.join(tempRoot, "home", "agents", "miko");
    const mikoHome = path.join(tempRoot, "home");
    const workspace = path.join(tempRoot, "project");
    const pluginSkill = path.join(mikoHome, "plugins", "demo", "skills", "reader", "SKILL.md");
    const pluginSource = path.join(mikoHome, "plugins", "demo", "index.js");
    const blockedAuth = path.join(mikoHome, "auth.json");
  ...
  it("treats cwd and explicit runtime roots as scoped write roots", () => {
    ...
    for (const dir of [agentDir, mikoHome, workspace, cwd, runtimeRoot]) {
      fs.mkdirSync(dir, { recursive: true });
    }
