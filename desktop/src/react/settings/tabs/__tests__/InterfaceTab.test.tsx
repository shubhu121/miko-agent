...
function setAppearanceGlobals() {
  (globalThis as AppearanceGlobals).setTheme = vi.fn((theme: string) => {
    localStorage.setItem('miko-theme', theme);
  ...
  (globalThis as AppearanceGlobals).setPaperTexture = vi.fn((enabled: boolean) => {
    localStorage.setItem('miko-paper-texture', enabled ? '1' : '0');
  });
}
...
describe('InterfaceTab appearance state', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mikoFetchMock.mockImplementation(async (path: string, init?: RequestInit) => {
  ...
  it('updates the reading font card from component state after the preference changes', () => {
    localStorage.setItem('miko-font-serif', '1');
    ...
    expect(localStorage.getItem('miko-font-serif')).toBe('0');
  });

  it('recomputes paper texture availability when the selected theme changes', () => {
    localStorage.setItem('miko-theme', registry.DEFAULT_THEME);
    localStorage.setItem('miko-paper-texture', '1');
