      ...
      <div className={styles['pv-models']}>
        <div className={styles['pv-fav-section']}>
          <div className={styles['pv-fav-title']}>
            {textOrFallback('settings.media.speechModels', "This feature is available in English only.")}
...
export function MediaTab() {
  ...
  const loadImageProviders = useCallback(async () => {
    ...
    try {
      const res = await mikoFetch('/api/media/image/providers');
  ...
  const loadVideoProviders = useCallback(async () => {
    ...
    try {
      const res = await mikoFetch('/api/media/video/providers');
      const data = await res.json();
      const nextProviders = data.providers || {};
  ...
  const loadSpeechProviders = useCallback(async () => {
    try {
      const res = await mikoFetch('/api/speech-recognition/providers');
      const data = await res.json();
      const nextProviders = data.providers || {};
  ...
  const speechRecognitionEnabledLabel = textOrFallback('settings.media.speechRecognitionEnabled', "This feature is available in English only.");
  const defaultSpeechModelLabel = textOrFallback('settings.media.defaultSpeechModel', "This feature is available in English only.");
  ...
  const saveConfig = async (updates: Partial<MediaConfig>) => {
    try {
      const res = await mikoFetch('/api/media/image/config', {
        method: 'PUT',
  ...
  const saveVideoConfig = async (updates: Partial<MediaConfig>) => {
    try {
      const res = await mikoFetch('/api/media/video/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ values: encodeConfigPatch(updates) }),
      });
  ...
  const saveSpeechConfig = async (updates: SpeechConfigPatch) => {
    try {
      const res = await mikoFetch('/api/speech-recognition/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
