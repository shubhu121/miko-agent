...
describe('PlanModeButton', () => {
  ...
  it('opens a menu and marks permission changes from the pending new-session surface explicitly', async () => {
    vi.mocked(mikoFetch).mockResolvedValueOnce(jsonResponse({ mode: 'auto' }));
    ...
    await waitFor(() => {
      expect(mikoFetch).toHaveBeenCalledWith('/api/preferences/session-permission-default', expect.objectContaining({
        method: 'PUT',
  ...
  it('targets the active session when changing an existing conversation permission mode', async () => {
    vi.mocked(mikoFetch).mockResolvedValueOnce(jsonResponse({ mode: 'operate' }));
    useStore.setState({
      currentSessionPath: '/tmp/miko-session.jsonl',
    ...
    await waitFor(() => {
      expect(mikoFetch).toHaveBeenCalledWith('/api/session-permission-mode', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({
          mode: 'operate',
          pendingNewSession: false,
