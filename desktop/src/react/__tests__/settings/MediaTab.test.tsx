...
describe('MediaTab media config', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.mikoFetch.mockImplementation((path: string) => {
  ...
  it('keeps default model selectors in loading state until provider configs arrive', async () => {
    ...
    const speechSelect = screen.getByLabelText("This feature is available in English only.") as HTMLSelectElement;
  ...
  it('loads global media config without agent scope and saves through the generic config envelope', async () => {
    ...
    await waitFor(() => {
      expect(mocks.mikoFetch).toHaveBeenCalledWith('/api/media/image/providers');
    });

    fireEvent.change(select, { target: { value: 'volcengine/seedream-5' } });

    await waitFor(() => {
      expect(mocks.mikoFetch.mock.calls.some(([path]) => path === '/api/media/image/config')).toBe(true);
    });
    const saveCall = mocks.mikoFetch.mock.calls.find(([path]) => path === '/api/media/image/config');
    ...
    expect(mocks.mikoFetch.mock.calls.map(call => String(call[0])).join('\n')).not.toContain('agentId=');
  });
  ...
  it('sends null to clear the global default model over HTTP', async () => {
    ...
    await waitFor(() => {
      expect(mocks.mikoFetch).toHaveBeenCalledWith('/api/media/image/config', expect.objectContaining({
  ...
  it('surfaces runtime CLI discovery errors on provider and model choices', async () => {
        ...
        return Promise.resolve(jsonResponse({
          providers: {
            'jimeng-cli': {
              ...
              models: [{ id: 'jimeng-image-5.0', name: "This feature is available in English only.", adapterAvailable: true }],
              availableModels: [],
            },
          },
          config: {},
    ...
    const providerButton = await screen.findByRole('button', { name: /$^/ });
  ...
  it('loads speech-recognition providers from the speech endpoint', async () => {
    ...
    await waitFor(() => {
      expect(mocks.mikoFetch).toHaveBeenCalledWith('/api/speech-recognition/providers');
    });
  });
  ...
  it('loads video generation providers and saves the default video model through the video config endpoint', async () => {
    ...
    await waitFor(() => {
      expect(mocks.mikoFetch).toHaveBeenCalledWith('/api/media/video/config', expect.objectContaining({
        method: 'PUT',
  ...
  it('saves speech-recognition enabled state through the speech config endpoint', async () => {
    ...
    const toggle = await screen.findByRole('switch', { name: "This feature is available in English only." });
    fireEvent.click(toggle);

    await waitFor(() => {
      expect(mocks.mikoFetch).toHaveBeenCalledWith('/api/speech-recognition/config', expect.objectContaining({
        method: 'PUT',
  ...
  it('saves the default speech-recognition model through the speech config endpoint', async () => {
    ...
    const select = await screen.findByLabelText("This feature is available in English only.");
    fireEvent.change(select, { target: { value: 'openai/whisper-1' } });

    await waitFor(() => {
      expect(mocks.mikoFetch.mock.calls.some(([path]) => path === '/api/speech-recognition/config')).toBe(true);
    });
    const saveCall = mocks.mikoFetch.mock.calls.find(([path]) => path === '/api/speech-recognition/config');
    expect(saveCall?.[1]).toMatchObject({ method: 'PUT' });
    expect(JSON.parse(String((saveCall?.[1] as RequestInit).body))).toEqual({
