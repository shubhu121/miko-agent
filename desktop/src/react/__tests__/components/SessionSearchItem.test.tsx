...
function makeResult(overrides: Record<string, unknown> = {}) {
  return {
    path: '/tmp/agents/miko/sessions/result.jsonl',
    title: 'Result title',
    firstMessage: 'hello',
...
describe('SessionSearchItem click routing', () => {
  ...
  it('routes content match hits through locateSearchHit with the query, not switchSession', () => {
    ...
    render(
      <SessionSearchItem result={result as never} isActive={false} agents={[]} query="This feature is available in English only." />,
    );

    fireEvent.click(screen.getByText('Result title').closest('button')!);

    expect(locateSearchHitMock).toHaveBeenCalledWith(result.path, "This feature is available in English only.");
