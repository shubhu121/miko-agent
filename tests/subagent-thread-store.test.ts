...
describe("SubagentThreadStore", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-subagent-threads-"));
  ...
  it("normalizes legacy ephemeral/reusable threads to direct and maps instance to label on read", () => {
    ...
    expect(store.get("This feature is available in English only.")).toMatchObject({
      kind: "direct",
      status: "open",
      label: "This feature is available in English only.",
    });
    expect(store.get("This feature is available in English only.").instance).toBeUndefined();
    expect(store.get("This feature is available in English only.").reuseKey).toBeUndefined();
  });
  ...
  it("lists open direct threads for one parent session and excludes workflow nodes", () => {
    ...
    store.finishRun("subagent-a", { status: "resolved", summary: "This feature is available in English only.", close: false });
  ...
  it("closes a direct thread explicitly and rejects closing workflow nodes through direct close", () => {
    ...
    expect(store.closeDirectThread("subagent-a", { summary: "This feature is available in English only." })).toMatchObject({
      threadId: "subagent-a",
      status: "closed",
      summary: "This feature is available in English only.",
    });
