...
const pluginCreatorCheck = path.join(repoRoot, "skills2set", "miko-plugin-creator", "scripts", "check_env.mjs");
...
function isolatedPathEnv(directory) {
  ...
  delete env.MIKO_SKILL_CREATOR_PYTHON;
  delete env.MIKO_PLUGIN_CREATOR_PYTHON;
  return env;
}
...
describe("skill-creator environment check", () => {
  ...
  it("reports missing Python before running skill-creator scripts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "miko-skill-creator-env-"));
    roots.push(root);
  ...
  it("reports PyYAML as a missing package for validation and packaging", () => {
    ...
    const result = runCheck(skillCreatorCheck, ["--capability", "package-skill"], {
      ...
      MIKO_SKILL_CREATOR_PYTHON: JSON.stringify([process.execPath, fakePython]),
...
describe("miko-plugin-creator environment check", () => {
  const roots = [];
  ...
  it("reports missing Python before running plugin scaffold scripts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "miko-plugin-creator-env-"));
    roots.push(root);
  ...
  it("passes plugin scaffold preflight with supported Python and no Python packages", () => {
    ...
    const result = runCheck(pluginCreatorCheck, ["--capability", "scaffold"], {
      ...isolatedPathEnv(root),
      MIKO_PLUGIN_CREATOR_PYTHON: JSON.stringify([process.execPath, fakePython]),
    });
