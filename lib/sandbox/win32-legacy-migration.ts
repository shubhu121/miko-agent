...
const LEGACY_PROFILE_PREFIX = "com.miko.sandbox.";
...
export function isLegacyWin32SandboxProfileName(name) {
  return typeof name === "string" && /^com\.miko\.sandbox\.\d+\.\d+$/i.test(name);
}
...
export function collectWin32LegacySandboxMigrationTargets({
  ...
  if (mikoHome) {
    push(mikoHome);
    push(joinWin32Aware(mikoHome, ".ephemeral"));
    push(joinWin32Aware(mikoHome, "agents"));
    push(joinWin32Aware(mikoHome, "session-files"));
    push(joinWin32Aware(mikoHome, "uploads"));
  }
...
function defaultMigrationMarkerPath(mikoHome) {
  return mikoHome
    ? path.join(mikoHome, "user", "win32-sandbox-migration-v3.json")
    : null;
}

function defaultCleanupMarkerPath(mikoHome) {
  return mikoHome
    ? path.join(mikoHome, "user", "win32-sandbox-cleanup-v4.json")
    : null;
}
...
function buildMigrationPhases({ targets, cleanup, cleanupProfiles = true }) {
  ...
  if (targets.aclPaths?.length) {
    const args = [];
    if (cleanup) {
      args.push(...buildWin32MikoWriteAclCleanupArgs({ paths: targets.aclPaths }));
...
export async function runWin32LegacySandboxMigration({
  ...
  const completionMarkerPath = cleanup && !disableMarker
    ? (markerPath || defaultMigrationMarkerPath(mikoHome))
    : null;
...
export async function runWin32LegacySandboxRootCleanup({
  ...
  const marker = markerPath || defaultCleanupMarkerPath(mikoHome);
...
export class Win32LegacySandboxCleanupQueue {
  ...
  constructor({
    ...
    this.markerPath = markerPath || defaultCleanupMarkerPath(mikoHome);
    this.env = env;
    this.spawn = spawn;
