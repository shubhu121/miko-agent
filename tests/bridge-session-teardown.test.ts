...
function bridgeLiveToolSnapshot(names) {
  const denied = new Set(["computer", "browser", "miko_card_guide", "show_card"]);
...
describe("BridgeSessionManager teardown", () => {
  ...
  it("passes bridge steer text to the SDK without adding an internal prefix", () => {
    ...
    expect(manager.steerSession("telegram:dm:owner", "This feature is available in English only.")).toBe(true);
  ...
  it("adds Bridge-facing /apply details from automation suggestion tool results", async () => {
    ...
    expect(reply.text).toContain("This feature is available in English only.");
    expect(reply.text).toContain("This feature is available in English only.");
    expect(reply.text).toContain("This feature is available in English only.");
    expect(reply.text).toContain("This feature is available in English only.");
  ...
  it("preserves captured partial text alongside a provider message_end error (#1607)", async () => {
    ...
    const session = {
      ...
      prompt: vi.fn(async () => {
        for (const fn of subscribers) {
          fn({ type: "message_update", assistantMessageEvent: { type: "text_delta", delta: "This feature is available in English only." } });
          fn({ type: "message_update", assistantMessageEvent: { type: "text_delta", delta: "This feature is available in English only." } });
  ...
  it("recomputes restored owner bridge tools from live tools and applies the Bridge deny-list", async () => {
    ...
    expect(setActiveToolsByName.mock.calls[0][0]).not.toContain("miko_card_guide");
  ...
  it("records non-context custom entries into an existing bridge session file", () => {
    ...
    expect(appendCustomEntry).toHaveBeenCalledWith("miko-deferred-result", { taskId: "task-img" });
  });
  ...
  it("adds a low-salience platform line and records bridge context metadata for owner sessions", async () => {
    ...
    await manager.executeExternalMessage(
      ...
      { userId: "wx-user", chatId: "wx-user", name: "This feature is available in English only." },
      { agentId: "agent-a" },
    );
    ...
    expect(createArgs.resourceLoader.getSystemPrompt()).toContain("This feature is available in English only.");
    expect(createArgs.resourceLoader.getSystemPrompt()).toContain("This feature is available in English only.");
  ...
  it("bridge vision context injection uses captured Miko refs when Pi context is stale", async () => {
    const agent = makeAgent(rootDir);
    ...
    const session = {
      ...
      prompt: vi.fn(async () => {
        ...
        const extension = resourceLoader.getExtensions().extensions
          .find((entry) => entry.path === "miko-vision-context-injection");
