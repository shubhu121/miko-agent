...
describe("updateConfig with agentId", () => {
  ...
  function makeTempDir(name) {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "miko-config-coord-"));
    tempRoots.push(root);
  ...
  it("clears a requested agent explicit home folder only when the path is missing", () => {
    const missingHome = path.join(os.tmpdir(), `miko-missing-home-${Date.now()}`);
  ...
  it("keeps a requested agent explicit home folder when stat reports temporary access failure", () => {
    const blockedHome = path.join(os.tmpdir(), `miko-blocked-home-${Date.now()}`);
