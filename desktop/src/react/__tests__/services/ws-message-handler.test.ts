...
describe('ws-message-handler session-scoped desktop events', () => {
  ...
  it('drops a websocket event whose sessionId is already bound to a different known path (#2078)', () => {
    useStore.setState({
      ...
      currentSessionPath: '/session/a.jsonl',
      sessions: [
        { path: '/session/a.jsonl', sessionId: 'sess_a', agentId: 'a1', agentName: 'Miko' },
        { path: '/session/b.jsonl', sessionId: 'sess_b', agentId: 'a1', agentName: 'Miko' },
      ],
      sessionLocatorsById: {
...
describe('ws-message-handler activity updates', () => {
  beforeEach(() => {
    useStore.setState({
      activities: [
        { id: 'activity-1', type: 'beautify', status: 'running', summary: "This feature is available in English only." },
        { id: 'activity-2', type: 'cron', status: 'done', summary: "This feature is available in English only." },
      ],
    } as never);
  });
  ...
  it('merges activity_update by id so stale running cards do not remain in the panel', () => {
    handleServerMessage({
      ...
      activity: { id: 'activity-1', type: 'beautify', status: 'done', summary: "This feature is available in English only." },
    });

    expect(useStore.getState().activities).toEqual([
      { id: 'activity-1', type: 'beautify', status: 'done', summary: "This feature is available in English only." },
      { id: 'activity-2', type: 'cron', status: 'done', summary: "This feature is available in English only." },
    ]);
  });
});
...
describe('ws-message-handler permission mode events', () => {
  ...
  it('keeps explicit auto mode from legacy plan_mode messages instead of collapsing to operate', () => {
    ...
    windowTarget.addEventListener('miko-plan-mode', (event) => {
...
describe('ws-message-handler turn_end side effects', () => {
  ...
  it('shows focused slash_result through the inline notice bridge', () => {
    ...
    window.addEventListener('miko-inline-notice', listener);
    ...
    try {
      ...
      window.removeEventListener('miko-inline-notice', listener);
      vi.unstubAllGlobals();
    }
  });
