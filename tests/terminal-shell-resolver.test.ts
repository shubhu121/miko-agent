...
describe("resolveTerminalShell", () => {
  ...
  it("uses an explicit Git Bash profile when requested", () => {
    const shellInfo = {
      ...
      args: ["-lc"],
      bundledRoot: "C:\\Miko\\resources\\git",
    };
    const shellEnv = { Path: "C:\\Miko\\resources\\git\\bin;C:\\Windows\\System32" };
    const resolveWin32ShellRuntime = vi.fn(() => shellInfo);
    const getWin32ShellEnvForRuntime = vi.fn(() => shellEnv);

    const resolved = resolveTerminalShell("codex exec \"hello world from miko\"", {
      platform: "win32",
      profile: "git-bash",
    ...
    expect(resolved).toEqual({
      ...
      args: ["-lc", "codex exec \"hello world from miko\""],
      env: shellEnv,
    });
