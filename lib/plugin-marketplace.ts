...
export const DEFAULT_OFFICIAL_PLUGIN_MARKETPLACE_URL = "https://raw.githubusercontent.com/shubhu121/miko-plugins/main/marketplace.json";
...
export function createDefaultPluginMarketplace({ mikoHome, env = process.env, fetchImpl }: any = {}) {
  const envPath = normalizeOptionalText(env.MIKO_PLUGIN_MARKETPLACE_FILE);
  const envUrl = normalizeOptionalText(env.MIKO_PLUGIN_MARKETPLACE_URL);
  const localIndex = mikoHome ? path.join(mikoHome, "plugin-marketplace", "marketplace.json") : null;
