...
export class MountProvider {
  ...
  constructor({ mikoHome, studioId, localFsProviderFactory }: Options) {
    if (!mikoHome) throw new Error("mikoHome is required");
  ...
  mountForRef(ref: ResourceRef) {
    ...
    let registry;
    try {
      registry = loadStudioMountRegistry(this.mikoHome);
    } catch (err) {
