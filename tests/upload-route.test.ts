...
function mktemp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "miko-upload-route-"));
}
...
describe("upload route", () => {
  ...
  it("rejects a symlink root path", async () => {
    ...
    const app = new Hono();
    app.route("/api", createUploadRoute({ mikoHome: path.join(tmpDir, "miko-home") }));
  ...
  it("upload-blob writes base64 image to uploads dir with sanitized name", async () => {
    ...
    const app = new Hono();
    app.route("/api", createUploadRoute({ mikoHome }));
  ...
  it("registers copied uploads as session files when sessionPath is provided", async () => {
    ...
    const app = new Hono();
    app.route("/api", createUploadRoute({ mikoHome, registerSessionFile }));
    ...
    expect(data.uploads[0].dest.startsWith(path.join(mikoHome, "session-files"))).toBe(true);
    expect(data.uploads[0]).toMatchObject({
      src: source,
      name: "note.txt",
      fileId: "sf_upload",
  ...
  it("reuses one session file for repeated uploads of the same source file", async () => {
    ...
    const registry = new SessionFileRegistry({ managedCacheRoot: path.join(mikoHome, "session-files") });
    const engine = {
      mikoHome,
  ...
  it("upload-blob forces extension to match mimeType (defends against name spoofing)", async () => {
    ...
    expect(up.dest.startsWith(path.join(mikoHome, "uploads"))).toBe(true);
  });
