...
const PROBE_TOKEN = "__miko_probe_ok__";
...
const WIN32_SANDBOX_HELPER_LAUNCH_FAILURE_RE = /miko-win-sandbox:\s+CreateProcessAsUserW failed/i;
...
function getBundledGitRoots(env = process.env, deps: Record<string, any> = {}) {
  ...
  const roots = [
    ...
    env.MIKO_ROOT ? resolve(env.MIKO_ROOT, "..", "git") : null,
...
function findGitRuntime({ env = process.env, bundledOnly = false } = {}) {
  ...
  throw new Error(
    "[win32-exec] No usable git.exe found. Install Git for Windows or rebuild Miko with bundled MinGit."
  );
}
...
function findAndCacheShell(startAfter: any, options: Record<string, any> = {}) {
  ...
  const allLabels = startAfter
    ? ["This feature is available in English only.", ...failures]
...
function withWin32SandboxRuntimeEnv(baseEnv, sandbox) {
  ...
  if (!sandboxIsEnabled(sandbox) || !sandbox?.mikoHome) return env;

  const root = joinRuntimePath(sandbox.mikoHome, ".ephemeral", WIN32_SANDBOX_ENV_DIR);
...
function findNodeRuntimeOnPath(commandName, args, env) {
  const candidates = [];
  if (env?.MIKO_DEV_NODE_BIN) candidates.push(executableInfoFromPath(env.MIKO_DEV_NODE_BIN, `MIKO_DEV_NODE_BIN (${env.MIKO_DEV_NODE_BIN})`));
...
function redactWin32DiagnosticText(value: any, { sandbox, env }: { sandbox?: any; env?: any } = {}) {
  ...
  const replacements = [
    [sandbox?.mikoHome, "<MIKO_HOME>"],
    [envValue(env, "MIKO_HOME"), "<MIKO_HOME>"],
...
function collectWin32EnvironmentDiagnostics(env, context) {
  ...
  return [
    ...
    `MIKO_ROOT: ${redactWin32DiagnosticPath(envValue(env, "MIKO_ROOT") || "(unset)", context.sandbox, env)}`,
    `MIKO_SERVER_ENTRY: ${redactWin32DiagnosticPath(envValue(env, "MIKO_SERVER_ENTRY") || "(unset)", context.sandbox, env)}`,
...
function emitWin32RuntimeFailureDiagnostic(onData, {
  ...
  const lines = [
    ...
    sandbox?.mikoHome ? "MIKO_HOME: <MIKO_HOME>" : null,
    ...
    "Next step: attach this diagnostic with the command log; for cached POSIX runtimes also clear MIKO_HOME/.ephemeral/win32-sandbox-runtime and retry.",
    "",
  ].filter(Boolean);
...
function prepareRuntimeForSandbox(runtimeInfo, sandbox, kind) {
  if (!sandboxIsEnabled(sandbox) || !sandbox?.mikoHome) return runtimeInfo;
  return prepareSandboxRuntime(runtimeInfo, {
    mikoHome: sandbox.mikoHome,
    kind,
  });
}
...
async function spawnViaSandboxHelper({ sandbox, executable, args, cwd, env, onData, signal, timeout }) {
  ...
  try {
    ...
    if (!terminal) {
      ...
      error.code = "MIKO_WIN32_SANDBOX_TERMINAL_PROTOCOL";
      throw error;
    }
    ...
    if (terminal.status === "termination_failed") {
      ...
      error.code = "MIKO_WIN32_SANDBOX_TERMINATION_FAILED";
