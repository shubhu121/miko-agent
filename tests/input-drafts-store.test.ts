...
describe("InputDraftsStore", () => {
  let home: string;
  beforeEach(() => {
    home = fs.mkdtempSync(path.join(os.tmpdir(), "miko-input-drafts-"));
  });
  afterEach(() => {
  ...
  it("persists home and session drafts per surface and survives reload", () => {
    const store = new InputDraftsStore({ mikoHome: home });
    ...
    const reloaded = new InputDraftsStore({ mikoHome: home });
