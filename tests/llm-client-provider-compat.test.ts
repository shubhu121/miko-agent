...
describe("callText provider-compat routing", () => {
  ...
  it("keeps provider-default output unbounded at Miko's request layer", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue({
      ok: true,
      status: 200,
  ...
  it("fails closed for native APIs without a Miko buffered adapter", async () => {
  ...
  it("uses a stable default User-Agent without overriding model headers", async () => {
    ...
    expect(fetchMock.mock.calls[0][1].headers["User-Agent"]).toBe("Miko/1.0");
