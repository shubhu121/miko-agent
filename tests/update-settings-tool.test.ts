...
describe("update-settings-tool", () => {
  ...
  function makeMemoryAgent(agentId = "agent-test") {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "miko-update-settings-memory-"));
  ...
  describe("bridge media public URL", () => {
    ...
    it("applies the Bridge media public URL as a global preference", async () => {
      ...
      await tool.execute("c-bridge-url-apply", {
        ...
        value: "https://miko.example.com",
      });

      expect(engine.setBridgeMediaPublicBaseUrl).toHaveBeenCalledWith("https://miko.example.com");
      expect(engine._prefs._store.bridge.mediaPublicBaseUrl).toBe("https://miko.example.com");
    });

    it("allows clearing the Bridge media public URL with an empty value", async () => {
      const { tool, engine } = buildTool({
        prefsData: { bridge: { mediaPublicBaseUrl: "https://miko.example.com" } },
      });
