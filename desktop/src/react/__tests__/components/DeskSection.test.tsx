...
describe('DeskSection workspace watching', () => {
  ...
  beforeEach(() => {
    ...
    window.t = ((key: string) => key === 'desk.workspaceTitle' ? "This feature is available in English only." : key) as typeof window.t;
    window.platform = {
      watchFile,
      unwatchFile,
      onFileChanged: vi.fn(),
  ...
  it('watches the workspace root plus expanded folders and reloads visible dirty tree keys from workspace events', async () => {
    ...
    expect(mocks.retainResourceWatch).toHaveBeenCalledWith({ kind: 'local-file', path: '/tmp/miko-desk' });
    expect(mocks.retainResourceWatch).toHaveBeenCalledWith({ kind: 'local-file', path: '/tmp/miko-desk/notes' });
  ...
  it('starts an app file drag from tree rows so workspace files can be moved or attached', async () => {
    ...
    expect(window.platform?.startDrag).toHaveBeenCalledWith('/tmp/miko-desk/root.md');
    expect(getActiveAppFileDragPayload()).toEqual(expect.objectContaining({
      source: 'workspace',
      files: [{
        id: 'workspace:root.md',
        name: 'root.md',
        path: '/tmp/miko-desk/root.md',
        sourceSubdir: '',
        isDirectory: false,
      }],
    }));
