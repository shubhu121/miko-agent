...
const HANDBOOK = `# Miko Interactive Card Design Handbook

You are generating HTML/SVG fragments that render inside an iframe in Miko's chat.
...
### KAMI extended palette (from Miko's notebook design language)
These are not injected as CSS variables but belong to Miko's full color spectrum. Use them directly as hex values.
...
### Chart palette (6 colors, all from Miko/KAMI ink-dyed family)
...
cursor: pointer;
\`\`\`
Hover: \`background: var(--accent-light)\`. No :active scale animation (Miko quiet motion principle).
...
| emoji | Miko uses SVG stroke icons only |
...
| Font-weight 700 | Miko allows 400/500 only. 600 for metrics only |
| Title Case / ALL CAPS | Miko uses sentence case exclusively |
...
- Default corner radius: \`rx="2"\` (Miko's square-corner style). Larger values only for deliberate pill shapes.
...
export function createCardGuideTool() {
  return {
    name: "miko_card_guide",
    label: "Interactive Card Guide",
    description:
      "Returns the Miko Interactive Card Design Handbook — CSS variables, colors, typography, layout rules, " +
