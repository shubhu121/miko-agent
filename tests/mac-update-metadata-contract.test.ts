...
function makeTmpDir() {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-mac-update-metadata-"));
  return tmpDir;
}
...
describe("macOS update metadata release contract", () => {
  ...
  it("merges arm64 and x64 metadata even when electron-builder emits an arch-suffixed arm64 file", () => {
    ...
    expect(urls).toEqual([
      "Miko-0.171.5-macOS-arm64.zip",
      "Miko-0.171.5-macOS-arm64.dmg",
      "Miko-0.171.5-macOS-x64.zip",
      "Miko-0.171.5-macOS-x64.dmg",
    ]);
    expect(merged.path).toBe("Miko-0.171.5-macOS-arm64.zip");
