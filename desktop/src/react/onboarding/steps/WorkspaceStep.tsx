...
export function WorkspaceStep({ preview, mikoFetch, goToStep, showError }: WorkspaceStepProps) {
  ...
  useEffect(() => {
    if (preview) return;
    let cancelled = false;
    loadDefaultWorkspace(mikoFetch)
      .then(path => {
        if (cancelled) return;
        setDefaultPath(path);
      })
  ...
  const onNext = useCallback(async () => {
    ...
    setSaving(true);
    try {
      await saveWorkspace({ mikoFetch, workspacePath: visiblePath, defaultPath });
      goToStep(6);
    } catch (err) {
  ...
  }, [preview, goToStep, defaultPath, visiblePath, mikoFetch, showError]);
