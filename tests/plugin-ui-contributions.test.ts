...
import { PLUGIN_UI_CAPABILITY } from '@miko/plugin-protocol';

function tmpPluginDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'miko-plugin-test-'));
}
...
function createPluginWithPage(dir, id = 'test-page-plugin') {
  ...
  fs.writeFileSync(path.join(pluginDir, 'manifest.json'), JSON.stringify({
    ...
    contributes: {
      page: {
        title: { zh: "This feature is available in English only.", en: 'Test' },
...
function makePM(communityDir: any) {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'miko-data-'));
  const builtinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'miko-builtin-'));
...
describe('Plugin UI Contributions', () => {
  ...
  it('loads page contribution from manifest', async () => {
    ...
    expect(pages[0].title).toEqual({ zh: "This feature is available in English only.", en: 'Test' });
  ...
  it('loads hidden built-in native settings tab contributions', async () => {
    const builtinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'miko-builtin-settings-'));
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'miko-data-settings-'));
    try {
