...
import { applyDevEnvironment, defaultDevMikoHome } from "./dev-env.js";
import {
...
const mikoHome = defaultDevMikoHome();
...
async function waitForServerInfo({ timeoutMs = 90_000, intervalMs = 200 } = {}) {
  ...
  while (Date.now() < deadline) {
    if (!isChildAlive(serverProcess)) {
      throw new Error("Miko server exited before writing server-info.json");
    }
    try {
...
function spawnServer() {
  ...
  serverEnv.MIKO_ROOT = rootDir;
  serverEnv.MIKO_SERVER_ENTRY = path.join(rootDir, "server", "main-full.ts");
  serverEnv.MIKO_CREATE_STARTUP_SESSION = "0";
  serverEnv.MIKO_PORT = process.env.MIKO_PORT || "0";
...
function spawnVite(clientConfig, serverInfo) {
  ...
  viteEnv.MIKO_DEV_WEB = "1";
  viteEnv.MIKO_DEV_WEB_CLIENT_PORT = clientConfig.serverPort;
  viteEnv.MIKO_DEV_WEB_API_BASE_URL = clientConfig.apiBaseUrl;
  viteEnv.MIKO_DEV_WEB_SERVER_URL = `http://127.0.0.1:${serverInfo.port}`;
  viteEnv.MIKO_DEV_WEB_SERVER_TOKEN = serverInfo.token;
