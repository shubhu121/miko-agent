...
const CHANNEL_MARKER_RE = /[<English only]\|[\s\S]*?\|[>English only]/g;
const TOOL_TAG_RE = /[<English only]\s*([/English only]?)\s*([\p{L}\p{N}_.:English only-]+)([^<English only>English only]*)[>English only]/gu;
