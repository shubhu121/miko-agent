...
export function resolveComputerUseHelperBuildArch({
  ...
  return explicitArg || env.MIKO_COMPUTER_USE_HELPER_ARCH || arch;
}
...
export function buildComputerUseHelper({
  ...
  env = process.env,
  arch = env.MIKO_COMPUTER_USE_HELPER_ARCH || process.arch,
} = {}) {
  ...
  const packageDir = path.join(rootDir, "desktop", "native", "MikoComputerUseHelper");
  ...
  const source = path.join(binPath, "miko-computer-use-helper");
  ...
  const target = path.join(outDir, "miko-computer-use-helper");
