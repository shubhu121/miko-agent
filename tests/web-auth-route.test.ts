...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "miko-web-auth-route-"));
}
...
describe("web auth route", () => {
  ...
  it("exchanges a paired device credential for an HttpOnly browser session cookie", async () => {
    ...
    expect(setCookie).toContain("miko_session=");
  ...
  it("allows local account password login only over local or secure transport", async () => {
    ...
    const insecure = await app.request("/api/web-auth/login", {
      ...
      body: JSON.stringify({ username: "miko-owner", password: "correct horse battery staple" }),
    });
    ...
    const secure = await app.request("https://miko.example.test/api/web-auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ username: "miko-owner", password: "correct horse battery staple" }),
    });
    expect(secure.status).toBe(200);
    expect(secure.headers.get("set-cookie")).toContain("miko_session=");
    ...
    const desktop = await app.request("https://miko.example.test/api/web-auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        username: "miko-owner",
