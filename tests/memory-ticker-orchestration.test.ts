...
describe("_doDaily step orchestration", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-test-"));
...
describe("memory facts environment ledger", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-memory-ledger-"));
  });
...
describe("memory-ticker getHealthStatus", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-health-"));
  });
