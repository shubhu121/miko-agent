...
describe("HTTP route security policy", () => {
  ...
  it("separates remote settings writes, provider management, bridge management, and secret mutation scopes", async () => {
    ...
    expect(authorizeHttpRoute({ method: "PUT", path: "/api/config", principal: settingsWriter }))
      .toMatchObject({ allowed: true });
    expect(authorizeHttpRoute({ method: "PUT", path: "/api/agents/miko/config", principal: settingsWriter }))
      .toMatchObject({ allowed: true });
    ...
    for (const [method, path] of [
      ...
      ["POST", "/api/agents/miko/avatar"],
      ["DELETE", "/api/agents/miko/avatar"],
    ]) {
  ...
  it("allows scoped device access to chat identity and resources without opening admin APIs", async () => {
    ...
    expect(authorizeHttpRoute({ method: "POST", path: "/api/session-projects/session-assignment", principal }))
      .toMatchObject({ allowed: true });
    expect(authorizeHttpRoute({ method: "DELETE", path: "/api/session-projects/projects/project-miko", principal }))
      .toMatchObject({ allowed: true });
  ...
  it("gates built-in MCP connector settings by settings scopes", async () => {
    ...
    for (const [method, path] of [
      ...
      ["PUT", "/api/plugins/mcp/agents/miko/connectors/github"],
  ...
  it("allows desktop owner clients to consume Studio server settings, plugins, skills, image generation, and connector routes", async () => {
    ...
    for (const [method, path] of [
      ["GET", "/api/agents"],
      ["GET", "/api/agents/miko/identity"],
      ["GET", "/api/agents/miko/ishiki"],
      ["GET", "/api/agents/miko/public-ishiki"],
      ["GET", "/api/agents/miko/pinned"],
      ["GET", "/api/agents/miko/experience"],
      ["GET", "/api/user-profile"],
      ["GET", "/api/preferences/notifications"],
      ["GET", "/api/preferences/computer-use"],
      ["GET", "/api/skills?agentId=miko"],
      ["GET", "/api/skills/bundles?agentId=miko"],
      ["PATCH", "/api/agents/miko/skills/imagegen"],
      ["PATCH", "/api/agents/miko/skill-bundles/story-pack"],
      ["PUT", "/api/agents/miko/skills"],
      ...
      ["GET", "/api/plugins/mcp/state?agentId=miko"],
      ...
      ["PUT", "/api/plugins/mcp/agents/miko/servers/github"],
    ]) {
