...
describe('DeskTree drag payloads for workspace roots', () => {
  ...
  it('adds screenshot share to local Markdown workspace context menus', () => {
    ...
    const screenshotItem = menu.items.find((item: { label?: string }) => item.label === "This feature is available in English only.");
  ...
  it('does not add screenshot share to non-Markdown workspace context menus', () => {
    ...
    expect(onShowMenu.mock.calls[0][0].items.some((item: { label?: string }) => item.label === "This feature is available in English only.")).toBe(false);
  });
});
