...
export class ProviderCatalogStore {
  ...
  constructor(mikoHome: string) {
    if (!mikoHome) throw new Error("ProviderCatalogStore requires mikoHome");
    this._mikoHome = mikoHome;
  }

  get catalogPath() {
    return path.join(this._mikoHome, PROVIDER_CATALOG_FILE);
  }

  get legacyAddedModelsPath() {
    return path.join(this._mikoHome, LEGACY_ADDED_MODELS_FILE);
  }
