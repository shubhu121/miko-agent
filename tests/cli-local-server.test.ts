...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "miko-cli-server-"));
}
...
describe("CLI local server discovery", () => {
  ...
  it("reports a missing server-info file explicitly", () => {
    tmpDir = makeTmpDir();
    const result = readLocalServerInfo({ mikoHome: tmpDir });
  ...
  it("builds a loopback connection from server-info", () => {
    ...
    expect(resolveConnection({ mikoHome: tmpDir })).toMatchObject({
      ok: true,
