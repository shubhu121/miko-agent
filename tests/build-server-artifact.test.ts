...
describe("build-server-artifact: Mach-O detection", () => {
  ...
  it("finds Mach-O files recursively and skips scripts", () => {
    const root = makeTempDir("miko-macho-");
    ...
    fs.writeFileSync(path.join(root, "miko-server"), "#!/bin/sh\n");
...
describe("build-server-artifact: keyset resolution", () => {
  ...
  it("uses the MIKO_SIGN_KEYSET override file when set", () => {
    const root = makeTempDir("miko-keyset-");
    const { keysetPath, keyId } = makeKeypairFiles(root, "override1");
    const { keyset } = resolveBuildKeyset({ MIKO_SIGN_KEYSET: keysetPath });
    expect(keyset[0].keyId).toBe(keyId);
  });

  it("hard-errors when MIKO_SIGN_KEYSET points at a missing file", () => {
    expect(() => resolveBuildKeyset({ MIKO_SIGN_KEYSET: "/nonexistent/keyset.json" })).toThrow(
      /MIKO_SIGN_KEYSET/,
    );
  });
});
...
describe("build-server-artifact: packServerArchive (pack-only, no manifest)", () => {
  it("packs the server tree into an archive without touching manifests", async () => {
    const root = makeTempDir("miko-pack-server-");
  ...
  it("passes env down to the darwin signer so MIKO_MACHO_SIGN_IDENTITY reaches it (no process.env grabbing)", async () => {
    const root = makeTempDir("miko-pack-server-");
    const outDir = makeServerTree(root);
    ...
    await packServerArchive({
      ...
      platform: "darwin",
      arch: "arm64",
      env: { MIKO_MACHO_SIGN_IDENTITY: "CAFEBABE" },
      log: () => {},
      deps: {
    ...
    expect(seenEnv).toEqual({ MIKO_MACHO_SIGN_IDENTITY: "CAFEBABE" });
  });
...
describe("build-server-artifact: packRendererArtifact", () => {
  it("packs the renderer tree, platform-independent", async () => {
    const root = makeTempDir("miko-pack-renderer-");
...
describe("build-server-artifact: packDualKindSeed guards and ordering", () => {
  ...
  it("hard-errors when MIKO_SIGN_KEY is unset (never a silent skip)", async () => {
    const root = makeTempDir("miko-dual-");
    await expect(
      packDualKindSeed({ ...baseOpts(root), env: {} }),
    ).rejects.toThrow(/MIKO_SIGN_KEY/);
  });

  it("hard-errors when MIKO_SIGN_KEY points at a missing file", async () => {
    const root = makeTempDir("miko-dual-");
    await expect(
      packDualKindSeed({ ...baseOpts(root), env: { MIKO_SIGN_KEY: path.join(root, "no-such-key.pem") } }),
    ).rejects.toThrow(/MIKO_SIGN_KEY/);
  });

  it("end-to-end: packs both archives, signs and verifies a seed whose manifest matches both", async () => {
    const root = makeTempDir("miko-dual-e2e-");
    const { keyPath, keysetPath, keyId } = makeKeypairFiles(root, "e2e2026");
    const opts = baseOpts(root);

    const result = await packDualKindSeed({ ...opts, env: { MIKO_SIGN_KEY: keyPath, MIKO_SIGN_KEYSET: keysetPath } });
  ...
  it("hard-errors at build time when the signing key does not match the packed keyset", async () => {
    const root = makeTempDir("miko-dual-mismatch-");
    ...
    await expect(
      packDualKindSeed({ ...baseOpts(root), env: { MIKO_SIGN_KEY: keyPath, MIKO_SIGN_KEYSET: mismatchKeysetPath } }),
    ).rejects.toThrow(/signature verification failed/i);
  });

  it("refuses to build a seed when the renderer dist dir is missing (build ordering guard)", async () => {
    const root = makeTempDir("miko-dual-missing-renderer-");
    const { keyPath, keysetPath } = makeKeypairFiles(root);
    const opts = baseOpts(root);
    fs.rmSync(opts.rendererDistDir, { recursive: true, force: true });
    await expect(
      packDualKindSeed({ ...opts, env: { MIKO_SIGN_KEY: keyPath, MIKO_SIGN_KEYSET: keysetPath } }),
    ).rejects.toThrow(/renderer dist dir not found/);
  });
});
...
describe("build-server-artifact: packDualKindSeed prebuilt renderer archive reuse (CI single-source box)", () => {
  ...
  it("reuses a prebuilt renderer archive instead of packing rendererDistDir on the spot", async () => {
    const root = makeTempDir("miko-dual-prebuilt-");
    const { keyPath, keysetPath } = makeKeypairFiles(root);
    ...
    const result = await packDualKindSeed({
      ...opts,
      env: { MIKO_SIGN_KEY: keyPath, MIKO_SIGN_KEYSET: keysetPath },
  ...
  it("picks up MIKO_PREBUILT_RENDERER_BOX from env when the option is not passed explicitly", async () => {
    const root = makeTempDir("miko-dual-prebuilt-env-");
    const { keyPath, keysetPath } = makeKeypairFiles(root);
    const prebuilt = await packSharedRendererBox(root);
    ...
    const result = await packDualKindSeed({
      ...opts,
      env: { MIKO_SIGN_KEY: keyPath, MIKO_SIGN_KEYSET: keysetPath, MIKO_PREBUILT_RENDERER_BOX: prebuilt.archivePath },
    });
  ...
  it("hard-errors when the prebuilt renderer archive path does not exist", async () => {
    const root = makeTempDir("miko-dual-prebuilt-missing-");
    const { keyPath, keysetPath } = makeKeypairFiles(root);
    const opts = baseOpts(root);
  ...
  it("hard-errors when the prebuilt renderer archive filename does not match the build version", async () => {
    const root = makeTempDir("miko-dual-prebuilt-mismatch-");
    const { keyPath, keysetPath } = makeKeypairFiles(root);
