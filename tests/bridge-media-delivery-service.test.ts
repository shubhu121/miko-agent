...
describe("MediaDeliveryService", () => {
  ...
  function makeTempFile(name, content = "hello") {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-media-delivery-"));
  ...
  function makeTempFileOutsideAllowedRoots(name, content = "hello") {
    ...
    const outsideRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-media-outside-"));
  ...
  it("publishes local files before sending them to URL-only adapters", async () => {
    ...
    const mediaPublisher = {
      setBaseUrl: vi.fn(),
      publish: vi.fn(() => ({
        publicUrl: "https://miko.example.com/api/bridge/media/token_123",
        expiresAt: 61_000,
      })),
    };
    const service = makeService({
    ...
    expect(adapter.sendMedia).toHaveBeenCalledWith(
      "chat-1",
      "https://miko.example.com/api/bridge/media/token_123",
      {
        kind: "image",
        mime: "image/png",
        filename: "image.png",
