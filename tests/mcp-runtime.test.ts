...
describe("MCP runtime policy", () => {
  ...
  it("normalizes Cherry-style MCP server fields into Miko connectors", () => {
    const config = normalizeMcpConfig({
      enabled: true,
      connectors: [
        {
  ...
  it("marks only the target agent stale for MCP REST agent tool mutations", async () => {
    ...
    const res = await app.request("/agents/miko/connectors/github", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns an explicit tool error when MCP is globally disabled at call time", async () => {
    ...
    const result = await tool.execute({}, { agentId: "miko" });
...
describe("MCP connectors status tool", () => {
  ...
  it("reports each connector status and tool count from getState", async () => {
    ...
    const result = await statusTool.execute("call-1", {}, { agentId: "miko" });
