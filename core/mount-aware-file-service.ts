...
export class MountAwareFileService {
  ...
  constructor({
    ...
    if (!mikoHome) throw new Error("mikoHome required");
  ...
  _resolveRootInternal(rootId = "default") {
    ...
    const mount = findLocalFsMount(this._mikoHome, this._studioId, id);
...
function createServiceResourceIO({ mikoHome, defaultRoot, studioId }) {
...
function findLocalFsMount(mikoHome, studioId, rootId) {
  if (!studioId) return null;
  let registry;
  try {
    registry = loadStudioMountRegistry(mikoHome);
  } catch {
    return null;
  }
