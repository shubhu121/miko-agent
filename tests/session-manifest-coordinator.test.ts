...
describe("SessionCoordinator session manifest integration", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-session-manifest-coord-"));
    sessionPath = path.join(tempDir, "agents", "miko", "sessions", "alpha.jsonl");
  ...
  it("pins by sessionId after a path move and treats the path as a legacy locator", async () => {
    ...
    const movedPath = path.join(tempDir, "agents", "miko", "sessions", "alpha-renamed.jsonl");
  ...
  it("rejects stale active locators after archive without cold-loading a header file", async () => {
    const coordinator = createCoordinator();
    await coordinator.createSession(null, tempDir, true);
    const archivedPath = path.join(tempDir, "agents", "miko", "sessions", "archived", "alpha.jsonl");
