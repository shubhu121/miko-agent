...
describe('PreviewPanel markdown editor status', () => {
  ...
  it('shows selected and total character counts for editable markdown', () => {
    ...
    expect(screen.getByTestId('markdown-editor-status')).toHaveTextContent("This feature is available in English only.");
  });
  ...
  it('refreshes open local preview files when the panel mounts', async () => {
    ...
    await waitFor(() => {
      ...
      expect(note?.content).toBe("This feature is available in English only.");
  ...
  it('keeps retained file watches alive when the open tab set changes', async () => {
    ...
    await waitFor(() => {
      expect(resourceEventMocks.retainResourceWatch).toHaveBeenCalledWith({ kind: 'local-file', path: '/tmp/miko-note.md' });
