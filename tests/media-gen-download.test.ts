...
const tmpDir = path.join(os.tmpdir(), "miko-test-imgdl-" + Date.now());

afterEach(() => { fs.rmSync(tmpDir, { recursive: true, force: true }); });

describe("saveImage", () => {
  it("writes buffer to generated/ with correct filename pattern", async () => {
    const { saveImage } = await import("../core/media/download.ts");
    const buf = Buffer.from("fake-png-data");
    const result = await (saveImage as any)(buf, "image/png", tmpDir);
  ...
  it("derives extension from mimeType", async () => {
    ...
    const result = await (saveImage as any)(buf, "image/jpeg", tmpDir);
