...
describe("archive route: mtime semantics", () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    vi.clearAllMocks();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-archived-"));
...
describe("POST /api/sessions/switch archived path", () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    vi.clearAllMocks();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-switch-archived-"));
    const archivedDir = path.join(tmpDir, "agents", "a", "sessions", "archived");
    fs.mkdirSync(archivedDir, { recursive: true });
...
describe("GET /api/sessions/archived", () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-archived-list-"));
    engine = makeEngine(tmpDir);
...
describe("POST /api/sessions/restore", () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-restore-"));
...
describe("POST /api/sessions/archived/delete", () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-del-arch-"));
    const sessDir = path.join(tmpDir, "agents", "a", "sessions");
...
describe("POST /api/sessions/cleanup (titles orphan cleanup)", () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-cleanup-titles-"));
    const sessDir = path.join(tmpDir, "agents", "a", "sessions");
    const archDir = path.join(sessDir, "archived");
    fs.mkdirSync(archDir, { recursive: true });
