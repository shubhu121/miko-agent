...
export const OFFICE_PDF_HELPER_FLAG = "--miko-office-html-to-pdf";
...
function helperCommandForJob(jobPath, input: any = {}, env = process.env) {
  const execPath = input.helperExecPath
    || env.MIKO_OFFICE_PDF_HELPER_EXEC
    || env.MIKO_DESKTOP_EXEC_PATH;
  if (!execPath) {
    throw new Error(
      "Chromium PDF helper is unavailable. Start Miko Desktop, or set MIKO_OFFICE_PDF_HELPER_EXEC to an Electron executable.",
    );
  }

  const helperAppPath = input.helperAppPath
    || env.MIKO_OFFICE_PDF_HELPER_APP_PATH
    || (env.MIKO_DESKTOP_IS_PACKAGED === "1" ? null : env.MIKO_DESKTOP_APP_PATH);
...
function buildJobPayload(input, htmlPath, outputPath) {
  return {
    ...
    embedMikoFonts: input.embedMikoFonts !== false,
