...
export function createAccessRoute({
  ...
  if (!engine?.mikoHome) throw new Error("engine.mikoHome required");
  ...
  route.put("/access/network", async (c) => {
    ...
    try {
      ...
      const existing = loadServerNetworkConfig(engine.mikoHome);
      ...
      const network = saveServerNetworkConfig(engine.mikoHome, {
        ...existing,
        mode,
        listenHost,
        listenPort,
  ...
  async function issueAccessCredential(c, profile) {
    ...
    try {
      ...
      const issued = createDeviceCredential(engine.mikoHome, {
  ...
  route.put("/access/account/profile", async (c) => {
    ...
    try {
      const body = await safeJson(c);
      const account = updateLocalAccountProfile(engine.mikoHome, {
  ...
  route.put("/access/account/password", async (c) => {
    ...
    try {
      const body = await safeJson(c);
      const account = setLocalAccountPassword(engine.mikoHome, {
  ...
  route.delete("/access/account/password", async (c) => {
    ...
    if (denied) return denied;
    try {
      const account = clearLocalAccountPassword(engine.mikoHome, { now: now() });
      recordSecurityAuditEvent(c, engine, {
...
export function createAccessSummary(engine, runtimeState, listLanAddresses = getLanAddresses) {
  const network = loadServerNetworkConfig(engine.mikoHome);
  const registries = loadDeviceAccessRegistries(engine.mikoHome);
  return {
    network: createNetworkSummary(network, runtimeState, listLanAddresses),
    account: getLocalAccountSummary(engine.mikoHome),
