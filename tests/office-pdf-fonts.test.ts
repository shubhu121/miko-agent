...
describe("office pdf font injection css", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-pdf-fonts-"));
  ...
  it("keeps only the whitelisted Miko families and drops the rest", () => {
  ...
  it("exposes the family whitelist as the single source for callers and tests", () => {
    expect(MIKO_PDF_FONT_FAMILIES).toEqual(["EB Garamond", "Noto Serif SC", "JetBrains Mono"]);
  });
