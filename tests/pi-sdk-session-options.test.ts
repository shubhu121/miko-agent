...
describe("Pi SDK session option normalization", () => {
  it("exposes stable Miko built-in tool names without SDK prebuilt objects", () => {
  ...
  it("dedupes missing-id calls by normalized name, args, and assistantMessageId", async () => {
    ...
    const first = await definition.execute("", { query: "Miko", filters: { b: 2, a: 1 } }, null, null, { assistantMessageId: "assistant-1" });
    const second = await definition.execute(null, { filters: { a: 1, b: 2 }, query: "Miko" }, null, null, { assistantMessageId: "assistant-1" });
    const third = await definition.execute(null, { filters: { a: 1, b: 2 }, query: "Miko" }, null, null, { assistantMessageId: "assistant-2" });
  ...
  it("normalizes Miko Tool[] plus customTools into Pi 0.68+ name allowlist and SDK custom tools", () => {
    const read = makeAgentTool("read");
