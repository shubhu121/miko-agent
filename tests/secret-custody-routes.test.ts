...
describe("secret custody across HTTP routes", () => {
  ...
  it("masks bridge secrets in status and preserves masked config updates", async () => {
    ...
    const engine = {
      currentAgentId: "miko",
      getAgent: (id) => id === "miko" ? agent : null,
      getBridgeIndex: () => ({}),
    ...
    const writeRes = await app.request("/api/bridge/config?agentId=miko", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        platform: "telegram",
    ...
    const feishuWriteRes = await app.request("/api/bridge/config?agentId=miko", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        platform: "feishu",
    ...
    agent.updateConfig.mockClear();
    const dingtalkWriteRes = await app.request("/api/bridge/config?agentId=miko", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        platform: "dingtalk",
  ...
  it("fingerprints the reloaded persisted DingTalk secret after config save", async () => {
    ...
    const logDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-dingtalk-log-"));
    ...
    const res = await app.request("/api/bridge/config?agentId=miko", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        platform: "dingtalk",
    ...
    expect(stopPlatform).toHaveBeenCalledWith("dingtalk", "miko");
  ...
  it("projects enabled DingTalk without corpId as an explicit config error", async () => {
    ...
    const res = await app.request("/api/bridge/status?agentId=miko");
    const body = await res.json();
  ...
  it("tests saved DingTalk credentials with omitted secret and legacy aliases", async () => {
    ...
    const res = await app.request("/api/bridge/test?agentId=miko", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        platform: "dingtalk",
  ...
  it("preserves saved legacy mode when the UI omits it and exits it after corpId is supplied", async () => {
    ...
    const save = (corpId: string) => app.request("/api/bridge/config?agentId=miko", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        platform: "dingtalk",
  ...
  it("requires an explicit agent for useSavedCredentials", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const app = new Hono();
    app.route("/api", createBridgeRoute({ currentAgentId: "miko" }, { getStatus: () => ({}) }));
  ...
  it("does not let the explicit plaintext test mode recover a displayed mask", async () => {
    ...
    const omittedModeRes = await app.request("/api/bridge/test?agentId=miko", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        platform: "dingtalk",
  ...
  it("tests a saved legacy QQ token through the canonical appSecret request field", async () => {
    ...
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce({ json: async () => ({ id: "bot-1", username: "miko-bot" }) });
    vi.stubGlobal("fetch", fetchMock);
    const agent = {
      id: "miko",
      config: {
