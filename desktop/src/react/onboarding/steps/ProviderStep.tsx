...
export function ProviderStep({
  preview, mikoFetch, goToStep, showError, onProviderReady,
}: ProviderStepProps) {
  // ── Provider state ──
  ...
  const onTest = useCallback(async () => {
    ...
    try {
      const result = await testConnection({ mikoFetch, providerUrl, providerApi, apiKey });
      if (result.ok) {
  ...
  }, [preview, mikoFetch, providerUrl, providerApi, apiKey]);
  ...
  const onNext = useCallback(async () => {
    ...
    try {
      await saveProviderAction({ mikoFetch, providerName, providerUrl, apiKey, providerApi });
  ...
  }, [preview, connectionTested, mikoFetch, providerName, providerUrl, apiKey, providerApi, goToStep, showError, onProviderReady]);
