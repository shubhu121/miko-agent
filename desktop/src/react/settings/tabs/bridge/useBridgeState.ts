...
export interface WhatsAppStatus extends PlatformStatusBase {
...
function bridgeEditableFields(status: BridgeStatus | null): BridgeCredentialFieldValues {
  return {
    whatsappPhoneNumberId: status?.whatsapp?.phoneNumberId || '',
    whatsappWebhookUrl: status?.whatsapp?.webhookUrl || '',
...
function storedBridgeSecrets(status: BridgeStatus | null): StoredBridgeSecrets {
  return {
    ...
    whatsappAccessToken: hasStoredSecret(status?.whatsapp?.hasAccessToken, status?.whatsapp?.accessToken),
    whatsappVerifyToken: hasStoredSecret(status?.whatsapp?.hasVerifyToken, status?.whatsapp?.verifyToken),
    whatsappAppSecret: hasStoredSecret(status?.whatsapp?.hasAppSecret, status?.whatsapp?.appSecret),
...
export function useBridgeState() {
  ...
  useEffect(() => {
    ...
    mikoFetch(`/api/agents/${selectedAgentId}/public-ishiki`, { signal: ac.signal })
      .then(r => r.json())
  ...
  const savePublicIshiki = async () => {
    ...
    try {
      await mikoFetch(`/api/agents/${agentId}/public-ishiki`, {
        method: 'PUT',
  ...
  const loadStatus = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const res = await mikoFetch(`/api/bridge/status${query}`, signal ? { signal } : undefined);
  ...
  useEffect(() => {
    ...
    window.addEventListener('miko-bridge-reload', handler);
    return () => window.removeEventListener('miko-bridge-reload', handler);
  }, [loadStatus]);
  ...
  const testPlatform = async (
    ...
    try {
      const agentQuery = agentId ? `?agentId=${encodeURIComponent(agentId)}` : '';
      const res = await mikoFetch(`/api/bridge/test${agentQuery}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const setOwner = async (plat: string, userId: string) => {
    ...
    try {
      const agentQuery = agentId ? `?agentId=${encodeURIComponent(agentId)}` : '';
      const res = await mikoFetch(`/api/bridge/owner${agentQuery}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const saveGlobalSettings = async (partial: { permissionMode?: BridgePermissionMode; readOnly?: boolean; receiptEnabled?: boolean; richStreamingEnabled?: boolean }) => {
    ...
    try {
      const res = await mikoFetch('/api/bridge/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return {
    ...
    setTgToken,
    whatsappPhoneNumberId: credentialFields.whatsappPhoneNumberId.value,
    setWhatsAppPhoneNumberId,
    whatsappWebhookUrl: credentialFields.whatsappWebhookUrl.value,
    setWhatsAppWebhookUrl,
    whatsappAccessToken: secretDrafts.whatsappAccessToken.value,
    whatsappAccessTokenDraft: secretDrafts.whatsappAccessToken,
    setWhatsAppAccessToken,
    whatsappVerifyToken: secretDrafts.whatsappVerifyToken.value,
    whatsappVerifyTokenDraft: secretDrafts.whatsappVerifyToken,
    setWhatsAppVerifyToken,
    whatsappAppSecret: secretDrafts.whatsappAppSecret.value,
    whatsappAppSecretDraft: secretDrafts.whatsappAppSecret,
    setWhatsAppAppSecret,
