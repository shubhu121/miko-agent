 ...
 *   $env:MIKO_WIN32_SMOKE="1"; npx vitest run tests/manual/win32-packaged-smoke.test.ts
 *
...
const SMOKE_ENABLED = process.platform === "win32" && process.env.MIKO_WIN32_SMOKE === "1";
...
smokeDescribe("win32 packaged smoke", () => {
  ...
  beforeAll(() => {
    workDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-win32-smoke-"));
  });
  ...
  it("4. restricted-token sandbox helper launches when the helper is present", async () => {
    ...
    if (!helper) {
      console.warn("[smoke] miko-win-sandbox.exe not found; sandbox chain NOT verified in this tree");
      return;
    }
    const mikoHome = fs.mkdtempSync(path.join(os.tmpdir(), "miko-smoke-home-"));
    try {
      ...
      const exec = (await loadExec())({ sandbox: { policy, mikoHome, helperPath: helper } });
      const result = await exec("ipconfig", workDir, {
        onData: () => {},
        signal: undefined,
        timeout: 60,
  ...
  it("5. node-pty terminal launches and exits cleanly", async () => {
    const mikoHome = fs.mkdtempSync(path.join(os.tmpdir(), "miko-smoke-term-"));
    try {
      const { TerminalSessionManager } = await import("../../lib/terminal/terminal-session-manager.ts");
      const manager = new TerminalSessionManager({ mikoHome });
      const sessionPath = path.join(mikoHome, "smoke-session.jsonl");
  ...
  it("6. missing cwd is rejected with the actionable error, not a fake ENOENT", async () => {
    ...
    ).rejects.toMatchObject({ code: "MIKO_EXEC_CWD_MISSING" });
  });
});
