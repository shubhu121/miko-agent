...
describe('FloatingActions cover gallery', () => {
  beforeEach(() => {
    ...
    mocks.mikoFetch.mockImplementation(async (url: string) => {
      ...
      return new Response(JSON.stringify({ ok: true, cover: { image: "This feature is available in English only." } }), { status: 200 });
    });
    useStore.setState({
  ...
  it('does not expose the removed pink daisy fireworks preset', () => {
    ...
    expect(COVER_GALLERY_ITEMS.some(item => item.title === "This feature is available in English only.")).toBe(false);
  });
  ...
  it('refreshes markdown preview toggle i18n after locale sync', async () => {
    ...
    window.t = ((key: string) => (
      key === 'preview.markdownPreview' ? "This feature is available in English only." : key
    )) as typeof window.t;
