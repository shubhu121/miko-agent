 ...
 *   {mikoHome}/agents/{agentId}/avatars/agent.{png|jpg|jpeg|webp}
...
/**
 * @param {string|null|undefined} mikoHome - resolved MIKO_HOME data root
 ...
 */
function resolveAgentAvatarPath(mikoHome, agentId) {
  if (typeof mikoHome !== "string" || mikoHome.length === 0) return null;
  if (!isSafeAgentId(agentId)) return null;

  const avatarDir = path.join(mikoHome, "agents", agentId, "avatars");
