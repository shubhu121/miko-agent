...
function readAppearancePrefs(): AppearancePrefs {
  ...
  return {
    ...
    serifEnabled: localStorage.getItem('miko-font-serif') !== '0',
    paperTextureEnabled: isPaperTextureEnabled(localStorage),
    paperTextureBlocked: isPaperTextureBlockedTheme(concreteTheme),
    leavesOverlayEnabled: localStorage.getItem('miko-leaves-overlay') === '1',
  };
}
        ...
        <SettingsRow
          ...
          control={
            <Toggle
              ...
              onChange={(next) => {
                localStorage.setItem('miko-leaves-overlay', next ? '1' : '0');
                window.dispatchEvent(new CustomEvent('miko-settings', {
