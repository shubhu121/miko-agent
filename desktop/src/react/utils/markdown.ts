...
const AUTO_LINK_SUFFIX_BOUNDARY_CHARS = new Set([
  '\u200b', '\u200c', '\u200d', '\ufeff',
  'English only', 'English only', 'English only', 'English only', 'English only', 'English only', 'English only',
  'English only', 'English only', 'English only', 'English only', 'English only', 'English only', 'English only', 'English only', 'English only', 'English only',
]);
...
function footnoteIdPrefixForSource(src: string): string {
  return `miko-fn-${hashStringBase36(src)}`;
}
...
function footnoteDefinitions(md: MarkdownItInstance): void {
  md.block.ruler.before('reference', 'miko_footnote_definitions', (
    state: StateBlock,
    startLine: number,
    _endLine: number,
    silent: boolean,
...
function footnoteReferences(md: MarkdownItInstance): void {
  md.inline.ruler.before('link', 'miko_footnote_refs', (state: StateInline, silent: boolean) => {
    const start = state.pos;
    if (state.src.slice(start, start + 2) !== '[^') return false;
...
function appendFootnoteList(md: MarkdownItInstance): void {
  md.core.ruler.after('inline', 'miko_footnote_tail', (state: StateCore) => {
    const markdownEnv = state.env as MarkdownRenderEnv;
...
function markdownHeadingAnchors(md: MarkdownItInstance): void {
  md.core.ruler.push('miko_heading_anchors', (state: StateCore) => {
