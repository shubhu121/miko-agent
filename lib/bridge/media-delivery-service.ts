...
export class MediaDeliveryService {
  ...
  async sendFailureNotice(adapter, chatId, err, replyContext = null) {
    ...
    try {
      const message = "This feature is available in English only.";
  ...
  async _sendSessionFile(adapter, chatId, source, platform, targetMetadata: any = {}) {
    ...
    if (localPath && !this._supportsInputMode(adapter, "buffer") && this._supportsInputMode(adapter, "public_url")) {
      throw new Error(publicUrlRequiredMessage(platform, "This feature is available in English only."));
    }
  ...
  _refreshPublisherBaseUrl() {
    ...
    const baseUrl = this.engine?.getBridgeMediaPublicBaseUrl?.() || process.env.MIKO_BRIDGE_PUBLIC_BASE_URL || "";
...
function publicUrlRequiredMessage(platform, detail = null) {
  const suffix = detail ? `English only${detail.message || detail}` : "";
