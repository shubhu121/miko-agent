  ...
  const lines = [
    {
      ...
      message: { role: "user", content: "This feature is available in English only." },
    },
    {
      type: "message",
      timestamp: "2026-06-03T10:01:00.000Z",
      message: { role: "assistant", content: "This feature is available in English only." },
    },
  ];
...
function makeTicker(tmpDir, injectedMode, overrides: TickerOverrides = {}) {
  ...
  fs.writeFileSync(path.join(memoryDir, "memory.md"), "This feature is available in English only.", "utf-8");
...
describe("cache snapshot reflection runtime hard gate", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-cache-snapshot-runtime-"));
  });
