...
describe("agents route", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-agents-route-"));
  ...
  it("emits agent-created after creating an agent", async () => {
    ...
    const app = new Hono();
    const engine = {
      createAgent: vi.fn().mockResolvedValue({ id: "miko", name: "Miko" }),
      emitEvent: vi.fn(),
    };
    ...
    const res = await app.request("/api/agents", {
      ...
      body: JSON.stringify({ name: "Miko" }),
    });

    expect(res.status).toBe(200);
    expectAppEvent(engine.emitEvent, "agent-created", { agentId: "miko", name: "Miko" });
  });
  ...
  it("rejects an explicit non-ASCII agent id before calling the engine", async () => {
    ...
    const res = await app.request("/api/agents", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "Ming", id: "This feature is available in English only." }),
    });
  ...
  it("rejects an invalid primary agent id before calling the engine", async () => {
    ...
    const res = await app.request("/api/agents/primary", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: "This feature is available in English only." }),
    });
  ...
  it.each([
    { label: "non-ASCII", order: ["miko", "This feature is available in English only."], error: "agent ID" },
    { label: "non-string", order: ["miko", 42], error: "agent ID" },
    { label: "unknown", order: ["miko", "missing"], error: "not found" },
    { label: "duplicate", order: ["miko", "miko"], error: "duplicate" },
  ])("rejects $label agent order without persisting it", async ({ order, error }) => {
    const { createAgentsRoute } = await import("../server/routes/agents.ts");
    const app = new Hono();
    const engine = {
      getAgent: vi.fn((id) => id === "miko" ? { id } : null),
  ...
  it("persists an order only after every agent id is valid, unique, and known", async () => {
    const { createAgentsRoute } = await import("../server/routes/agents.ts");
    const app = new Hono();
    const knownIds = new Set(["Legacy_AGENT-1", "miko"]);
    const engine = {
    ...
    const order = ["Legacy_AGENT-1", "miko"];
    const res = await app.request("/api/agents/order", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ order }),
  ...
  it("invalidates the agent list cache when a fresh list is requested", async () => {
    ...
    const engine = {
      ...
      listAgents: vi.fn(() => [{ id: "miko", name: "Miko", isCurrent: true }]),
    };
    ...
    expect(res.status).toBe(200);
    expect(data.agents).toEqual([{ id: "miko", name: "Miko", isCurrent: true }]);
  ...
  it("returns create validation status codes without emitting agent-created", async () => {
    ...
    const res = await app.request("/api/agents", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "This feature is available in English only.", yuan: "caikangyong" }),
    });
    const data = await res.json();
  ...
  it("emits scoped config events after saving changed config blocks", async () => {
    ...
    const res = await app.request(`/api/agents/${agentId}/config`, {
      ...
      body: JSON.stringify({
        locale: "en-US",
        agent: { name: "Miko Prime", yuan: "butter" },
        desk: { home_folder: "/tmp/miko-work" },
  ...
  it("rejects invalid yuan updates before writing agent config", async () => {
    ...
    expect(fs.readFileSync(path.join(agentDir, "config.yaml"), "utf-8")).toContain("yuan: miko");
    expect(engine.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("refreshes generated description after identity changes", async () => {
    ...
    const res = await app.request(`/api/agents/${agentId}/identity`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ content: "This feature is available in English only." }),
    });

    expect(res.status).toBe(200);
    expect(fs.readFileSync(path.join(agentDir, "identity.md"), "utf-8")).toBe("This feature is available in English only.");
