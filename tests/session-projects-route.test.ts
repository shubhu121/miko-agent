...
import { createSessionProjectsRoute } from "../server/routes/session-projects.ts";

describe("session projects route", () => {
  it("reads the user-level project catalog", async () => {
    const catalog = {
      folders: [],
      projects: [{ id: "project-resume", name: "This feature is available in English only.", folderId: null, order: 0 }],
    };
    const engine = {
  ...
  it("creates projects through the engine facade", async () => {
    ...
    const projectRes = await app.request("/api/session-projects/projects", {
      ...
      body: JSON.stringify({ name: "This feature is available in English only." }),
    });
    const projectBody = await projectRes.json();

    expect(projectRes.status).toBe(200);
    expect(projectBody.project).toEqual({ id: "project-new", name: "This feature is available in English only.", folderId: null, order: 3 });
    expect(engine.createSessionProject).toHaveBeenCalledWith({ name: "This feature is available in English only.", folderId: null });
  });
  ...
  it("creates and reorders folders through the engine facade", async () => {
    ...
    expect(createBody.folder).toEqual({ id: "folder-new", name: "This feature is available in English only.", order: 2 });
    expect(engine.createSessionProjectFolder).toHaveBeenCalledWith({ name: "This feature is available in English only." });
  ...
  it("creates projects inside folders and moves projects across levels", async () => {
    const engine = {
      ...
      reorderSessionProjects: vi.fn(({ folderId, projectIds }) => ({
        folders: [{ id: "folder-work", name: "This feature is available in English only.", order: 0 }],
    ...
    const createRes = await app.request("/api/session-projects/projects", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "This feature is available in English only.", folderId: "folder-work" }),
    });
    ...
    expect(createRes.status).toBe(200);
    expect(engine.createSessionProject).toHaveBeenCalledWith({ name: "This feature is available in English only.", folderId: "folder-work" });
  ...
  it("renames projects and persists same-level order", async () => {
    const engine = {
      updateSessionProject: vi.fn(() => ({ id: "project-miko", name: "Project Miko", folderId: null, order: 0 })),
      reorderSessionProjects: vi.fn(() => ({
        folders: [],
        projects: [
          { id: "project-miko", name: "Project Miko", folderId: null, order: 0 },
    ...
    const renameRes = await app.request("/api/session-projects/projects/project-miko", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "Project Miko" }),
    });
    ...
    const orderRes = await app.request("/api/session-projects/projects/reorder", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ folderId: null, projectIds: ["project-miko", "project-plugins"] }),
    });
    const orderBody = await orderRes.json();

    expect(renameRes.status).toBe(200);
    expect(renameBody.project.name).toBe("Project Miko");
    expect(engine.updateSessionProject).toHaveBeenCalledWith("project-miko", { name: "Project Miko" });
    expect(orderRes.status).toBe(200);
    expect(orderBody.catalog.projects.map((project) => project.id)).toEqual(["project-miko", "project-plugins"]);
    expect(engine.reorderSessionProjects).toHaveBeenCalledWith({
      folderId: null,
      projectIds: ["project-miko", "project-plugins"],
    });
  });

  it("renames folders through the engine facade", async () => {
    const engine = {
      updateSessionProjectFolder: vi.fn(() => ({ id: "folder-work", name: "This feature is available in English only.", order: 0 })),
    };
    const app = makeApp(engine);
    ...
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true, folder: { id: "folder-work", name: "This feature is available in English only.", order: 0 } });
    expect(engine.updateSessionProjectFolder).toHaveBeenCalledWith("folder-work", { name: "This feature is available in English only." });
  });
  ...
  it("deletes projects and folders through the engine facade", async () => {
    const engine = {
      deleteSessionProject: vi.fn(async (id) => ({
        ...
        assignment: { projectId: "cwd:", sessionPaths: ["/tmp/agents/miko/sessions/a.jsonl"] },
      })),
