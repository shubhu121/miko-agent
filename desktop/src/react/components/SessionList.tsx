...
const SESSION_VIEW_MODE_KEY = 'miko-session-sidebar-view-mode';
const SESSION_DRAG_MIME = 'application/x-miko-session-path';
const PROJECT_DRAG_MIME = 'application/x-miko-project-id';
const FOLDER_DRAG_MIME = 'application/x-miko-project-folder-id';
...
function SessionListInner() {
  ...
  useEffect(() => {
    ...
    mikoFetch('/api/browser/session-states')
      .then(r => r.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const titleRes = await mikoFetch(`/api/sessions/search?q=${encodedQuery}&phase=title&limit=20`, {
        ...
        const contentRes = await mikoFetch(`/api/sessions/search?q=${encodedQuery}&phase=content&limit=20`, {
          signal: controller.signal,
          timeout: 12_000,
        });
  ...
  const persistSidebarProjectView = useCallback((
    ...
    mikoFetch('/api/preferences/sidebar-ui', {
      method: 'PUT',
  ...
  useEffect(() => {
    ...
    const loadSidebarUiPrefs = async () => {
      attempt += 1;
      try {
        const res = await mikoFetch('/api/preferences/sidebar-ui');
  ...
  useEffect(() => {
    ...
    window.addEventListener('miko-settings', handleLocalSettings);
    ...
    return () => {
      window.removeEventListener('miko-settings', handleLocalSettings);
  ...
  const handleCloseBrowserSession = useCallback(async (sessionPath: string) => {
    ...
    try {
      const res = await mikoFetch('/api/browser/close-session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
const SessionSummaryPreviewCard = memo(function SessionSummaryPreviewCard({
  ...
  useEffect(() => {
    ...
    mikoFetch(`/api/sessions/summary?path=${encodeURIComponent(session.path)}`)
      .then(res => res.json())
