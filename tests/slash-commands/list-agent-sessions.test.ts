...
import { listRecentAgentSessions } from "../../core/slash-commands/list-agent-sessions.ts";
...
describe("listRecentAgentSessions", () => {
  ...
  it("preserves title (null passthrough)", async () => {
    const engine = {
      listSessions: async () => [
        makeSession({ path: "/p.jsonl", agentId: "a1", modified: 1, title: "This feature is available in English only." }),
        makeSession({ path: "/q.jsonl", agentId: "a1", modified: 0, title: null }),
      ],
    };
    const r = await listRecentAgentSessions(engine, "a1");
    expect(r[0].title).toBe("This feature is available in English only.");
