...
describe("MediaPublisher", () => {
  ...
  function makeFile(name = "image.png") {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-media-publisher-"));
  ...
  it("publishes registered session files as tokenized expiring public URLs", () => {
    ...
    const publisher = new MediaPublisher({
      baseUrl: "https://miko.example.com",
      allowedRoots: [tmpDir],
      ttlMs: 60_000,
      now: () => 1000,
    ...
    expect(result.publicUrl).toBe("https://miko.example.com/api/bridge/media/token_123");
  ...
  it("can update the public base URL after construction", () => {
    ...
    expect(publisher.setBaseUrl("https://miko.example.com/")).toBe("https://miko.example.com");
    const result = publisher.publish({
      id: "sf_1",
      filePath,
      realPath: fs.realpathSync(filePath),
