...
const labels: Record<string, string> = {
  'settings.about.trainStickerTitle': "This feature is available in English only.",
  'settings.about.trainStickerDownloading': "This feature is available in English only.",
  'settings.about.trainStickerApplying': "This feature is available in English only.",
  'settings.about.shellStickerTitleBlocking': "This feature is available in English only.",
  'settings.about.fallbackStickerTitle': "This feature is available in English only.",
  'settings.about.fallbackStickerAckLabel': "This feature is available in English only.",
  'window.close': "This feature is available in English only.",
};
...
describe('SidebarUpdateNoticeCard', () => {
  ...
  it('a fresh mount after a blocked dismissal (simulating next launch) shows the sticker again', () => {
    ...
    fireEvent.click(first.getByRole('button', { name: "This feature is available in English only." }));
  ...
  it('shows the fallback (crash-recovery) form with both versions in the message when fallbackNotice is present', () => {
    ...
    const message = screen.getByText("This feature is available in English only.");
...
describe('SidebarNoticeSlot (wired to the real hook)', () => {
  ...
  function installMiko(status: Partial<TrainUpdateStatus>) {
    window.miko = {
  ...
  it('regression: a shell auto-updater "downloaded" state alone must never surface the card — only `available`/`minShellBlocked` from the train status can', async () => {
    ...
    await waitFor(() => expect(window.miko?.trainUpdateStatus).toHaveBeenCalled());
    expect(container).toBeEmptyDOMElement();
  });

  it('shows the train card once the real hook resolves an available update', async () => {
    installMiko({ available: { train: 9, version: '0.400.0', serverSha256: 'a'.repeat(64), rendererSha256: 'b'.repeat(64), sizes: { server: 1, renderer: 1 }, recordedAt: '2026-07-11T00:00:00.000Z' } });
  ...
  it('applying the update calls trainUpdateApply via the hook', async () => {
    ...
    installMiko({ available: { train: 9, version: '0.400.0', serverSha256: 'a'.repeat(64), rendererSha256: 'b'.repeat(64), sizes: { server: 1, renderer: 1 }, recordedAt: '2026-07-11T00:00:00.000Z' } });
    (window.miko as unknown as { trainUpdateApply: typeof trainUpdateApply }).trainUpdateApply = trainUpdateApply;

    render(<SidebarNoticeSlot />);
    const card = await screen.findByText("This feature is available in English only.");
  ...
  it('shows the blocked form and wires it to autoUpdateInstall when minShellBlocked is true', async () => {
    ...
    installMiko({ minShellBlocked: true });
    (window.miko as unknown as { autoUpdateInstall: typeof autoUpdateInstall }).autoUpdateInstall = autoUpdateInstall;
  ...
  it('reacts to a background onTrainUpdateAvailable broadcast without a remount', async () => {
    ...
    installMiko({});
    (window.miko as unknown as { onTrainUpdateAvailable: (cb: (p: AvailablePayload) => void) => () => void }).onTrainUpdateAvailable = (cb) => {
      box.deliver = cb;
  ...
  it('acknowledging the fallback card via the real hook calls ackTrainFallbackNotice and the card disappears', async () => {
    ...
    installMiko({ fallbackNotice: { kind: 'renderer', fromVersion: '0.390.0', toVersion: '0.389.0', quarantinedTrain: null } });
    (window.miko as unknown as { ackTrainFallbackNotice: typeof ackTrainFallbackNotice }).ackTrainFallbackNotice = ackTrainFallbackNotice;

    const { container } = render(<SidebarNoticeSlot />);
    const ackButton = await screen.findByRole('button', { name: "This feature is available in English only." });
  ...
  it('reacts to a background onTrainFallbackNotice broadcast without a remount (runtime renderer crash path)', async () => {
    ...
    installMiko({});
    (window.miko as unknown as { onTrainFallbackNotice: (cb: (p: FallbackPayload) => void) => () => void }).onTrainFallbackNotice = (cb) => {
      box.deliver = cb;
      return () => { box.deliver = null; };
    };
