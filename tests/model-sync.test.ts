...
const tmpDir = path.join(os.tmpdir(), "miko-test-model-sync-" + Date.now());
let modelsJsonPath;
let authJsonPath;
...
describe("syncModels", () => {
  it("writes providers with credentials and models to models.json", async () => {
    ...
    expect(result.providers.dashscope.apiKey).toBe("miko-runtime-api-key:dashscope");
  ...
  it("projects user-entered api keys as runtime refs so Pi SDK does not resolve env names", async () => {
    ...
    const result = JSON.parse(fs.readFileSync(modelsJsonPath, "utf-8"));
    expect(result.providers.custom.apiKey).toBe("miko-runtime-api-key:custom");
  });
  ...
  it("normalizes legacy Kimi Coding Plan transport without rewriting the configured model id", async () => {
    ...
    expect(result.providers["kimi-coding"]).toMatchObject({
      ...
      api: "openai-completions",
      apiKey: "miko-runtime-api-key:kimi-coding",
    });
  ...
  it("preserves user metadata for the configured Kimi Coding model id", async () => {
    ...
    const providers = {
      "kimi-coding": {
        ...
        api: "anthropic-messages",
        api_key: "sk-test",
        models: [{ id: "kimi-for-coding", name: "This feature is available in English only.", maxOutput: 16000, image: true, video: true, xhigh: true }],
      },
    };
  ...
  it("keeps Kimi Coding on official OpenAI-compatible thinking while other Anthropic-compatible reasoning models use anthropic format", async () => {
    ...
    expect(result.providers["minimax-token-plan"].apiKey).toBe("miko-runtime-api-key:minimax-token-plan");
    ...
    expect(result.providers["minimax-token-plan"].models[0].compat?.mikoVideoInput).toBeUndefined();
  });
  ...
  it("projects explicit video capability into Miko compat and keeps Pi input schema-compatible", async () => {
    const syncModels = await loadSync();
    ...
    const result = JSON.parse(fs.readFileSync(modelsJsonPath, "utf-8"));
    expect(result.providers.custom.models.map((m) => [m.id, m.input, m.compat?.mikoVideoInput])).toEqual([
  ...
  it("projects explicit audio capability into Miko compat and keeps Pi input schema-compatible", async () => {
    const syncModels = await loadSync();
    ...
    const result = JSON.parse(fs.readFileSync(modelsJsonPath, "utf-8"));
    expect(result.providers.custom.models.map((m) => [m.id, m.input, m.compat?.mikoVideoInput, m.compat?.mikoAudioInput])).toEqual([
  ...
  it("projects known OpenAI audio models into Miko compat without invalid Pi input", async () => {
    const syncModels = await loadSync();
    ...
    expect(result.providers.openai.models[0]).toMatchObject({
      id: "gpt-audio-mini",
      input: ["text"],
      compat: { mikoAudioInput: true },
    });
  });

  it("projects known video-capable models into Miko compat without invalid Pi input", async () => {
    const syncModels = await loadSync();
    ...
    expect(result.providers.dashscope.models[0].compat.mikoVideoInput).toBe(true);
  });

  it("projects Moonshot Kimi official video capability into Miko compat", async () => {
    const syncModels = await loadSync();
  ...
  it("writes Pi-loadable models when Miko video capability is enabled", async () => {
    const syncModels = await loadSync();
  ...
  it("derives no-auth policy from ProviderRegistry for existing Ollama configs", async () => {
    ...
    const mm = new ModelManager({ mikoHome: tmpDir });
    mm._modelRegistry = {
      refresh: vi.fn(),
  ...
  it("projects GLM Coding Plan through the Z.AI OpenAI-compatible endpoint with Zhipu thinking compat", async () => {
    ...
    expect(result.providers["zhipu-coding"]).toMatchObject({
      ...
      api: "openai-completions",
      apiKey: "miko-runtime-api-key:zhipu-coding",
    });
  ...
  it("projects OpenCode Go GLM-5.2 with explicit Zhipu thinking compat", async () => {
    ...
    expect(result.providers["opencode-go"]).toMatchObject({
      ...
      api: "openai-completions",
      apiKey: "miko-runtime-api-key:opencode-go",
    });
