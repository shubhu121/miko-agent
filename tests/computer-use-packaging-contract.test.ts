...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-computer-use-packaging-"));
  tempDirs.push(dir);
  return dir;
}
...
function makeMacAfterPackContext(appOutDir) {
  return {
    ...
    packager: {
      platform: { name: "mac" },
      appInfo: { productFilename: "Miko" },
    },
  };
}

function resourcesDir(appOutDir) {
  return path.join(appOutDir, "Miko.app", "Contents", "Resources");
}
...
describe("Computer Use packaging contract", () => {
  ...
  it("runs the helper build before electron-builder in the GitHub macOS release workflow", () => {
    ...
    expect(workflow).toContain("MIKO_COMPUTER_USE_HELPER_ARCH=${{ matrix.arch }}");
  });
});
