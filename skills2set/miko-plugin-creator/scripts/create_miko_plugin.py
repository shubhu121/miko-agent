...
SDK_PACKAGES = {
    "@miko/plugin-protocol": "plugin-protocol",
    "@miko/plugin-runtime": "plugin-runtime",
    "@miko/plugin-sdk": "plugin-sdk",
    "@miko/plugin-components": "plugin-components",
}

SDK_TARBALLS = {
    "@miko/plugin-protocol": "miko-plugin-protocol-*.tgz",
    "@miko/plugin-runtime": "miko-plugin-runtime-*.tgz",
    "@miko/plugin-sdk": "miko-plugin-sdk-*.tgz",
    "@miko/plugin-components": "miko-plugin-components-*.tgz",
}
...
def slugify(value: str) -> str:
    ...
    return slug or "miko-plugin"


def titleize(value: str) -> str:
    parts = re.split(r"[\s_-]+", value.strip())
    return " ".join(part[:1].upper() + part[1:] for part in parts if part) or "Miko Plugin"
...
def find_miko_root(*starts: Path) -> Path | None:
    seen: set[Path] = set()
    for start in starts:
...
def choose_sdk_mode(sdk_mode: str, miko_root: Path | None, template: str) -> str:
...
def required_sdk_packages(include_tool: bool, include_ui: bool, include_lifecycle: bool, template: str) -> list[str]:
    ...
    if include_tool or include_lifecycle:
        packages.append("@miko/plugin-protocol")
        packages.append("@miko/plugin-runtime")
    if include_ui:
        if "@miko/plugin-protocol" not in packages:
            packages.append("@miko/plugin-protocol")
        packages.extend(["@miko/plugin-sdk", "@miko/plugin-components"])
    return packages
...
def sdk_dependency(
    ...
    if sdk_mode == "workspace":
        if not miko_root:
            raise SystemExit("workspace SDK mode requires running from a Miko repo. Use --sdk-mode bundled.")
        package_dir = miko_root / "packages" / SDK_PACKAGES[package_name]
...
def manifest_for(
    ...
    manifest = {
        ...
        "name": display_name,
        "version": "0.1.0",
        "description": args.description or f"{display_name} plugin for Miko.",
...
def package_json_for(
    ...
    for package_name in required_sdk_packages(include_tool, include_ui, include_lifecycle, template):
        dependencies[package_name] = sdk_dependency(plugin_dir, miko_root, package_name, sdk_mode, bundled_sdk)
...
export async function execute(input = {{}}, toolCtx) {{
  ...
  const body = typeof input.body === "string" ? input.body : "Generated from a Miko plugin.";
...
import fs from "node:fs";
import path from "node:path";
import {{ createMediaDetails, defineTool }} from "@miko/plugin-runtime";
...
def create_direct_index(plugin_id: str, display_name: str) -> str:
    ...
    return f"""
const MIKO_BUS_SKIP = Symbol.for("miko.event-bus.skip");
  ...
  async onload() {{
    ...
    if (ctx.bus.handle) {{
      this.register(ctx.bus.handle({js_string(status_type)}, (payload) => {{
        if (payload?.pluginId && payload.pluginId !== ctx.pluginId) return MIKO_BUS_SKIP;
        return {{
          ok: true,
...
def create_direct_panel_js(display_name: str) -> str:
    return f"""
const PROTOCOL = "miko.plugin.ui";
const VERSION = 1;
let seq = 0;

function targetOrigin() {{
  const params = new URLSearchParams(window.location.search);
  const explicit = params.get("miko-host-origin");
...
function request(type, payload, timeoutMs = 10000) {{
  const id = `miko-plugin-${{Date.now()}}-${{++seq}}`;
...
function pluginApiFetch(path, init = {{}}) {{
  ...
  if (!surfaceSession) throw new Error("miko.api.fetch requires pluginSurfaceSession in the iframe URL.");
  const headers = new Headers(init.headers || {{}});
  headers.set("X-Miko-Plugin-Surface-Session", surfaceSession);
  return fetch(pluginApiUrl(path), {{ ...init, headers }});
}}

const miko = {{
  ready: () => event("miko.ready"),
  ...
  theme: {{
    getSnapshot: () => {{
      const params = new URLSearchParams(window.location.search);
      return {{ theme: params.get("miko-theme") || undefined, cssUrl: params.get("miko-css") || undefined }};
    }},
  }},
}};
...
function render() {{
    ...
    <main class="panel">
      <section class="card">
        <header class="card-header">
          <div>
            <h1 id="titleText"></h1>
            <p>A no-build Miko plugin panel.</p>
          </div>
  ...
  document.getElementById("copyBtn").addEventListener("click", async () => {{
    await miko.clipboard.writeText(titleInput.value);
    await miko.toast.show({{ message: "Copied title", type: "success" }});
  }});
  document.getElementById("openBtn").addEventListener("click", () => {{
    miko.external.open("https://example.com");
  }});
}}

render();
miko.ready();
miko.ui.resize({{ height: surface === "widget" ? 320 : 500 }});
"""
...
function Panel() {{
  ...
  useEffect(() => {{
    miko.ready();
    miko.ui.resize({{ height: surface === 'widget' ? 320 : 520 }});
  }}, [surface]);
  ...
  return (
    <MikoThemeProvider
      mode={{themeMode}}
      theme={{customTheme || (themeMode === 'miko' ? 'warm-paper' : undefined)}}
      className="plugin-panel"
    >
      <CardShell
        title={{title}}
        description="A Miko plugin panel using the SDK and shared components."
...
body {
  background: var(--miko-plugin-bg, #f8f5ed);
  color: var(--miko-plugin-text, #3b3d3f);
}
...
def create_readme(
    ...
    lines = [
        f"# {display_name}",
        "",
        f"Miko plugin id: `{plugin_id}`.",
        "",
    ]
    if audience == "beginner" or template == "direct":
        lines.extend([
            "This is a small Miko plugin starter. Start by changing labels, button actions, and the sample tool.",
            "",
            "## What to edit first",
            "",
            "- `manifest.json`: the name, description, and permissions Miko sees.",
        ])
        if include_provider:
            lines.extend([
                "- `providers/*.js`: the provider declaration Miko discovers.",
        ...
        if include_ui:
            lines.extend([
                ...
                "- `ui/Panel.tsx`: React iframe UI built with Miko SDK components.",
    ...
    lines.extend([
        "",
        "Install by dragging this folder into Miko Settings > Plugins, or place it under the user plugin directory reported by `/api/plugins/settings`.",
        "",
        ...
        "- Browser iframe code may open, pick, or request access to resources through `miko.resources.*`, but real file reads and writes belong in server-side plugin tools, routes, or lifecycle code.",
    ...
    if include_ui:
        lines.extend([
            "This plugin requires full-access because Miko page and widget contributions are route-backed WebView/iframe UI.",
            ...
            "The starter manifest grants only the host calls used by the sample panel. If you add `miko.resources.open()`, `miko.resources.pick()`, or `miko.resources.requestAccess()` calls, add the matching `resource.open`, `resource.pick`, or `resource.requestAccess` entries under `ui.hostCapabilities`.",
        ])
    if include_provider:
        lines.extend([
...
def scaffold(args: argparse.Namespace) -> Path:
    ...
    miko_root = find_miko_root(Path.cwd(), parent)
    root_package = read_package_json(miko_root)
    sdk_mode = choose_sdk_mode(args.sdk_mode, miko_root, template)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a Miko plugin scaffold.")
    ...
    template = choose_scaffold_template(args)
    print(f"Created Miko plugin scaffold: {plugin_dir}")
    ...
    if args.kind == "provider":
        ...
        print("  3. Install or drag the plugin folder into Miko with full-access enabled.")
        ...
        print("  2. Install or drag the plugin folder into Miko; no build step is required.")
    return 0
