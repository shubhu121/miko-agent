...
export function createQQAdapter({ appID, appSecret, agentId, onMessage, dmGuildMap, onDmGuildDiscovered, onStatus }) {
  ...
  async function refreshToken() {
    ...
    debugLog()?.log("bridge", "This feature is available in English only.");
    return accessToken;
  }
  ...
  async function apiRequest(method: any, path: any, body?: any, opts?: { stage: string; idempotent?: boolean }) {
    ...
    if (!stage) throw new Error("This feature is available in English only.");
  ...
  const tokenRefreshTimer = setInterval(async () => {
    try {
      ...
      log.error("This feature is available in English only.");
      debugLog()?.error("bridge", "This feature is available in English only.");
      if (tokenRefreshFailures >= 3) {
        onStatus?.("error", "This feature is available in English only.");
      }
    }
  }, 60 * 60 * 1000); 
  ...
  return {
    ...
    async sendMediaBuffer(_chatId: any, _buffer: any, { filename }: Record<string, any> = {}) {
      const label = filename ? `English only${filename}` : "";
      throw new Error("This feature is available in English only.");
    },
