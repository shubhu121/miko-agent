...
describe("AgentManager lazy runtime initialization", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    instances.length = 0;
    rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-agent-lazy-"));
  ...
  it("does not require startup logger objects to expose error()", async () => {
    ...
    expect(logger).toHaveBeenCalledWith(expect.stringContaining("This feature is available in English only."));
