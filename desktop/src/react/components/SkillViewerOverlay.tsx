...
export function SkillViewerOverlay() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const items = await miko?.listSkillFiles?.(data.baseDir) as TreeItem[] | undefined;
      setFiles(items || []);
  ...
  async function loadFile(filePath: string, name: string) {
    ...
    setFileName(name);
    const text = await window.miko?.readSkillFile?.(filePath);
  ...
  async function onCopy() {
    ...
    try {
      ...
      const res = await mikoFetch(`/api/skills/install?agentId=${encodeURIComponent(agentId)}`, {
        method: 'POST',
