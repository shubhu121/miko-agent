...
describe("MikoEngine.buildTools", () => {
  let tmpDir;
  ...
  it("passes the engine approval gateway into auto-mode tool execution", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-auto-approval-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
  ...
  it("wires utility model reviewers into the default approval gateway", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-engine-approval-gateway-"));
    const engine = new MikoEngine({
      mikoHome: tmpDir,
      productDir: tmpDir,
      agentId: "miko",
    } as any);
  ...
  it("hides stable availability-disabled tools before building the model schema", () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-availability-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
    const agent = {
      id: "focus",
      agentDir,
  ...
  it("passes a session workbench execution boundary into plugin tools", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-boundary-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
  ...
  it("passes the explicit buildTools SessionRef into plugin tool runtime context", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-plugin-session-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
    const workspace = path.join(tmpDir, "workspace");
  ...
  it("freezes and injects the explicit runtime SessionRef into agent tool context", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-agent-session-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
    const workspace = path.join(tmpDir, "workspace");
  ...
  it("passes Pi SDK fifth-argument session ctx into plugin tools", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-plugin-pi-ctx-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
    const workspace = path.join(tmpDir, "workspace");
  ...
  it("registers files created or modified by write and edit tools in the active session", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-touch-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
    const workspace = path.join(tmpDir, "workspace");
    ...
    const editResult = await edit.execute("edit-1", {
      path: "draft.md",
      edits: [{ oldText: "hello", newText: "hello Miko" }],
    });
  ...
  it("registers write and edit session files when Pi SDK uses the file_path alias", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-file-path-alias-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
    const workspace = path.join(tmpDir, "workspace");
  ...
  it("lets built-in file tools pick up newly authorized session folders without rebuilding tools", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-authorized-folders-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
    const workspace = path.join(tmpDir, "workspace");
  ...
  it("blocks direct agent config edits from built-in file tools even when sandbox is disabled", async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-managed-config-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
    const workspace = path.join(tmpDir, "workspace");
    ...
    const editResult = await edit.execute("edit-config", {
      path: configPath,
      edits: [{ oldText: "yuan: miko", newText: "yuan: caikangyong" }],
    });
    ...
    expect(fs.readFileSync(configPath, "utf-8")).toContain("yuan: miko");
  ...
  it("keeps plugin dev Agent tools hidden until the global dev setting is enabled", () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-build-tools-dev-"));
    const agentDir = path.join(tmpDir, "agents", "focus");
    const engine = Object.create(MikoEngine.prototype);
    engine.mikoHome = tmpDir;
    engine.getAgent = vi.fn(() => ({ id: "focus", agentDir, tools: [] }));
