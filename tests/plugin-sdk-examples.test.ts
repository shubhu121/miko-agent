...
const bundledSdkDir = path.join(root, "skills2set", "miko-plugin-creator", "assets", "sdk");
...
describe("plugin SDK examples and docs", () => {
  it("uses an absolute file URL for workspace SDK dependencies across Windows drives", () => {
    const scriptPath = path.join(root, "skills2set", "miko-plugin-creator", "scripts", "create_miko_plugin.py");
    const result = execFileSync("python3", [
      ...
      [
        ...
        "import sys",
        "spec = importlib.util.spec_from_file_location('create_miko_plugin', sys.argv[1])",
        "mod = importlib.util.module_from_spec(spec)",
        "spec.loader.exec_module(mod)",
        "print(mod.relative_file_spec(",
        "    PureWindowsPath('C:/Users/runner/AppData/Local/Temp/miko-ui-scaffold/sdk-panel'),",
        "    PureWindowsPath('D:/a/miko-agent/miko-agent/packages/plugin-sdk'),",
        "))",
      ].join("\n"),
      scriptPath,
    ], { cwd: root, encoding: "utf-8" }).trim();

    expect(result).toBe("file:///D:/a/miko-agent/miko-agent/packages/plugin-sdk");
  ...
  it("documents the SDK package map in a top-level guide", () => {
    ...
    expect(guide).toContain("@miko/plugin-protocol");
    expect(guide).toContain("@miko/plugin-sdk");
    expect(guide).toContain("@miko/plugin-runtime");
    expect(guide).toContain("@miko/plugin-components");
    expect(guide).toContain("miko.assets.url");
  ...
  it("documents ResourceIO access for user resources in plugin guides and Miko Plugin Creator", () => {
    ...
    const creatorSkill = fs.readFileSync(path.join(root, "skills2set", "miko-plugin-creator", "SKILL.md"), "utf-8");
  ...
  it("documents plugin tool sessionPermission metadata in SDK guides and Miko Plugin Creator", () => {
    const sdkGuide = fs.readFileSync(path.join(root, "PLUGIN_SDK.md"), "utf-8");
    const zhGuide = fs.readFileSync(path.join(root, "PLUGINS.md"), "utf-8");
    const enGuide = fs.readFileSync(path.join(root, "PLUGINS_EN.md"), "utf-8");
    const creatorSkill = fs.readFileSync(path.join(root, "skills2set", "miko-plugin-creator", "SKILL.md"), "utf-8");
  ...
  it("covers runtime tools, EventBus, iframe SDK, and shared components in example source", () => {
    ...
    expect(index).toContain("MIKO_BUS_SKIP");
    ...
    expect(panel).toContain("@miko/plugin-sdk");
    expect(panel).toContain("@miko/plugin-components");
    expect(panel).toContain("MikoThemeProvider");
    expect(readme).toContain("bundle the UI");
    expect(readme).toContain("miko.assets.url");
  });

  it("keeps miko-plugin-creator bundled SDK tarballs aligned with current runtime and protocol APIs", () => {
    const runtimePackage = JSON.parse(readBundledSdkFile("miko-plugin-runtime-0.0.0.tgz", "package.json"));
    const runtimeTypes = readBundledSdkFile("miko-plugin-runtime-0.0.0.tgz", "dist/index.d.ts");
    const runtimeReadme = readBundledSdkFile("miko-plugin-runtime-0.0.0.tgz", "README.md");
    const sdkTypes = readBundledSdkFile("miko-plugin-sdk-0.0.0.tgz", "dist/index.d.ts");
    const sdkReadme = readBundledSdkFile("miko-plugin-sdk-0.0.0.tgz", "README.md");
    const protocolTypes = readBundledSdkFile("miko-plugin-protocol-0.0.0.tgz", "dist/index.d.ts");
    ...
    expect(runtimeTypes).toContain("MikoProviderMediaMode");
    expect(runtimeTypes).toContain("MikoPluginResources");
    expect(runtimeTypes).toContain("MikoResourceRef");
    expect(runtimeTypes).toContain("writeExpectedVersion");
    expect(runtimeTypes).toContain("MikoResourceMoveResult");
    expect(runtimeTypes).toContain("MikoResourceTrashResult");
    expect(runtimeTypes).toContain("MikoResourceWatchSubscription");
    expect(runtimeTypes).toContain("watch(");
    expect(runtimeTypes).toContain("subscribe(");
    expect(runtimeTypes).toContain("MikoToolSessionPermission");
    ...
    expect(sdkReadme).toContain("miko.api.fetch");
    expect(sdkReadme).toContain("miko.resources.open");
  ...
  it("bundles protocol with runtime templates because runtime reuses protocol resource contracts", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-full-bundled-scaffold-"));
    try {
      execFileSync("python3", [
        path.join(root, "skills2set", "miko-plugin-creator", "scripts", "create_miko_plugin.py"),
      ...
      expect(pkg.dependencies).toMatchObject({
        "@miko/plugin-runtime": expect.stringContaining("vendor/sdk/miko-plugin-runtime-"),
        "@miko/plugin-protocol": expect.stringContaining("vendor/sdk/miko-plugin-protocol-"),
      });
      expect(fs.existsSync(path.join(pluginDir, "vendor", "sdk", "miko-plugin-protocol-0.0.0.tgz"))).toBe(true);
      expect(fs.existsSync(path.join(pluginDir, "vendor", "sdk", "miko-plugin-runtime-0.0.0.tgz"))).toBe(true);
    } finally {
  ...
  it("scaffolds provider contribution plugins with explicit media capabilities", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-provider-scaffold-"));
    try {
      execFileSync("python3", [
        path.join(root, "skills2set", "miko-plugin-creator", "scripts", "create_miko_plugin.py"),
        "Jimeng Provider",
  ...
  it("scaffolds React UI plugins for host-served assets without source maps", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-ui-scaffold-"));
    try {
      execFileSync("python3", [
        path.join(root, "skills2set", "miko-plugin-creator", "scripts", "create_miko_plugin.py"),
        "SDK Panel",
  ...
  it("scaffolds direct UI helpers with plugin API surface-session fetch support", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-direct-ui-scaffold-"));
    try {
      execFileSync("python3", [
        path.join(root, "skills2set", "miko-plugin-creator", "scripts", "create_miko_plugin.py"),
        "Direct API Panel",
      ...
      expect(panel).toContain("X-Miko-Plugin-Surface-Session");
