...
describe("parseSessionKey", () => {
  ...
  it("parses new format with @agentId suffix", () => {
    const cases = [
      ["tg_dm_123@miko",       { platform: "telegram",  chatType: "dm",    chatId: "123",  agentId: "miko" }],
      ["tg_group_abc@kuro",    { platform: "telegram",  chatType: "group", chatId: "abc",  agentId: "kuro" }],
      ["fs_dm_u001@miko",      { platform: "feishu",    chatType: "dm",    chatId: "u001", agentId: "miko" }],
      ["dt_dm_u001@miko",      { platform: "dingtalk",  chatType: "dm",    chatId: "u001", agentId: "miko" }],
      ["dt_group_c99@miko",    { platform: "dingtalk",  chatType: "group", chatId: "c99",  agentId: "miko" }],
      ["qq_dm_x99@agent-1",    { platform: "qq",        chatType: "dm",    chatId: "x99",  agentId: "agent-1" }],
      ["wx_dm_wxid@miko",      { platform: "wechat",    chatType: "dm",    chatId: "wxid", agentId: "miko" }],
    ];
...
describe("resolveBridgeSessionIdentity", () => {
  it("resolves QQ display name and avatar from principal metadata", () => {
    ...
    }, { sessionKey: "qq_dm_c2c-openid@miko" })).toMatchObject({
      userId: "principal-1",
      principalId: "principal-1",
