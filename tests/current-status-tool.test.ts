...
function makeCtx(sessionPath = "/tmp/agents/miko/sessions/s1.jsonl") {
  return {
    sessionManager: {
...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-current-status-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("current_status tool", () => {
  ...
  it("lists available status keys without returning live status values", async () => {
    const tool = createCurrentStatusTool({
      ...
      getAgent: () => ({ id: "miko", agentName: "Miko" }),
    ...
    expect(JSON.stringify(payload)).not.toContain("Miko");
  ...
  it("returns the current session folder scope without relying on prompt injection", async () => {
    const sessionPath = "/tmp/agents/miko/sessions/s1.jsonl";
    const tool = createCurrentStatusTool({
  ...
  it("records the exact session and same timestamp after get time succeeds", async () => {
    ...
    const sessionPath = "/tmp/agents/miko/sessions/time.jsonl";
    const tool = createCurrentStatusTool({
      now: () => now,
      getTimezone: () => "UTC",
      onTimeObserved,
  ...
  it("does not record time when the time provider fails", async () => {
    ...
    await expect(tool.execute(
      ...
      null,
      null,
      makeCtx("/tmp/agents/miko/sessions/failure.jsonl"),
  ...
  it("returns no appearance image when only the Yuan default avatar exists", async () => {
    ...
    const agentDir = path.join(root, "agents", "miko");
    fs.mkdirSync(path.join(root, "desktop", "src", "assets"), { recursive: true });
    fs.writeFileSync(path.join(root, "desktop", "src", "assets", "Miko.png"), Buffer.from("yuan-default"));
    ...
    const tool = createCurrentStatusTool({
      getAgent: () => ({
        ...
        agentDir,
        productDir: root,
        config: { agent: { name: "Miko", yuan: "miko" } },
      }),
  ...
  it("reads agent appearance from the profile resource and only summarizes user avatars through the auxiliary bridge", async () => {
    ...
    expect(summarizeResources).toHaveBeenCalledWith(expect.objectContaining({
      sessionPath: "/tmp/agents/miko/sessions/s1.jsonl",
    ...
    expect(payload.appearance.agent.summary).toBe("This feature is available in English only.");
