...
describe('Mobile PWA entry split', () => {
  ...
  it('wires service worker update detection to an explicit mobile refresh event', () => {
    ...
    expect(entrySource).toContain('miko-mobile-update-available');
    expect(entrySource).toContain('miko-mobile-apply-update');
    expect(entrySource).toContain('__mikoMobileUpdateAvailable');
  ...
  it('serves mobile PWA assets explicitly in Vite dev instead of falling back to HTML', () => {
    ...
    expect(viteConfig).toContain("name: 'miko-serve-mobile-pwa-static-files'");
