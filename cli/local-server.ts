...
export function resolveCliMikoHome(env = process.env) {
  return resolveHomePath(env.MIKO_HOME || path.join(os.homedir(), ".miko"));
}

function resolveHomePath(value) {
  const raw = String(value || "").trim();
  if (!raw) return path.join(os.homedir(), ".miko");
...
export function readLocalServerInfo({ mikoHome = resolveCliMikoHome(), checkProcess = true } = {}) {
  const filePath = path.join(mikoHome, "server-info.json");
  if (!fs.existsSync(filePath)) {
    return {
      ok: false,
      reason: "missing_server_info",
      filePath,
      message: `No running Miko Server was found at ${filePath}`,
    };
  }
  ...
  if (checkProcess && Number.isInteger(info.pid) && !isProcessAlive(info.pid)) {
    return {
      ...
      filePath,
      message: `Miko Server process ${info.pid} is no longer running`,
    };
  }
...
export function resolveConnection({ url, token, mikoHome }: { url?: string; token?: string; mikoHome?: string } = {}) {
  if (url) {
    return {
      ok: true,
  ...
  const local = readLocalServerInfo({ mikoHome });
