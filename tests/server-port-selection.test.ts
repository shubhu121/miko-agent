...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "miko-server-port-selection-"));
}
...
describe("server port selection", () => {
  ...
  describe("isMikoServerListeningOnPort", () => {
    it("returns true for a server that answers with status and version", async () => {
      const { isMikoServerListeningOnPort } = await import("../core/server-port-selection.ts");
      ...
      const result = await isMikoServerListeningOnPort({ port: address.port, host: "127.0.0.1" });
    ...
    it("returns false when nothing is listening on the port", async () => {
      ...
      const result = await isMikoServerListeningOnPort({ port: freePort, host: "127.0.0.1", timeoutMs: 500 });
      expect(result).toBe(false);
    });
  });

  describe("decideLoopbackBindFallback", () => {
    it("falls back on EADDRINUSE in loopback mode when unpinned and not another Miko server", async () => {
    ...
    it("refuses to fall back when the occupant is another Miko server", async () => {
      const { decideLoopbackBindFallback } = await import("../core/server-port-selection.ts");
      expect(decideLoopbackBindFallback({
        errCode: "EADDRINUSE",
        networkMode: "loopback",
