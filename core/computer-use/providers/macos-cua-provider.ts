...
const MIKO_CURSOR_BLOOM_COLOR = "#537D96";
const MIKO_CURSOR_GRADIENT_COLORS = Object.freeze(["#FFFDF8", "#8FAABD", "#2F4A56"]);
const MIKO_CUA_CURSOR_STYLE = Object.freeze({
  gradient_colors: MIKO_CURSOR_GRADIENT_COLORS,
  bloom_color: MIKO_CURSOR_BLOOM_COLOR,
  image_path: "",
});
const MIKO_AGENT_CURSOR_CONFIG_ENV = "MIKO_AGENT_CURSOR_CONFIG_JSON";
const MIKO_AGENT_SOCKET_PATH_ENV = "MIKO_COMPUTER_USE_SOCKET_PATH";
const MIKO_CURSOR_MOTION = Object.freeze({
  start_handle: 0.38,
  end_handle: 0.28,
  arc_size: 0.08,
  arc_flow: 0,
...
function helperPath(root: any) {
  return path.join(root, "miko-computer-use-helper");
}

function defaultMikoComputerUseSocketPath(homeDir = os.homedir()) {
  return path.join(homeDir, "Library", "Caches", "miko-computer-use", "miko-computer-use-helper.sock");
}

function commandIsBundledMikoHelper(command: any) {
  return path.basename(String(command || "")) === "miko-computer-use-helper";
}

function bundledHelperCandidates({ env, mikoRoot, cwd, arch }: any) {
  const roots = [];
  if (env.MIKO_COMPUTER_USE_RUNTIME_ROOT) {
    roots.push(env.MIKO_COMPUTER_USE_RUNTIME_ROOT);
  }
  if (mikoRoot) {
    roots.push(path.resolve(mikoRoot, "..", "computer-use", "macos"));
    roots.push(path.resolve(mikoRoot, "dist-computer-use", `mac-${arch}`));
  }
  if (cwd) {
...
export function resolveCuaDriverCommand({
  ...
  mikoRoot = env.MIKO_ROOT,
  cwd = process.cwd(),
  arch = process.arch,
} = {}) {
  const candidates = [
    env.MIKO_COMPUTER_USE_HELPER_PATH,
    ...bundledHelperCandidates({ env, mikoRoot, cwd, arch }),
    env.MIKO_CUA_DRIVER_PATH,
...
export function createMacosCuaProvider({
  ...
  cursorStyle = MIKO_CUA_CURSOR_STYLE,
  cursorImagePath = null,
  cursorBloomColor = MIKO_CURSOR_BLOOM_COLOR,
  cursorEnabled = true,
  cursorMotion = MIKO_CURSOR_MOTION,
  runner = createCommandRunner(),
  timeoutMs = 30000,
  launchRetryAttempts = 3,
  launchRetryDelayMs = 350,
  socketPath = process.env[MIKO_AGENT_SOCKET_PATH_ENV] || defaultMikoComputerUseSocketPath(),
  ...
  const bundledMikoHelper = commandIsBundledMikoHelper(command);
  const shouldAutoStartDaemon = autoStartDaemon ?? bundledMikoHelper;
  const resolvedCursorStyle = normalizeCursorStyle({ cursorStyle, cursorImagePath, cursorBloomColor });
  const nativeCursorEnabled = bundledMikoHelper && cursorEnabled !== false && Boolean(resolvedCursorStyle);
  const mikoCursorRuntimeConfig = resolvedCursorStyle && bundledMikoHelper
    ? {
  ...
  function runEnv(baseEnv: any) {
    return {
      ...(baseEnv || process.env),
      [MIKO_AGENT_SOCKET_PATH_ENV]: socketPath,
      ...(mikoCursorRuntimeConfig
        ? { [MIKO_AGENT_CURSOR_CONFIG_ENV]: JSON.stringify(mikoCursorRuntimeConfig) }
        : {}),
    };
  }
  ...
  async function stopManagedDaemon() {
    if (!bundledMikoHelper || !shouldAutoStartDaemon) {
  ...
  async function ensureNativeCursorConfigured() {
    if (!resolvedCursorStyle || !bundledMikoHelper) return;
