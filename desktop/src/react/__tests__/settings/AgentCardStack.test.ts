...
import { mikoFetch } from '../../settings/api';
...
vi.mock('../../settings/api', () => ({
  ...
  yuanFallbackAvatar: (yuan?: string) => `fallback:${yuan || 'miko'}`,
}));
...
const agents = [
  { id: 'miko', name: "This feature is available in English only.", yuan: 'miko', isPrimary: true, hasAvatar: false },
  { id: 'deepseek', name: 'DeepSeek', yuan: 'deepseek', isPrimary: false, hasAvatar: false },
  { id: 'maomao', name: "This feature is available in English only.", yuan: 'maomao', isPrimary: false, hasAvatar: false },
];
...
describe('AgentCardStack actions', () => {
  it('keeps avatar URLs stable across hover renders and changes them only with avatarRevision', () => {
    ...
    const stack = screen.getByText("This feature is available in English only.").closest('[class*="agent-cards"]') as HTMLElement;
  ...
  it('keeps hover, focus, and drag expansion ownership independent and cleans up pointer cancellation', () => {
    ...
    expect(mikoFetch).not.toHaveBeenCalled();
  });

  it('keeps the existing drag reorder commit semantics on pointerup', () => {
    vi.mocked(mikoFetch).mockResolvedValue(new Response('{}'));
    render(React.createElement(AgentCardStack, {
      agents,
      selectedId: 'miko',
      currentAgentId: 'miko',
    ...
    const stack = screen.getByText('DeepSeek').closest('[class*="agent-cards"]') as HTMLElement;
    const card = screen.getByText("This feature is available in English only.").closest('[data-agent-id="miko"]') as HTMLElement;
    vi.spyOn(stack, 'matches').mockReturnValue(true);
    Object.defineProperty(card, 'setPointerCapture', { configurable: true, value: vi.fn() });
    ...
    expect(mikoFetch).toHaveBeenCalledWith('/api/agents/order', expect.objectContaining({
      method: 'PUT',
      body: JSON.stringify({ order: ['deepseek', 'miko', 'maomao'] }),
    }));
  });
