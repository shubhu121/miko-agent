...
export function getLocalAccountSummary(mikoHome) {
  const { user } = loadDefaultUser(mikoHome);
  const auth = loadLocalUserAuth(mikoHome);
  const credential = findCredential(auth, user.userId);
  return sanitizeAccount(user, { passwordSet: !!credential });
}

export function updateLocalAccountProfile(mikoHome, {
  username,
  displayName,
  now = new Date().toISOString(),
}: { username?: string; displayName?: string; now?: string } = {}) {
  const users = loadUsers(mikoHome);
  ...
  user.updatedAt = now;
  users.updatedAt = now;
  writeJsonAtomic(path.join(mikoHome, USERS_FILE), users);
  const auth = loadLocalUserAuth(mikoHome);
  return sanitizeAccount(user, { passwordSet: !!findCredential(auth, user.userId) });
}

export function setLocalAccountPassword(mikoHome, {
  password,
  now = new Date().toISOString(),
  ...
  auth.updatedAt = now;
  writeJsonAtomic(path.join(mikoHome, LOCAL_USER_AUTH_FILE), auth);
  return sanitizeAccount(user, { passwordSet: true });
}

export function clearLocalAccountPassword(mikoHome, {
  now = new Date().toISOString(),
} = {}) {
  const { user } = loadDefaultUser(mikoHome);
  const auth = loadLocalUserAuth(mikoHome);
...
export function verifyLocalAccountPassword(mikoHome, { username, password }: { username?: string; password?: string } = {}) {
...
export function loadLocalUserAuth(mikoHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(mikoHome, LOCAL_USER_AUTH_FILE);
...
function loadDefaultUser(mikoHome) {
  const users = loadUsers(mikoHome);
  return { users, user: getDefaultUser(users) };
}

function loadUsers(mikoHome) {
  return validateUsers(readJsonRequired(path.join(mikoHome, USERS_FILE), USERS_FILE), USERS_FILE);
}
