...
function getCachePath(engine: any) {
  return path.join(engine.mikoHome, "models-cache.json");
}
...
export function createProvidersRoute(engine: any) {
  ...
  function mikoCatalogFallback(name: any) {
    if (!name) return { error: "name is required for Miko model catalog", models: [] };
    ...
    if (payload.models.length === 0) {
      return {
        source: "miko-catalog",
        error: `No models found in Miko catalog for provider "${sourceProviderId}"`,
        ...payload,
      };
    }
    return { source: "miko-catalog", ...payload };
  }
  ...
  route.post("/providers/fetch-models", async (c) => {
    ...
    // Miko-owned models-json OAuth catalogs never probe ChatGPT /models. Legacy
    ...
    if (chatProvider?.projection === "models-json" && chatProvider.credentialSource === "auth-storage") {
      return c.json(mikoCatalogFallback(name));
    }
      ...
      try {
        ...
        if (res.ok) {
          ...
          const remoteModels = enrichDiscoveredModelMetadata(
            name,
            normalizeRemoteModels(data, effectiveApi),
          );
