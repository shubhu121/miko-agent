...
describe("TerminalSessionManager", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-terminal-"));
  ...
  it("keeps terminal ownership scoped to the creating session", async () => {
    ...
    const sessionPath = path.join(tmpDir, "agents", "miko", "sessions", "s1.jsonl");
    const otherSessionPath = path.join(tmpDir, "agents", "miko", "sessions", "s2.jsonl");
  ...
  it("rejects start with a missing cwd before touching the pty backend", async () => {
    ...
    await expect(
      manager.start({ sessionPath, agentId: "miko", cwd: gone }),
    ).rejects.toMatchObject({ code: "MIKO_EXEC_CWD_MISSING", cwd: gone });
  ...
  it("writes only to a running terminal owned by the same session", async () => {
    ...
    const sessionPath = path.join(tmpDir, "agents", "miko", "sessions", "s1.jsonl");
    const started = await manager.start({ sessionPath, agentId: "miko", cwd: tmpDir });
  ...
  it("closes live terminals for one session without touching another session", async () => {
    ...
    const sessionPath = path.join(tmpDir, "agents", "miko", "sessions", "s1.jsonl");
    const otherSessionPath = path.join(tmpDir, "agents", "miko", "sessions", "s2.jsonl");
    const first = await manager.start({ sessionPath, agentId: "miko", cwd: tmpDir });
    const second = await manager.start({ sessionPath: otherSessionPath, agentId: "miko", cwd: tmpDir });
  ...
  it("closes live terminals through a moved session path with the same session id", async () => {
    const originalPath = path.join(tmpDir, "agents", "miko", "sessions", "original.jsonl");
    const movedPath = path.join(tmpDir, "agents", "miko", "sessions", "archived", "renamed.jsonl");
    ...
    const started = await manager.start({ sessionPath: originalPath, agentId: "miko", cwd: tmpDir });
