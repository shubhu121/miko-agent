...
export function ensureDeviceAccessRegistries(mikoHome, { now = new Date().toISOString() } = {}) {
  const created = [];
  ...
  for (const [file, emptyRegistry, validate] of registries) {
    const filePath = path.join(mikoHome, file);
...
export function loadDeviceAccessRegistries(mikoHome) {
  ensureDeviceAccessRegistries(mikoHome);
  const devices = readJsonRequired(path.join(mikoHome, DEVICES_FILE), DEVICES_FILE);
  const credentials = readJsonRequired(path.join(mikoHome, DEVICE_CREDENTIALS_FILE), DEVICE_CREDENTIALS_FILE);
  const pairingSessions = readJsonRequired(path.join(mikoHome, PAIRING_SESSIONS_FILE), PAIRING_SESSIONS_FILE);
...
export function createDeviceCredential(mikoHome, input) {
  const now = input?.now || new Date().toISOString();
  const registries = loadDeviceAccessRegistries(mikoHome);
  const issued = issueDeviceCredential(registries, input, { now });
  persistDeviceAccessRegistries(mikoHome, registries);
  return cloneIssuedCredential(issued);
}

export function authenticateDeviceCredential(mikoHome, secret, { now = new Date().toISOString() } = {}) {
...
export function revokeDeviceCredential(mikoHome, credentialId, { now = new Date().toISOString() } = {}) {
...
export function revokeDevice(mikoHome, deviceId, { now = new Date().toISOString() } = {}) {
...
export function createPairingSession(mikoHome, input) {
  const now = input?.now || new Date().toISOString();
...
export function approvePairingSession(mikoHome, input) {
  const now = input?.now || new Date().toISOString();
...
function issueDeviceCredential(registries, input, { now }) {
  ...
  const secret = `miko_dev_${randomToken(32)}`;
...
function persistDeviceAccessRegistries(mikoHome, registries) {
  ...
  writeJsonAtomic(path.join(mikoHome, DEVICES_FILE), registries.devices);
  writeJsonAtomic(path.join(mikoHome, DEVICE_CREDENTIALS_FILE), registries.credentials);
  writeJsonAtomic(path.join(mikoHome, PAIRING_SESSIONS_FILE), registries.pairingSessions);
}
