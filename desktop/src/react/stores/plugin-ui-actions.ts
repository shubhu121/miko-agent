...
export async function refreshPluginUI(): Promise<void> {
  try {
    ...
    const [pagesResult, widgetsResult, grantsResult, prefsResult] = await Promise.allSettled([
      mikoFetch('/api/plugins/pages').then(r => r.json()),
      mikoFetch('/api/plugins/widgets').then(r => r.json()),
      mikoFetch('/api/plugins/ui-host-capabilities').then(r => r.json()),
      mikoFetch('/api/preferences/plugin-ui').then(r => r.json()),
    ]);
...
function persistField(field: Record<string, unknown>): void {
  mikoFetch('/api/preferences/plugin-ui', {
    method: 'PUT',
