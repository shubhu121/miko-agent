...
export function ensureGrantRegistry(mikoHome, { now = new Date().toISOString() } = {}) {
  const filePath = grantRegistryPath(mikoHome);
...
export function loadGrantRegistry(mikoHome) {
  ensureGrantRegistry(mikoHome);
  return validateGrantRegistry(readJsonRequired(grantRegistryPath(mikoHome), GRANTS_FILE), GRANTS_FILE);
}

export function createGrant(mikoHome, input: Record<string, any> = {}) {
  const now = input.now || new Date().toISOString();
  const registry = loadGrantRegistry(mikoHome);
  ...
  persistGrantRegistry(mikoHome, registry);
  return clonePlain(grant);
}

export function findActiveGrantsForPrincipal(mikoHome, principalId, { now = new Date().toISOString() } = {}) {
  ...
  if (changed) persistGrantRegistry(mikoHome, registry);
  return active;
}

export function revokeGrant(mikoHome, grantId, { now = new Date().toISOString() } = {}) {
...
export function persistGrantRegistry(mikoHome, registry) {
  validateGrantRegistry(registry, GRANTS_FILE);
  writeJsonAtomic(grantRegistryPath(mikoHome), registry);
}

export function grantRegistryPath(mikoHome) {
  if (!isNonEmptyString(mikoHome)) throw new Error("mikoHome required");
  return path.join(mikoHome, SECURITY_DIR, GRANTS_FILE);
}
