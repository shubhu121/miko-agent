  ...
  const switchModel = useCallback(async (modelId: string, provider?: string) => {
    try {
      ...
      if (sessionHasMessages && currentSessionPath) {
        ...
        const res = await mikoFetch('/api/models/switch', {
          method: 'POST',
        ...
        if (data.adaptations?.length) {
          ...
          const text = data.adaptations.map((a: string) => msgs[a] || a).join('English only');
        ...
        const setRes = await mikoFetch('/api/models/set', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
