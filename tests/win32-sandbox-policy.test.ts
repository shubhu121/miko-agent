...
describe("Windows sandbox policy projection", () => {
  ...
  function makeTree() {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-win32-sandbox-"));
  ...
  it("projects restricted-token write roots without external read grants", () => {
    ...
    expect(grants.optionalWritePaths).toContain(real(path.join(mikoHome, ".ephemeral")));
    expect(grants.denyWritePaths).not.toContain(real(path.join(workspace, ".git")));
    expect(grants.denyWritePaths).not.toContain(real(path.join(mikoHome, "session-files")));
  });

  it("keeps the Windows write model functionality-first for Git worktrees", () => {
    const { mikoHome, agentDir, workspace } = makeTree();
    const policy = deriveSandboxPolicy({
      agentDir,
      workspace,
      workspaceFolders: [],
  ...
  it("projects explicit runtime writable roots for language caches and bundled runtimes", () => {
    const { mikoHome, agentDir, workspace } = makeTree();
    const runtimeRoot = path.join(mikoHome, ".ephemeral", "runtime-cache");
  ...
  it("keeps read-only Miko prompt files out of Windows ACL projection", () => {
    const { mikoHome, agentDir, workspace, externalDir } = makeTree();
    const externalFile = path.join(externalDir, "reference.md");
  ...
  it("derives read-only external grants from active session files without re-granting workspace or managed-cache files", () => {
    const { mikoHome, workspace, externalDir } = makeTree();
    const externalFile = path.join(externalDir, "reference.md");
    const workspaceFile = path.join(workspace, "owned.md");
    const managedFile = path.join(mikoHome, "session-files", "cache", "image.png");
