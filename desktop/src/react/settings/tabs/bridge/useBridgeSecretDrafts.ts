...
const PLATFORM_SECRET_FIELDS: Partial<Record<string, Array<{
  key: BridgeSecretDraftKey;
  field: string;
}>>> = {
  telegram: [{ key: 'telegramToken', field: 'token' }],
  whatsapp: [
    { key: 'whatsappAccessToken', field: 'accessToken' },
    { key: 'whatsappVerifyToken', field: 'verifyToken' },
    { key: 'whatsappAppSecret', field: 'appSecret' },
  ],
...
const PLATFORM_CREDENTIAL_FIELDS: Partial<Record<string, Record<string, BridgeCredentialFieldKey>>> = {
  whatsapp: { phoneNumberId: 'whatsappPhoneNumberId', webhookUrl: 'whatsappWebhookUrl' },
...
function createDrafts(
  ...
  return {
    ...
    whatsappAccessToken: createDraft(ownerId, stored.whatsappAccessToken),
    whatsappVerifyToken: createDraft(ownerId, stored.whatsappVerifyToken),
    whatsappAppSecret: createDraft(ownerId, stored.whatsappAppSecret),
...
export function useBridgeCredentialDrafts(
  ...
  ): BridgeSecretSubmission[] => {
    const bindings = PLATFORM_SECRET_FIELDS[platform];
    if (!bindings || !credentials) return [];
    return bindings.flatMap((binding) => {
      if (!Object.prototype.hasOwnProperty.call(credentials, binding.field)) return [];
      const current = draftsRef.current[binding.key];
      if (current.ownerId !== ownerIdRef.current || !current.dirty) return [];
      return [{
        key: binding.key,
  ...
  const markSubmissionSaved = useCallback((submissions: BridgeSecretSubmission[]) => {
    if (submissions.length === 0) return;
    setDrafts(previous => {
      let next = previous;
      for (const submission of submissions) {
        if (!belongsToOwner(next, submission.ownerId)) continue;
  ...
  const setWhatsAppAccessToken = useCallback((value: string) => {
    setSecretValue('whatsappAccessToken', value);
  }, [setSecretValue]);
  const setWhatsAppVerifyToken = useCallback((value: string) => {
    setSecretValue('whatsappVerifyToken', value);
  }, [setSecretValue]);
  const setWhatsAppAppSecret = useCallback((value: string) => {
    setSecretValue('whatsappAppSecret', value);
  }, [setSecretValue]);
  const setWhatsAppPhoneNumberId = useCallback((value: string) => {
    setCredentialFieldValue('whatsappPhoneNumberId', value);
  }, [setCredentialFieldValue]);
  const setWhatsAppWebhookUrl = useCallback((value: string) => {
    setCredentialFieldValue('whatsappWebhookUrl', value);
  }, [setCredentialFieldValue]);
