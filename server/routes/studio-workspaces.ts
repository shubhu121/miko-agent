...
async function listStudioWorkspaces(engine, requestContext) {
  ...
  for (const mount of listStudioMountsForStudio(engine.mikoHome, studioId)) {
...
async function createLocalPathWorkspace(engine, requestContext, body) {
  ...
  const mount = upsertStudioMount(engine.mikoHome, {
...
function disableLocalPathWorkspace(engine, requestContext, mountId) {
  const studioId = requireStudioId(requestContext);
  const activeMount = listStudioMountsForStudio(engine.mikoHome, studioId)
  ...
  return disableStudioMount(engine.mikoHome, mountId, { hostStudioId: studioId });
}
