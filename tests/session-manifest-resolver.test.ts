...
describe("SessionManifestResolver", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-session-resolver-"));
    nextId = 1;
  ...
  function createSessionFile(name) {
    const sessionPath = path.join(tmpDir, "agents", "miko", "sessions", `${name}.jsonl`);
  ...
  it("creates a manifest on demand only for an existing legacy JSONL session", () => {
    ...
    const manifest = resolver.resolve({ sessionPath }, {
      createOnDemand: true,
      manifestDefaults: { domain: "desktop", ownerAgentId: "miko" },
    });

    expect(manifest.sessionId).toBe("sess_resolver_0001");
    expect(manifest.ownerAgentId).toBe("miko");
  ...
  it("rejects missing legacy paths instead of silently inventing a session", () => {
    const missingPath = path.join(tmpDir, "agents", "miko", "sessions", "missing.jsonl");
