...
export function AgentTab() {
  ...
  const currentYuan = settingsConfig?.agent?.yuan || 'miko';
  ...
  const saveAgent = async () => {
    try {
      ...
      if (identityChanged) {
        requests.push(mikoFetch(`${agentBase}/identity`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (ishikiChanged) {
        requests.push(mikoFetch(`${agentBase}/ishiki`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const openAgentExportPreview = async (agentId: string) => {
    ...
    try {
      const res = await mikoFetch('/api/character-cards/export/preview', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const confirmAgentExport = async () => {
    ...
    try {
      const res = await mikoFetch('/api/character-cards/export', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
        ...
        <AgentCardStack
          ...
          onAvatarClick={() => {
            ...
            input.addEventListener('change', () => {
              if (input.files?.[0]) {
                window.dispatchEvent(new CustomEvent('miko-open-cropper', {
          ...
          onDelete={(id) => window.dispatchEvent(new CustomEvent('miko-show-agent-delete', {
            detail: { agentId: id },
          }))}
          onExport={openAgentExportPreview}
          onAdd={() => window.dispatchEvent(new Event('miko-show-agent-create'))}
            ...
            <SelectWidget
              ...
              onChange={async (refKey) => {
                ...
                if (slashIdx <= 0 || slashIdx === refKey.length - 1) {
                  ...
                  console.warn("This feature is available in English only.", refKey);
                  return;
                }
        ...
        <div className={`${styles['settings-form-field']} ${styles['settings-form-field-center']}`}>
          ...
          <YuanSelector
            ...
            onChange={async (key) => {
              const agentId = getSettingsAgentId()!;
              try {
                await mikoFetch(`/api/agents/${agentId}/config`, {
                  method: 'PUT',
                  headers: { 'Content-Type': 'application/json' },
