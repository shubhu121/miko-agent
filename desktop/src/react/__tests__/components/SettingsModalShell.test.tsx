...
vi.mock('../../settings/SettingsContent', () => ({
  SettingsContent: ({
    ...
    <>
      {variant === 'modal' && (
        <button type="button" aria-label="This feature is available in English only." onClick={onClose}>
          English-only content.</button>
      )}
      <button type="button" aria-label="This feature is available in English only." onClick={() => onActiveTabChange?.('experiments')}>
        English-only content.</button>
...
describe('SettingsModalShell', () => {
  beforeEach(() => {
    vi.stubGlobal('t', (key: string) => {
      if (key === 'settings.title') return "This feature is available in English only.";
      if (key === 'settings.back') return "This feature is available in English only.";
      return key;
    });
  ...
  it('renders the settings content inside a modal dialog when open', async () => {
    ...
    expect(screen.getByRole('dialog', { name: "This feature is available in English only." })).toBeInTheDocument();
  ...
  it('keeps the plugin marketplace on the standard settings shell', async () => {
    ...
    expect(screen.getByRole('dialog', { name: "This feature is available in English only." })).not.toHaveAttribute('data-wide');
  });
  ...
  it('does not close when clicking inside the settings card', async () => {
    ...
    render(<SettingsModalShell />);
    fireEvent.mouseDown(screen.getByRole('dialog', { name: "This feature is available in English only." }));
