...
function analyzeFile(file) {
  ...
  const hasAcceptanceLanguage = //.test(text);
  const flags = {
    hasSnapshot: text.includes("toMatchSnapshot") || text.includes("toMatchInlineSnapshot"),
    hasFsTmp: /\bmkdtemp(?:Sync)?\b|\btmpdir\b|\bMIKO_HOME\b|\.miko(?:-dev)?/.test(haystack),
    ...
    hasPersistence: /migration|persist|store|registry|sidecar|sqlite|jsonl|yaml|atomic|write|read|cache|MIKO_HOME|mkdtemp/i.test(haystack),
    ...
    hardcodesMikoHome: /\.miko(?:-dev)?/.test(haystack),
  };
...
function decide(flags, category) {
  ...
  if (category === "persistence" || category === "adapter" || category === "architecture" || category === "ui-state") {
    return flags.touchesPrivateMock || flags.hardcodesMikoHome ? "SHRINK" : "KEEP";
  }
  if (category === "scaffold") return "QUARANTINE";
  if (flags.touchesPrivateMock || flags.hardcodesMikoHome) return "SHRINK";
  return "QUARANTINE";
}
...
function deleteOrMergeCandidate(flags, category, decision) {
  ...
  if (flags.hardcodesMikoHome) candidates.push("replace hardcoded Miko data directory with injected temp MIKO_HOME");
