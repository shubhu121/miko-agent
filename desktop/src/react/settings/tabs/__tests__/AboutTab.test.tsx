...
function installMiko(overrides: Record<string, unknown> = {}) {
...
const digest = (version: string) => ({
  ...
  summary: { zh: "This feature is available in English only.", en: `${version} summary` },
  counts: { feature: 0, fix: 1, improvement: 0, migration: 0 },
  items: [{
    id: `${version}-fix`,
    kind: 'fix' as const,
    importance: 'high' as const,
    title: { zh: "This feature is available in English only.", en: `${version} fix` },
    summary: { zh: "This feature is available in English only.", en: `${version} fix detail` },
    details: [],
    sources: [],
  }],
});
...
describe('AboutTab', () => {
  ...
  it('the beta toggle drives both the shell channel IPC and a train update check', async () => {
    ...
    installMiko({ autoUpdateSetChannel });
    useSettingsStore.setState({ settingsConfig: { auto_check_updates: true, update_channel: 'stable' } });
