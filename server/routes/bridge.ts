...
import { WHATSAPP_GRAPH_API_VERSION } from "../../lib/bridge/whatsapp-adapter.ts";
...
function timingSafeStringEqual(left: unknown, right: unknown) {
  const a = Buffer.from(String(left || ""));
  const b = Buffer.from(String(right || ""));
  return a.length === b.length && a.length > 0 && timingSafeEqual(a, b);
}

function hasCompleteWhatsAppConfig(config: any = {}) {
  return ["accessToken", "phoneNumberId", "verifyToken", "appSecret"]
    .every((key) => typeof config[key] === "string" && config[key].trim());
}

function hasValidWhatsAppSignature(rawBody: string, appSecret: string, signature: string | undefined) {
...
export function buildBridgeStatus(engine: any, manager: any, agent: any) {
  ...
  return {
    ...
    whatsapp: {
      ...platformStatus("whatsapp", wa, {
        configured: hasCompleteWhatsAppConfig(wa),
        phoneNumberId: cleanBridgeString(wa.phoneNumberId),
        webhookUrl: cleanBridgeString(wa.webhookUrl),
        accessToken: maskSecretValue(wa.accessToken),
        verifyToken: maskSecretValue(wa.verifyToken),
        appSecret: maskSecretValue(wa.appSecret),
        hasAccessToken: !!cleanBridgeString(wa.accessToken),
        hasVerifyToken: !!cleanBridgeString(wa.verifyToken),
        hasAppSecret: !!cleanBridgeString(wa.appSecret),
      }),
    },
    permissionMode,
    readOnly,
...
export function createBridgeRoute(engine: any, bridgeManagerRef: any) {
  ...
  route.get("/bridge/whatsapp/:agentId", (c) => {
    const agent = engine.getAgent(c.req.param("agentId"));
    const config = agent?.config?.bridge?.whatsapp;
    const mode = c.req.query("hub.mode");
    const verifyToken = c.req.query("hub.verify_token");
    const challenge = c.req.query("hub.challenge");
    if (!config || mode !== "subscribe" || !timingSafeStringEqual(config.verifyToken, verifyToken)) {
      return c.text("Forbidden", 403);
    }
    return c.text(challenge || "", 200);
  });

  route.post("/bridge/whatsapp/:agentId", async (c) => {
    const agent = engine.getAgent(c.req.param("agentId"));
    const config = agent?.config?.bridge?.whatsapp;
    if (!agent || !config?.enabled || !hasCompleteWhatsAppConfig(config)) {
      return c.text("Not Found", 404);
    }
    const rawBody = await c.req.text();
    if (!hasValidWhatsAppSignature(rawBody, config.appSecret, c.req.header("x-hub-signature-256"))) {
      return c.text("Forbidden", 403);
    }
    let payload: any;
    try {
    ...
    if (!manager) return c.text("Service Unavailable", 503);
    await manager.handleWhatsAppWebhook(agent.id, payload);
  ...
  route.get("/bridge/sessions/:sessionKey/messages", async (c) => {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        const visibleContent = sanitizeBridgeVisibleText(textContent) || (hasMedia ? `[Image x${mediaCount}]` : "");
        messages.push({
          role: msg.role,
  ...
  route.post("/bridge/test", async (c) => {
    ...
    try {
      ...
      if (platform === "telegram") {
        ...
        if (!hasCompleteWhatsAppConfig(effectiveCredentials)) {
          return c.json({ ok: false, error: "WhatsApp requires an access token, phone number ID, verify token, and app secret" });
        }
        const response = await fetch(
          `https://graph.facebook.com/${WHATSAPP_GRAPH_API_VERSION}/${encodeURIComponent(effectiveCredentials.phoneNumberId)}?fields=display_phone_number,verified_name`,
          {
        ...
        if (!response.ok || info?.error) {
          return c.json({ ok: false, error: info?.error?.message || `WhatsApp credential test failed (${response.status})` });
        }
        return c.json({ ok: true, info: { name: info.verified_name, phoneNumber: info.display_phone_number } });
      ...
      if (platform === "wechat") {
        ...
        return c.json({ ok: true, info: { msg: "WeChat iLink connected" } });
      }
...
function isUnsupportedMediaDeliveryError(err: any) {
  ...
  return /unsupported|public_url fallback|cannot deliver|does not support media input mode/i.test(message);
}
