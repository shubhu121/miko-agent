...
describe("fs route", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-fs-route-"));
  ...
  function buildApp({ mikoHome, workspace }) {
    const app = new Hono();
    const engine = {
      mikoHome,
      currentAgentId: "miko",
      getHomeCwd: vi.fn((agentId) => agentId === "miko" ? workspace : null),
      getAgent(id) {
        if (id !== "miko") return null;
        return {
          id: "miko",
  ...
  it("rejects symlink escapes from an allowed workspace", async () => {
    ...
    const app = buildApp({ mikoHome, workspace });
