...
describe('renderer i18n flat dotted keys', () => {
  ...
  it('resolves exact flat dotted keys before nested dot-path fallback', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response(JSON.stringify({
      'preview.markdownPreview': "This feature is available in English only.",
    ...
    expect(window.t('preview.markdownPreview')).toBe("This feature is available in English only.");
    expect(window.t('common.screenshot')).toBe("This feature is available in English only.");
  });
});
