...
describe("macos Cua provider", () => {
  it("resolves a configured Cua Driver command before common locations", () => {
    const command = resolveCuaDriverCommand({
      env: { MIKO_CUA_DRIVER_PATH: "/opt/cua-driver" },
  ...
  it.skipIf(!isMacOS)("prefers a Miko-bundled Computer Use helper over an external Cua Driver install", () => {
    const command = resolveCuaDriverCommand({
      env: {
        MIKO_ROOT: "/Applications/Miko.app/Contents/Resources/server",
        MIKO_CUA_DRIVER_PATH: "/opt/cua-driver",
      },
      existsSync: (p) => p === "/Applications/Miko.app/Contents/Resources/computer-use/macos/miko-computer-use-helper"
        || p === "/opt/cua-driver",
      homeDir: "/Users/miko",
      arch: "arm64",
      cwd: "/Users/miko/project-miko",
    });

    expect(command).toBe("/Applications/Miko.app/Contents/Resources/computer-use/macos/miko-computer-use-helper");
  });

  it.skipIf(!isMacOS)("resolves the development helper build output before falling back to PATH", () => {
    const command = resolveCuaDriverCommand({
      env: { MIKO_ROOT: "/Users/miko/project-miko" },
      existsSync: (p) => p === "/Users/miko/project-miko/dist-computer-use/mac-arm64/miko-computer-use-helper",
      homeDir: "/Users/miko",
      arch: "arm64",
      cwd: "/Users/miko/project-miko",
    });

    expect(command).toBe("/Users/miko/project-miko/dist-computer-use/mac-arm64/miko-computer-use-helper");
  });
  ...
  it("normalizes text-only permission output from the embedded helper", async () => {
    const { runner } = makeRunner((_command, args) => {
      if (args[0] === "status") {
        expect(args).toEqual(["status", "--socket", "/tmp/miko.sock"]);
        return { stdout: "miko-computer-use-helper running", stderr: "", exitCode: 0 };
      }
    ...
    const provider = createMacosCuaProvider({
      platform: "darwin",
      command: "/tmp/miko-computer-use-helper",
      runner,
      socketPath: "/tmp/miko.sock",
    });
  ...
  it("configures Miko's native cursor before controlling an app", async () => {
    const { runner, calls } = makeRunner((_command, args) => {
  ...
  it("starts the bundled helper daemon before using cached element-index tools", async () => {
    ...
    expect(serveCall).toMatchObject({
      command: "/tmp/miko-computer-use-helper",
      args: ["serve", "--socket", "/tmp/miko.sock"],
  ...
  it("stops the bundled helper daemon when the provider is stopped", async () => {
    ...
    const { runner, calls } = makeRunner((_command, args) => {
      ...
      if (args[0] === "stop") {
        return { stdout: "miko-computer-use-helper daemon stopped.", stderr: "", exitCode: 0 };
      }
      if (args[0] === "launch_app") {
        return rawResult({
          pid: 844,
    ...
    expect(stopCall).toMatchObject({
      command: "/tmp/miko-computer-use-helper",
      args: ["stop", "--socket", "/tmp/miko.sock"],
    });
  });
  ...
  it("normalizes a Cua window state response into a Miko snapshot", async () => {
    const { runner } = makeRunner((_command, args) => {
    ...
    expect(snapshot).toMatchObject({
      ...
      elements: [
        { elementId: "3", role: "AXRow", label: "This feature is available in English only." },
  ...
  it("passes Miko cursor runtime config to each bundled helper process", async () => {
    const { runner, calls } = makeRunner((_command, args) => {
      return rawResult({ ok: true });
    });
    const provider = createMacosCuaProvider({
    ...
    const clickCall = calls.find((call) => call.command === "/tmp/miko-computer-use-helper" && call.args[0] === "click");
    expect(clickCall?.options?.env?.MIKO_AGENT_CURSOR_CONFIG_JSON).toBeTruthy();
    const cursorConfig = JSON.parse(clickCall.options.env.MIKO_AGENT_CURSOR_CONFIG_JSON);
  ...
  it("uses AXShowDefaultUI instead of double-click fallback for rows that do not support AXPress", async () => {
    ...
    const provider = createMacosCuaProvider({ platform: "darwin", command: "/tmp/miko-computer-use-helper", runner });
    const lease = {
      leaseId: "lease-1",
      appId: "com.apple.Music",
      windowId: "10725",
