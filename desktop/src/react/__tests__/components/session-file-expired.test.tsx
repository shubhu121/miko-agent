...
describe('expired session file presentation', () => {
  beforeEach(() => {
    const tMap: Record<string, string> = {
      'chat.fileExpired': "This feature is available in English only.",
  ...
  it('renders voice-input transcript above the playable audio strip without an empty text bubble', () => {
    ...
    const transcript = screen.getByText("This feature is available in English only.");
    const wave = screen.getByTestId('audio-attachment-wave');
  ...
  it('renders assistant image previews through resource content URL when desktop file URL is unavailable', () => {
    ...
    useStore.setState({
      activeServerConnection: {
        ...
        label: 'Remote Miko',
        baseUrl: 'https://miko.example',
        wsUrl: 'wss://miko.example',
        token: 'remote token',
        authState: 'paired',
        trustState: 'tunnel',
    ...
    expect(preview).toHaveAttribute(
      'src',
      'https://miko.example/api/resources/res_sf_img/content',
    );
  });
  ...
  it('renders a phone-download action for staged file cards backed by a resource URL', () => {
    ...
    expect(download).toHaveAttribute('href', 'http://miko.local:14500/api/resources/res_sf_demo/content');
