...
export const WEB_SESSION_COOKIE_NAME = "miko_session";

const SCHEMA_VERSION = 1;
const SECRET_PREFIX_LENGTH = 18;
const DEFAULT_TTL_MS = 14 * 24 * 60 * 60 * 1000;

export function ensureWebSessionRegistry(mikoHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(mikoHome, WEB_SESSIONS_FILE);
...
export function loadWebSessionRegistry(mikoHome) {
  ensureWebSessionRegistry(mikoHome);
  const registry = readJsonRequired(path.join(mikoHome, WEB_SESSIONS_FILE), WEB_SESSIONS_FILE);
  return validateWebSessionRegistry(registry, WEB_SESSIONS_FILE);
}

export function createWebSession(mikoHome, input: { principal?: any; now?: string; ttlMs?: number; userAgent?: string } = {}) {
  assertNonEmptyString(mikoHome, "mikoHome");
  ...
  const secret = `miko_web_${randomToken(32)}`;
  ...
  const registry = loadWebSessionRegistry(mikoHome);
  registry.sessions.push(session);
  registry.updatedAt = now;
  persistWebSessionRegistry(mikoHome, registry);
  return {
...
export function authenticateWebSession(mikoHome, cookieHeader, { now = new Date().toISOString() } = {}) {
...
export function revokeWebSession(mikoHome, cookieHeader, { now = new Date().toISOString() } = {}) {
  const secret = parseCookie(cookieHeader, WEB_SESSION_COOKIE_NAME);
...
function persistWebSessionRegistry(mikoHome, registry) {
  validateWebSessionRegistry(registry, WEB_SESSIONS_FILE);
  writeJsonAtomic(path.join(mikoHome, WEB_SESSIONS_FILE), registry);
}
