...
const VALID_YUAN = new Set(["miko", "butter", "ming", "kong"]);
...
function isZipPath(filePath) {
  ...
  return lower.endsWith(".zip") || lower.endsWith(".miko-package.zip");
}
...
function normalizeAgent(card) {
  ...
  const yuan = VALID_YUAN.has(agent.yuan || card?.yuan) ? (agent.yuan || card?.yuan) : "miko";
...
function ensurePreviewAssets(plan) {
  const yuan = plan.agent?.yuan || "miko";
  if (!plan.assets.avatar) {
    copyPlanAsset(plan, defaultAvatarForYuan(yuan), "avatar", "miko-default-avatar");
  }
  if (!plan.assets.cardFront && plan.assets.avatar) {
    plan.assets.cardFront = { ...plan.assets.avatar };
  }
  if (!plan.assets.cardBack) {
    copyPlanAsset(plan, defaultCardBackForYuan(yuan), "cardBack", "miko-default-card-back");
  }
  if (!plan.assets.yuanIcon) {
    copyPlanAsset(plan, defaultYuanIconForYuan(yuan), "yuanIcon", "miko-default-yuan-icon");
  }
}
...
export function createCharacterCardService(engine) {
  const stagingRoot = path.join(engine.mikoHome, ".ephemeral", "character-card-imports");
  ...
  function readAgentExportSource(agentId) {
    ...
    const yuan = VALID_YUAN.has(config?.agent?.yuan) ? config.agent.yuan : "miko";
  ...
  async function exportAgentPackage(agentId, options: Record<string, any> = {}) {
    ...
    const token = randomToken();
    const exportRoot = path.join(engine.mikoHome, ".ephemeral", "character-card-exports", token);
