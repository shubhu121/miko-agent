...
// Miko custom tools — in production these come from agent.tools getter
// and flow through buildTools.customTools.
const MIKO_CUSTOM_OBJS = [
...
describe("session-coordinator tool snapshot (createSession)", () => {
  ...
  beforeEach(() => {
    ...
    coord = new SessionCoordinator({
      ...
      getSkills: () => null,
      buildTools: () => ({ tools: SDK_BUILTIN_OBJS, customTools: MIKO_CUSTOM_OBJS }),
      emitEvent: vi.fn(),
      emitDevLog: vi.fn(),
      getHomeCwd: () => tmpDir,
  ...
  it("includes enabled MCP tools in fresh session runtime options and snapshots", async () => {
    ...
    coord._d.buildTools = () => ({
      ...
      customTools: [...MIKO_CUSTOM_OBJS, mcpTool],
    });
  ...
  it("Case C: fresh sessions exclude computer when its global experiment gate is closed", async () => {
    ...
    coord._d.buildTools = () => ({
      tools: SDK_BUILTIN_OBJS,
      customTools: [...MIKO_CUSTOM_OBJS, computerTool],
    });
    currentAgentConfig = { tools: { disabled: [] } };
  ...
  it("This feature is available in English only.", async () => {
    ...
    coord._d.buildTools = () => ({
      tools: SDK_BUILTIN_OBJS,
      customTools: [...MIKO_CUSTOM_OBJS, beautifyTool],
    });
    currentAgentConfig = {};
  ...
  it("Case A: restore keeps newly registered tools inactive when they are absent from the frozen snapshot", async () => {
    ...
    coord._d.buildTools = () => ({
      tools: SDK_BUILTIN_OBJS,
      customTools: [...MIKO_CUSTOM_OBJS, dynamicTool],
    });
  ...
  it("reminds from the owning session frozen-minus-live inventory without activating new live tools", async () => {
    ...
    focusAgent.getToolsSnapshot = () => [...MIKO_CUSTOM_OBJS, oldMcpTool, newlyAddedTool];
  ...
  it("does not inject passive UI context into the last user message", async () => {
    ...
    const messages = [{ role: "user", content: "This feature is available in English only." }];
    ...
    expect(messages).toEqual([{ role: "user", content: "This feature is available in English only." }]);
  });
  ...
  it("restores plugin tools from the sessionId capability snapshot even when legacy meta is partial", async () => {
    ...
    coord._d.buildTools = () => ({
      tools: SDK_BUILTIN_OBJS,
      customTools: [...MIKO_CUSTOM_OBJS, mediaTool],
    });
  ...
  describe("capability drift (#1624)", () => {
    ...
    it("restore keeps the frozen snapshot but no longer computes or wakes capability drift", async () => {
      ...
      coord._d.buildTools = () => ({
        tools: SDK_BUILTIN_OBJS,
        customTools: [...MIKO_CUSTOM_OBJS, newTool],
      });
      currentAgentConfig = { tools: { disabled: [] } };
  ...
  describe("refreshCapabilitySnapshots (#1624)", () => {
    it("rebuilds the tool snapshot with Case C semantics (plugin tools included) and persists it", async () => {
      ...
      coord._d.buildTools = () => ({
        tools: SDK_BUILTIN_OBJS,
        customTools: [...MIKO_CUSTOM_OBJS, pluginTool],
      });
      currentAgentConfig = { tools: { disabled: [] } };
      await fsp.writeFile(
        path.join(sessionDir, "session-meta.json"),
