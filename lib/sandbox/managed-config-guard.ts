...
export function isManagedAgentConfigPath(filePath, { mikoHome }: { mikoHome?: string } = {}) {
  if (!filePath || !mikoHome) return false;

  const target = resolveExistingOrNearest(filePath);
  const home = resolveExistingOrNearest(mikoHome);
...
export function createManagedConfigWriteGuard({ mikoHome }: { mikoHome?: string } = {}) {
  return (absolutePath, operation) => {
    if (operation !== "write" && operation !== "delete") return { allowed: true };
    if (!isManagedAgentConfigPath(absolutePath, { mikoHome })) return { allowed: true };
    return {
      allowed: false,
