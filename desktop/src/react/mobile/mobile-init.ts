...
export async function initializeMobileRuntime(principal: MobilePrincipal): Promise<{
  ...
  const bootstrapRes = await mikoFetch('/api/mobile/bootstrap');
  ...
  await applyAgentIdentity({
    agentName: bootstrap.agentName || 'Miko',
  ...
  if (currentAgent) {
    ...
    useStore.setState({
      ...
      agentYuan: currentAgent.yuan || bootstrap.agentYuan || 'miko',
      homeFolder,
...
export async function createMobileSession(): Promise<string | null> {
  ...
  const res = await mikoFetch('/api/sessions/new', {
    method: 'POST',
