...
describe("skill bundle store", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-skill-bundles-"));
    engine = { mikoHome: tempDir };
  });
  ...
  it("detaches deleted agents from imported bundles while keeping bundle membership", () => {
    const bundle = createSkillBundle(engine, {
      ...
      agentId: "miko-2",
      sourcePackage: "miko-charactercard.zip",
    });

    const store = detachAgentFromBundles(engine, "miko-2");
