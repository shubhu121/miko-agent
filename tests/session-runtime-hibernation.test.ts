...
function makeAgent(root = "/tmp/miko-runtime-hibernation") {
  return {
    id: "miko",
    agentDir: path.join(root, "agents", "miko"),
    sessionDir: path.join(root, "agents", "miko", "sessions"),
...
function makeCoordinator( overrides: any = {}) {
  const root = overrides.root || "/tmp/miko-runtime-hibernation";
...
describe("SessionCoordinator runtime hibernation", () => {
  ...
  it("releases a focused runtime while preserving the current session path", async () => {
    const sessionPath = "/tmp/miko-runtime-hibernation/agents/miko/sessions/current.jsonl";
  ...
  it("rejects the next live-session prompt before vision or Pi when its model was disabled", async () => {
    const sessionPath = "/tmp/miko-runtime-hibernation/agents/miko/sessions/gpt56.jsonl";
    const staleModel = {
      id: "gpt-5.6-sol",
  ...
  it("rebinds a live session to current Miko metadata and continues prompting", async () => {
    const sessionPath = "/tmp/miko-runtime-hibernation/agents/miko/sessions/gpt56-metadata.jsonl";
    const staleModel = {
      id: "gpt-5.6-sol",
      provider: "openai-codex",
  ...
  it("hibernates only heavy idle runtimes under memory pressure", async () => {
    const sessionPath = "/tmp/miko-runtime-hibernation/agents/miko/sessions/heavy.jsonl";
    const session = makeSession(sessionPath, {
      messages: [
        {
          role: "user",
