...
export function QuickChatApp() {
    ...
    async function bootstrap() {
      try {
        const [serverPort, serverToken] = await Promise.all([
          window.miko?.getServerPort?.(),
          window.miko?.getServerToken?.(),
        ]);
  ...
  const resetDetachedSession = useCallback(() => {
    ...
    window.miko?.quickChatResize?.('compact');
  }, []);

  useEffect(() => {
    const dispose = window.miko?.onQuickChatShown?.(() => {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      setDraft('');
      setAttachments([]);
      window.miko?.quickChatResize?.('chat');
  ...
  const openFullSession = useCallback(() => {
    ...
    markHidden();
    window.miko?.quickChatOpenSession?.(sessionPathRef.current);
  }, [markHidden]);

  const closeQuickChat = useCallback(() => {
    markHidden();
    window.miko?.quickChatHide?.();
  }, [markHidden]);
  ...
  useLayoutEffect(() => {
    ...
    window.miko?.quickChatResize?.({ mode, height });
