...
export function ensureServerNetworkConfig(mikoHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(mikoHome, SERVER_NETWORK_FILE);
...
export function loadServerNetworkConfig(mikoHome) {
  ensureServerNetworkConfig(mikoHome);
  const config = readJsonRequired(path.join(mikoHome, SERVER_NETWORK_FILE), SERVER_NETWORK_FILE);
  return validateServerNetworkConfig(config, SERVER_NETWORK_FILE);
}

export function saveServerNetworkConfig(mikoHome, config, { now = new Date().toISOString() } = {}) {
  ...
  writeJsonAtomic(path.join(mikoHome, SERVER_NETWORK_FILE), normalized);
  return normalized;
}

export function resolveServerListenOptions(mikoHome) {
  const config = loadServerNetworkConfig(mikoHome);
  return {
    mode: config.mode,
