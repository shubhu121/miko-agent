...
describe("SpeechRecognitionService", () => {
  ...
  it("transcribes a voice-input SessionFile and persists pending then ready state", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-speech-service-"));
    try {
      ...
      const adapter = {
        ...
        protocolId: "mimo-chat-completions-asr",
        types: ["speechRecognition"],
        transcribe: vi.fn(async () => ({ text: "This feature is available in English only.", language: "zh" })),
      };
      const service = new SpeechRecognitionService({
        providerRegistry: makeProviderRegistry(),
        preferences: {
      ...
      expect(result).toMatchObject({ status: "ready", text: "This feature is available in English only." });
      ...
      expect(emitEvent).toHaveBeenCalledWith(expect.objectContaining({
        type: "voice_transcription_update",
        fileId: file.id,
        transcription: expect.objectContaining({ status: "ready", text: "This feature is available in English only." }),
      }), sessionPath);
    } finally {
  ...
  it("resolves MiMo Token Plan STT with Token Plan credentials and base URL", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-speech-token-plan-"));
    try {
  ...
  it("rejects transcribeAudio when fileId belongs to another loaded session", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-speech-cross-session-"));
    try {
