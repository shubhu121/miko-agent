...
export function classifyHttpRoute({ method = "GET", path = "" } = {}) {
  ...
  if (isWhatsAppWebhookRoute(verb, routePath)) return PUBLIC;
...
function normalizePath(path) {
  ...
  try {
    return new URL(raw, "http://miko.local").pathname;
  } catch {
    return raw.split("?")[0] || "/";
  }
}
...
function isWhatsAppWebhookRoute(verb, routePath) {
  return (verb === "GET" || verb === "POST")
    && /^\/api\/bridge\/whatsapp\/[^/]+$/.test(routePath);
}
