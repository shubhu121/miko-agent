...
export async function autoSaveConfig(
  ...
  try {
    ...
    const cfgRes = await mikoFetch(`/api/agents/${agentId}/config`);
...
export async function autoSaveGlobalModels(
  ...
  try {
    ...
    if (data.error) throw new Error(data.error);
    if (!opts.silent) store.showToast(t('settings.autoSaved'), 'success');
    const refreshRes = await mikoFetch('/api/preferences/models');
...
export function savePins() {
  ...
  _savePinsTimer = setTimeout(async () => {
    ...
    try {
      const agentId = store.getSettingsAgentId();
      const res = await mikoFetch(`/api/agents/${agentId}/pinned`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
export const VALID_THEMES = [
  _ids[0],                    // miko
  _ids[1],                    // warm-paper
  registry.AUTO_OPTION.id,    
  ..._ids.slice(2),           // midnight, high-contrast, grass-aroma, contemplation, absolutely, delve, deep-think, new-warm-paper
];
