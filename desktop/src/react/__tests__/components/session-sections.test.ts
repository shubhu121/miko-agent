...
describe('buildSessionProjectView', () => {
  it('excludes pinned sessions and groups unassigned sessions by derived cwd project', () => {
    const cwd = '/Users/test/Desktop/project-miko';
  ...
  it('keeps catalog folders and places assigned projects under their folder', () => {
    const catalog = {
      folders: [
        { id: 'folder-work', name: "This feature is available in English only.", order: 0 },
      ],
      projects: [
        { id: 'project-resume', name: "This feature is available in English only.", folderId: 'folder-work', order: 0 },
        { id: 'project-root', name: "This feature is available in English only.", folderId: null, order: 1 },
      ],
    ...
    const sections = buildSessionProjectView([
      makeSession({
        ...
        cwd: '/Users/test/Desktop/project-miko',
  ...
  it('surfaces empty catalog folders so users can drag projects into them', () => {
    const sections = buildSessionProjectView([], {
      folders: [{ id: 'folder-empty', name: "This feature is available in English only.", order: 0 }],
      projects: [],
    } as unknown as SessionProjectCatalog);
  ...
  it('does not demote catalog-assigned sessions to a cwd project while the catalog is still loading', () => {
    ...
    // sessions into the auto "Miko"-style group until the user toggles sort order.
    const view = buildSessionProjectView([
      makeSession({
