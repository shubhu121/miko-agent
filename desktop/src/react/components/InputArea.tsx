...
function InputAreaInner({ surface }: Required<InputAreaProps>) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('miko-inline-notice', handler);
  }, []);
      ...
      for (const file of files) {
        ...
        try {
          ...
          const waveform = mimeType.startsWith('audio/')
            ...
            : undefined;
          const res = await mikoFetch('/api/upload-blob', {
            method: 'POST',
  ...
  const loadVisionAuxiliaryConfig = useCallback(async () => {
    if (surface === 'mobile') {
      const res = await mikoFetch('/api/models/auxiliary-vision');
      const data = await res.json();
    ...
    const res = await mikoFetch('/api/preferences/models');
    const data = await res.json();
    return {
  ...
  useEffect(() => {
    if (activeServerConnection && surface !== 'mobile') {
      const query = pendingNewSession
        ...
        : currentSessionPath
          ...
          : '';
      mikoFetch(`/api/session-thinking-level${query}`)
        .then(r => r.json())
    ...
    window.addEventListener('miko-plan-mode', handler);
    return () => window.removeEventListener('miko-plan-mode', handler);
