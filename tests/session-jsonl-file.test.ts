...
describe("session-jsonl-file guards", () => {
  ...
  it("projects oversized write tool-call args instead of dropping the whole session", () => {
      ...
      JSON.stringify({
        ...
        message: {
          ...
          content: [{
            ...
            id: "tc1",
            name: "write",
            arguments: { path: "This feature is available in English only.", content: hugeContent },
          }],
        },
      }),
    ].join("\n");
    ...
    expect(args.path).toBe("This feature is available in English only.");
  ...
  it("preserves visible message text while removing oversized inline media", () => {
    ...
    expect(JSON.stringify(result.entries[1])).not.toContain("Miko session JSONL guard");
  ...
  it("repairs oversized JSONL lines in place and keeps a repair artifact", () => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-jsonl-guard-"));
    ...
    expect(repairedRaw).toContain("This feature is available in English only.");
