...
describe('MainContent app file drag attachments', () => {
  beforeEach(() => {
    ...
    useStore.setState({
      ...
      agentName: 'Miko',
      agentYuan: 'miko',
      agents: [{ id: 'agent-a', name: 'Miko', yuan: 'miko', isPrimary: true }],
      attachedFiles: [],
  ...
  it('ends the global drag lifecycle when SkillsPanel exclusively consumes the nested drop', async () => {
    vi.mocked(mikoFetch).mockImplementation(async (url: string) => {
    ...
    await waitFor(() => expect(vi.mocked(mikoFetch).mock.calls.filter(([url]) =>
    ...
    expect(vi.mocked(mikoFetch).mock.calls.some(([url]) => url === '/api/upload')).toBe(false);
  });
  ...
  it('keeps an ordinary root drop on the chat attachment path exactly once', async () => {
    ...
    expect(vi.mocked(mikoFetch).mock.calls.filter(([url]) => url === '/api/upload')).toHaveLength(1);
    expect(overlay).not.toHaveClass('visible');
  });
  ...
  it('attaches workspace files dragged from a native-root mount directly by absolute path', async () => {
    ...
    await attachAppFileDragPayloadToInput({
      dragId: 'miko-drag-mount-workspace',
      source: 'workspace',
      files: [{
  ...
  it('does not attach dragged files to the chat input while viewing channels', async () => {
    ...
    await attachAppFileDragPayloadToInput({
      dragId: 'miko-drag-channel-session-file',
      source: 'session-file',
      files: [{
        id: 'sf_channel',
        fileId: 'sf_channel',
