...
describe('ComputerUseOverlay', () => {
  beforeEach(() => {
    ...
    useStore.setState({
      ...
      sessions: [{ path: '/session/a.jsonl', sessionId: 'sess_a', agentId: 'miko' }],
  ...
  it('does not draw background action cursors in the Miko window', () => {
  ...
  it('does not draw done pulses in the Miko window', () => {
    useStore.getState().setComputerOverlayForSession('/session/a.jsonl', {
      phase: 'done',
      action: 'click_element',
      target: { coordinateSpace: 'element', elementId: 'mock-button' },
  ...
  it('keeps the Miko overlay reserved for foreground takeover UI only', () => {
    useStore.getState().setComputerOverlayForSession('/session/a.jsonl', {
      phase: 'running',
      action: 'click_point',
