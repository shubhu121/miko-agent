...
describe("agent phone session policy", () => {
  it("uses a stable safe session directory per conversation", () => {
    const dir = getAgentPhoneSessionDir("/agents/miko", "dm:yui");
    expect(dir).toContain(["agents", "miko", "phone", "sessions"].join(path.sep) + path.sep);
    expect(dir.split(path.sep).at(-1)).not.toContain(":");
  });

  it("recognizes phone sessions so memory pipelines can exclude them", () => {
    expect(isAgentPhoneSessionPath("/agents/miko/phone/sessions/ch_crew/session.jsonl")).toBe(true);
    expect(isAgentPhoneSessionPath("/agents/miko/sessions/session.jsonl")).toBe(false);
  });
