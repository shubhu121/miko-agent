...
describe('SettingsContent title placement', () => {
  beforeEach(() => {
    ...
    mockMikoFetch.mockResolvedValue(jsonResponse({ locale: 'zh-CN' }));
    window.platform = {
  ...
  it('pins the active tab title beside the settings title in the modal header', async () => {
    ...
    expect(within(header as HTMLElement).getByRole('heading', { name: "This feature is available in English only." })).toBeInTheDocument();
  ...
  it('does not echo the initially rendered tab back to the modal shell', async () => {
    ...
    await waitFor(() => {
      expect(mockMikoFetch).toHaveBeenCalledWith('/api/config');
    });
    await waitFor(() => {
