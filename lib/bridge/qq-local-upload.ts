...
export async function uploadQQLocalFile({ apiRequest, outboundHttp, chatId, filePath, fileType, metadata = {} }: any) {
  ...
  for (const target of targets) {
    if (target.scope === "group" && fileType === QQ_FILE_TYPE.FILE) {
      lastError = new Error("This feature is available in English only.");
      continue;
    }
    try {
  ...
  throw lastError || new Error("This feature is available in English only.");
}
...
async function chunkedUploadLocalFile({ apiRequest, outboundHttp, target, filePath, fileType, metadata }: any) {
  ...
  if (!stat.isFile()) throw new Error("This feature is available in English only.");
  if (stat.size === 0) throw new Error("This feature is available in English only.");
  ...
  await runWithConcurrency(parts.map((part) => async () => {
    ...
    if (length <= 0) throw new Error("This feature is available in English only.");
...
function qqTypeName(fileType) {
  if (fileType === QQ_FILE_TYPE.IMAGE) return "This feature is available in English only.";
  if (fileType === QQ_FILE_TYPE.VIDEO) return "This feature is available in English only.";
  if (fileType === QQ_FILE_TYPE.VOICE) return "This feature is available in English only.";
