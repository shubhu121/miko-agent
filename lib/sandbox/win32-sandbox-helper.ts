...
export const WIN32_SANDBOX_HELPER_NAME = "miko-win-sandbox.exe";
export const WIN32_SANDBOX_TERMINAL_PREFIX = "miko-win-sandbox: terminal-v1";
...
export function resourceSiblingDir(name, { env = process.env, resourcesPath = (process as any).resourcesPath } = {}) {
  ...
  if (env.MIKO_ROOT) candidates.push(path.resolve(env.MIKO_ROOT, "..", name));
...
export function resolveWin32SandboxHelper({
  ...
  const candidates = [
    ...
    env.MIKO_ROOT ? path.resolve(env.MIKO_ROOT, "..", "sandbox", "windows", WIN32_SANDBOX_HELPER_NAME) : null,
...
function parseWin32SandboxTerminalRecordLine(line: string): Win32SandboxTerminalRecord | null {
  const match = line.match(
    /^miko-win-sandbox: terminal-v1 status="([^"]*)" exitCode="([^"]*)" timeoutMs="([^"]*)" win32Error="([^"]*)"$/,
  );
