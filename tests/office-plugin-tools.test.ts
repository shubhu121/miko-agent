...
describe("office plugin tools", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-office-plugin-"));
  });
  ...
  it("renders HTML to PDF through the desktop helper contract and stages the output", async () => {
    ...
    const result = await renderHtmlToPdf(
      ...
      {
        env: {
          MIKO_DESKTOP_EXEC_PATH: "/Applications/Miko.app/Contents/MacOS/Miko",
          MIKO_DESKTOP_IS_PACKAGED: "1",
        },
        spawn: fakeSpawn,
      },
    );
    ...
    expect(observedCommand).toMatchObject({
      command: "/Applications/Miko.app/Contents/MacOS/Miko",
      args: ["--miko-office-html-to-pdf", expect.stringMatching(/job\.json$/)],
    });
    expect(fs.readFileSync(result.outputPath, "utf-8")).toContain("%PDF-1.4");
    expect(observedJob).toMatchObject({ embedMikoFonts: true });
  ...
  it("lets callers opt out of Miko font embedding via embedMikoFonts:false", async () => {
    let observedJob = null;
    const fakeSpawn = vi.fn((command, args) => {
      const child: any = new EventEmitter();
      child.stdout = new EventEmitter();
    ...
    await renderHtmlToPdf(
      { html: "<h1>plain</h1>", embedMikoFonts: false },
      { dataDir: tempDir },
      {
        env: { MIKO_DESKTOP_EXEC_PATH: "/usr/local/bin/electron" },
        spawn: fakeSpawn,
      },
    );

    expect(observedJob).toMatchObject({ embedMikoFonts: false });
  });
});
