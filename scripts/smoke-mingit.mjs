...
const workRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miko-mingit-smoke-"));
...
const IDENT = [
  "-c", "user.email=smoke@miko.invalid",
  "-c", "user.name=miko-smoke",
];
...
const results = [
  ...
  run("git config write", git, ["-C", repoDir, "config", "miko.smoke", "1"]),
  run("git config read", git, ["-C", repoDir, "config", "miko.smoke"]),
