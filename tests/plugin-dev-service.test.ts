...
describe("PluginDevService", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-plugin-dev-"));
    sourceRoot = path.join(tmpDir, "sources");
    communityPluginsDir = path.join(tmpDir, "miko-home", "plugins");
    devPluginsDir = path.join(tmpDir, "miko-home", "plugins-dev");
    runDataDir = path.join(tmpDir, "miko-home", "plugin-dev-runs");
    dataDir = path.join(tmpDir, "miko-home", "plugin-data");
  ...
  it("keeps dev full-access plugins restricted when re-enabled without dev permission", async () => {
    const sourcePath = writeDevPlugin(sourceRoot, "dev-full-toggle", {
      ...
      lifecycle: `
        export default class DevFullToggle {
          async onload() { globalThis.__mikoDevFullToggleLoaded = true; }
        }
      `,
    });
    ...
    delete globalThis.__mikoDevFullToggleLoaded;
    ...
    expect(globalThis.__mikoDevFullToggleLoaded).toBeUndefined();
  });
  ...
  it("keeps full-access dev plugins restricted without explicit dev permission", async () => {
    const sourcePath = writeDevPlugin(sourceRoot, "dev-full", {
      ...
      lifecycle: `
        export default class DevFull {
          async onload() { globalThis.__mikoDevFullLoaded = true; }
        }
      `,
    });
    ...
    expect(globalThis.__mikoDevFullLoaded).toBeUndefined();
  });
