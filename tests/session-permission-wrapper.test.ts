...
describe("session permission wrapper", () => {
  ...
  it("passes trust context to the auto reviewer for reviewer-bound tool actions", async () => {
    ...
    const [wrapped] = wrapWithSessionPermission([tool], {
      ...
      knownRemotes: ["origin git@example.com:miko/project.git"],
  ...
  it("This feature is available in English only.", async () => {
    ...
    const result = await wrapped.execute("call-1", { task: "This feature is available in English only." }, null, null, ctx);
    expect(tool.execute).not.toHaveBeenCalled();
