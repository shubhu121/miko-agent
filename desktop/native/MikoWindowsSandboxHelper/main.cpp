...
struct Options {
    ...
    std::vector<std::wstring> mikoWriteAclCleanupPaths;
...
static void fail(const std::wstring& message) {
    std::wcerr << L"miko-win-sandbox: " << message << std::endl;
}

static void debug(const std::wstring& message) {
    wchar_t enabled[8] = {};
    DWORD n = GetEnvironmentVariableW(L"MIKO_WIN32_SANDBOX_DEBUG", enabled, 8);
...
static void emitTerminalRecord(
    ...
    std::wcerr
        << L"miko-win-sandbox: terminal-v1"
        << L" status=\"" << status << L"\""
...
static std::wstring sidForWritableRoot(const std::wstring& root) {
    return hashSidForWritableRoot(root, L"S-1-5-21-", L"miko-win32-write-root-v3:");
}

static std::wstring sidForWritableRootLegacyCapabilityNamespace(const std::wstring& root) {
    return hashSidForWritableRoot(root, L"S-1-15-3-4096-", L"miko-win32-write-root-v2:");
}

static std::wstring sidForWritableRootLegacyAccountNamespace(const std::wstring& root) {
    return hashSidForWritableRoot(root, L"S-1-5-21-", L"miko-win32-write-root:");
}
...
static Options parseArgs(int argc, wchar_t** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == L"--cleanup-miko-write-acl" && i + 1 < argc) {
            opts.mikoWriteAclCleanupPaths.push_back(argv[++i]);
            continue;
        }
    ...
    bool maintenanceMode = !opts.mikoWriteAclCleanupPaths.empty() ||
...
static bool createSandboxDesktop(const std::vector<WritableRoot>& roots, SandboxDesktop& desktop) {
    ...
    desktop.name = L"miko-win-sandbox-" +
...
static void emitCreateProcessLaunchFailureDiagnostic(
    ...
    std::wcerr
        ...
        << std::endl;
    std::wcerr
        << L"miko-win-sandbox: launch-failure-probes"
...
static void emitGuardianRecord(
    ...
    std::wcerr
        << L"miko-win-sandbox: guardian-v1"
        << L" status=\"" << status << L"\""
...
static bool isLegacyAppContainerProfileName(const std::wstring& name) {
    const std::wstring prefix = L"com.miko.sandbox.";
...
static std::wstring probeNamedObjectNamespace(HANDLE restrictedToken) {
    ...
    std::wstring name = L"Local\\miko-win-sandbox-diagnose-" +
        std::to_wstring(GetCurrentProcessId()) + L"-" +
        std::to_wstring(GetTickCount64());
...
static int diagnoseRestrictedToken(const Options& opts) {
    ...
    std::wcerr
        << L"miko-win-sandbox: diagnose-token"
        << L" cwd=\"" << opts.cwd << L"\""
    ...
    for (const auto& root : opts.writableRoots) {
        std::wcerr
            << L"miko-win-sandbox: diagnose-token-writable-root"
    ...
    for (const auto& sid : restrictingSids) {
        std::wcerr
            << L"miko-win-sandbox: diagnose-token-restricting-sid"
    ...
    std::wcerr
        << L"miko-win-sandbox: diagnose-token-base-named-objects-probe"
...
static MigrationResult cleanupMikoWriteAcls(const std::vector<std::wstring>& paths) {
    ...
    for (const auto& path : paths) {
        ...
        if (rc != ERROR_SUCCESS) {
            fail(L"miko-write-acl-cleanup path=\"" + path + L"\" error=\"" + win32Message(rc) + L"\"");
            result.failures++;
        ...
        if (!matchedSids.empty()) {
            if (revokeSidsFromPath(path, matchedSids, dacl)) {
                ...
                for (PSID sid : matchedSids) {
                    std::wcerr
                        << L"miko-win-sandbox: miko-write-acl-cleaned"
                        << L" path=\"" << path << L"\""
    ...
    for (const auto& path : opts.legacyAclDiagnosticPaths) {
        ...
        if (dacl) {
            for (DWORD i = 0; i < dacl->AceCount; i++) {
                ...
                std::wcerr
                    << L"miko-win-sandbox: legacy-appcontainer-acl"
                    << L" path=\"" << path << L"\""
                    << L" sid=\"" << sidString << L"\""
        ...
        if (opts.cleanupLegacyAcl) {
            if (legacySids.empty()) {
                debug(L"legacy ACL cleanup found no Miko-owned AppContainer SID for " + path);
...
static MigrationResult cleanupLegacyProfiles(const std::vector<std::wstring>& profileNames) {
    ...
    for (const auto& name : profileNames) {
        ...
        if (SUCCEEDED(hr)) {
            result.findings++;
            std::wcerr
                << L"miko-win-sandbox: legacy-appcontainer-profile-cleaned"
                << L" name=\"" << name << L"\""
                << std::endl;
            continue;
        }
...
int wmain(int argc, wchar_t** argv) {
    ...
    try {
        ...
        std::wcerr << L"miko-win-sandbox: " << wide << std::endl;
        return 2;
    }
    ...
    if (!opts.mikoWriteAclCleanupPaths.empty() ||
        !opts.legacyAclDiagnosticPaths.empty() ||
        !opts.legacyProfileNames.empty() ||
        !opts.legacyProfileCleanupNames.empty() ||
        opts.cleanupLegacyAcl) {
        ...
        MigrationResult mikoWriteResult;
        if (!opts.mikoWriteAclCleanupPaths.empty()) {
            mikoWriteResult = cleanupMikoWriteAcls(opts.mikoWriteAclCleanupPaths);
        }
        ...
        failures += mikoWriteResult.failures + aclResult.failures;
        ...
        int findings = mikoWriteResult.findings + aclResult.findings + profileResult.findings;
