...
function createLegacyV1FactDb(dbPath) {
  ...
  `).run(
    ...
    JSON.stringify(["This feature is available in English only.", "This feature is available in English only."]),
    "2026-05-05T18:00",
    "legacy-session",
...
describe("FactStore CJK full-text search", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-fact-store-cjk-"));
  ...
  it("finds Chinese facts by meaningful substrings instead of requiring the full sentence", () => {
    ...
    expect(store.searchFullText("This feature is available in English only.", 10).map((r) => r.fact)).toEqual([
      "This feature is available in English only.",
    ]);
    expect(store.searchFullText("This feature is available in English only.", 10).map((r) => r.fact)).toEqual([
      "This feature is available in English only.",
  ...
  it("keeps the CJK full-text index consistent when facts are cleared", () => {
    ...
    expect(store.searchFullText("This feature is available in English only.", 10)).toHaveLength(1);

    store.clearAll();

    expect(store.searchFullText("This feature is available in English only.", 10)).toEqual([]);
  });
