...
import { splitMediaFromOutput } from "../lib/bridge/media-utils.ts";
...
describe("Bridge reply media extraction", () => {
  ...
  it("does not extract local MEDIA paths because local files must be staged", () => {
    const localPath = path.join(path.parse(process.cwd()).root, "tmp", "miko-output.png");
  ...
  it("does not extract file URLs from MEDIA lines", () => {
    const result = splitMediaFromOutput("MEDIA:file:///tmp/miko-output.png");
