...
describe("session:history", () => {
  ...
  it("does not expose reminder blocks through plugin session history", async () => {
    mockEngine._fakeMessages = [{
      role: "user",
      content: "[miko_reminder at 2026-07-05 14:05]\n- Plugin demo loaded\n[/miko_reminder]\n\nhello",
    }];
...
describe("agent:list", () => {
  it("returns agents with isCurrent and isPrimary", async () => {
    mockEngine.listAgents.mockReturnValue([
      { id: "a1", name: "Miko", isCurrent: true, isPrimary: true, extraField: "ignored" },
      { id: "a2", name: "Kuro", isCurrent: false, isPrimary: false, extraField: "ignored" },
    ]);
    const result = await bus.request("agent:list", {});
    expect(result.agents).toHaveLength(2);
    expect(result.agents[0]).toEqual({ id: "a1", name: "Miko", isCurrent: true, isPrimary: true });
