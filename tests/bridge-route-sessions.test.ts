...
describe("bridge sessions route", () => {
  beforeEach(() => {
    rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-bridge-route-"));
  });
  ...
  it("returns the resolved sessionPath for ChatTranscript hydration", async () => {
    ...
    const res = await app.request("/api/bridge/sessions?platform=telegram&agentId=miko");
    const body = await res.json();

    expect(res.status).toBe(200);
    expect(body.sessions).toHaveLength(1);
    expect(body.sessions[0]).toMatchObject({
      sessionKey: "tg_dm_owner@miko",
      file: "owner/tg-owner.jsonl",
      sessionPath,
      displayName: "Owner",
      avatarUrl: "https://example.com/avatar.png",
  ...
  it("strips Bridge internal time tags from visible session messages", async () => {
    ...
    const res = await app.request(`/api/bridge/sessions/${encodeURIComponent("tg_dm_owner@miko")}/messages?agentId=miko`);
    const body = await res.json();
  ...
  it("returns refreshed bridge status after owner changes", async () => {
    ...
    const res = await ownerApp.request("/api/bridge/owner?agentId=miko", {
      method: "POST",
  ...
  it("marks QQ sessions as owner when configured owner matches a principal alias", async () => {
    ...
    const res = await qqApp.request("/api/bridge/sessions?platform=qq&agentId=miko");
    const body = await res.json();
