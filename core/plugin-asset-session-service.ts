...
export function pluginAssetSessionCookieName(pluginId: string) {
  ...
  return `miko_plugin_assets_${digest}`;
}
...
function pluginAssetSessionKeyPath(mikoHome) {
  assertNonEmpty(mikoHome, "mikoHome");
  return path.join(mikoHome, "security", PLUGIN_ASSET_SESSION_KEY_FILE);
}
...
function readOrCreateSessionKey(mikoHome) {
  const filePath = pluginAssetSessionKeyPath(mikoHome);
  try {
