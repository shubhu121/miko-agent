...
export function issueRemoteWriteLease({
  ...
  if (!mikoHome || isLocalOwner(requestContext)) return null;
  ...
  const lease = issueExecutionLease(mikoHome, {
    schemaVersion: 1,
...
export function consumeRemoteWriteLease(mikoHome, lease, { now = new Date().toISOString() } = {}) {
  if (!mikoHome || !lease?.leaseId) return null;
  return consumeExecutionLease(mikoHome, lease.leaseId, { now });
}

export function revokeRemoteWriteLease(mikoHome, lease, { now = new Date().toISOString() } = {}) {
  if (!mikoHome || !lease?.leaseId) return null;
  return revokeExecutionLease(mikoHome, lease.leaseId, { now });
}
