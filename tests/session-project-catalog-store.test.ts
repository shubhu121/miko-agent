...
import { SessionProjectCatalogStore } from "../core/session-project-catalog-store.ts";

function makeStore() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "miko-session-projects-"));
...
describe("SessionProjectCatalogStore", () => {
  it("creates a project catalog file only when the user creates organization data", () => {
    ...
    const project = store.createProject({ name: "This feature is available in English only." });
  ...
  it("creates folders and moves projects into a folder with scoped ordering", () => {
    ...
    const folder = store.createFolder({ name: "This feature is available in English only." });
    const projectA = store.createProject({ name: "This feature is available in English only." });
    const projectB = store.createProject({ name: "This feature is available in English only.", folderId: folder.id });

    expect(folder).toMatchObject({ id: expect.stringMatching(/^folder-/), name: "This feature is available in English only.", order: 0 });
  ...
  it("preserves catalog files that contain folders", () => {
    ...
    fs.writeFileSync(path.join(userDir, "session-projects.json"), JSON.stringify({
      ...
      projects: [{ id: "project-resume", name: "This feature is available in English only.", folderId: "folder-work", order: 0 }],
    }));
  ...
  it("materializes cwd-derived project metadata when an auto project is renamed or reordered", () => {
    const { store } = makeStore();
    const project = store.updateProject("cwd:%2Ftmp%2Fproject-miko", { name: "project-miko" });

    expect(project).toMatchObject({
      id: "cwd:%2Ftmp%2Fproject-miko",
      name: "project-miko",
      folderId: null,
      order: 0,
    });
