...
describe("chat route model switch guard", () => {
  ...
  it("routes streaming interject messages through the desktop interjection contract", async () => {
    ...
    handlers.onMessage({
      data: JSON.stringify({
        ...
        sessionPath: "/tmp/session.jsonl",
        displayMessage: { text: "This feature is available in English only." },
    ...
    expect(hub.send).not.toHaveBeenCalled();
    expect(engine.steerSession).toHaveBeenCalledWith("/tmp/session.jsonl", "This feature is available in English only.");
    expect(engine.emitEvent).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "session_user_message",
        message: expect.objectContaining({ text: "This feature is available in English only." }),
      }),
      "/tmp/session.jsonl",
    );
    expect(JSON.parse(ws.send.mock.calls[0][0])).toMatchObject({
  ...
  it("buffers OpenAI Responses text until text_end and suppresses commentary phase", () => {
    ...
    expect(deltas.map((payload) => payload.delta)).toEqual(["This feature is available in English only."]);
  ...
  it("renders deferred interludes only when the actual hidden custom message is consumed", () => {
    ...
    const nextTextIndex = payloads.findIndex((payload) => payload.type === "text_delta" && payload.delta === "This feature is available in English only.");
  ...
  it("does not render successful media-generation custom inputs as deferred interludes", () => {
    ...
    const payloads = ws.send.mock.calls.map(([raw]) => JSON.parse(raw));
    expect(payloads.some((payload) => payload.type === "content_block" && payload.block?.type === "interlude")).toBe(false);
    expect(payloads.some((payload) => payload.type === "text_delta" && payload.delta === "This feature is available in English only.")).toBe(true);
    expect(engine.recordCustomEntry).not.toHaveBeenCalled();
  ...
  it("orders deferred interludes by actual custom-message consumption order", () => {
    ...
    subscriber?.({
      ...
      message: {
        ...
        customType: "miko-background-result",
        display: false,
        content: "<miko-background-result task-id=\"task-b\" status=\"success\" type=\"subagent\">\ntask-b done\n</miko-background-result>",
    ...
    for (const taskId of ["task-a", "task-b"]) {
      subscriber?.({
        ...
        message: {
          ...
          customType: "miko-background-result",
          display: false,
          content: `<miko-background-result task-id="${taskId}" status="success" type="subagent">\n${taskId} done\n</miko-background-result>`,
  ...
  it("fails closed for internal and missing session manifests", async () => {
    for (const manifest of [
      ...
      const engine = {
        ...
        deliverNotification,
        getSessionByPath: vi.fn(() => ({ entries: [], agentId: "agent-2", agentName: "This feature is available in English only." })),
        isSessionStreaming: vi.fn(() => false),
        isSessionSwitching: vi.fn(() => false),
        steerSession: vi.fn(() => false),
        slashDispatcher: null,
  ...
  it("aborts a streaming session only after the turn has been idle past the stall timeout", async () => {
    vi.useFakeTimers();
    const prevTimeout = process.env.MIKO_TURN_STALL_ABORT_MS;
    process.env.MIKO_TURN_STALL_ABORT_MS = "100";
    try {
      let createHandlers;
      let subscriber;
      const upgradeWebSocket = vi.fn((factory) => {
      ...
      if (prevTimeout === undefined) delete process.env.MIKO_TURN_STALL_ABORT_MS;
      else process.env.MIKO_TURN_STALL_ABORT_MS = prevTimeout;
      vi.useRealTimers();
    }
  });
  ...
  it("does not render interludes directly from standalone deferred result notifications", () => {
    ...
    for (const taskId of ["standalone-1", "standalone-2"]) {
      subscriber?.({
        ...
        meta: { type: "subagent", interlude: true, executorAgentNameSnapshot: "This feature is available in English only.", label: "This feature is available in English only." },
