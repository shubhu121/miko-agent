...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "miko-mobile-workbench-"));
}
...
async function makeRouteResourceIO({ mikoHome, workspace, eventBus = {}, studioId = "studio_1" }: Record<string, any>) {
...
describe("mobile workbench route", () => {
  ...
  it("exposes the same server workbench through desktop-neutral aliases", async () => {
    ...
    expect(Number(content.headers.get("X-Miko-File-MtimeMs"))).toBeGreaterThan(0);
    expect(content.headers.get("X-Miko-File-Size")).toBe(String(Buffer.byteLength("old")));
  ...
  it("returns mobile bootstrap metadata for desktop-compatible agent workbench selection", async () => {
    ...
    const app = await makeApp({
      mikoHome: path.join(tmpDir, "miko"),
      userDir: path.join(tmpDir, "miko", "user"),
      agentDir: path.join(tmpDir, "miko", "agents", "miko"),
      deskCwd: workspace,
      homeCwd: workspace,
      getLocale: () => "zh-CN",
      agentName: "Miko",
  ...
  it("serves UTF-8 file content with HEAD and Range support", async () => {
    ...
    const workspace = path.join(tmpDir, "workspace");
    fs.mkdirSync(workspace, { recursive: true });
    fs.writeFileSync(path.join(workspace, "This feature is available in English only."), "abcdef", "utf-8");
    const app = await makeApp({
      mikoHome: path.join(tmpDir, "miko"),
      deskCwd: workspace,
      homeCwd: workspace,
    });
    const query = "This feature is available in English only.";
  ...
  it("safe-deletes mobile files into recoverable trash instead of hard removing bytes", async () => {
    ...
    const trashDir = path.join(mikoHome, "trash", "mobile-workbench", data.trashId);
  ...
  it("accepts mountId as the canonical workbench mount selector while preserving rootId in the response", async () => {
    ...
    expect(Number(content.headers.get("X-Miko-File-MtimeMs"))).toBeGreaterThan(0);
    expect(content.headers.get("X-Miko-File-Size")).toBe(String(Buffer.byteLength("mount body")));
  ...
  it("creates and consumes an execution lease for remote mobile writes", async () => {
    ...
    const leases = JSON.parse(fs.readFileSync(path.join(mikoHome, "security", "execution-leases.json"), "utf-8"));
    ...
    const audit = fs.readFileSync(path.join(mikoHome, "logs", "security-audit.jsonl"), "utf-8");
