...
export function ClearMemoryConfirm() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('miko-show-clear-confirm', handler);
    return () => window.removeEventListener('miko-show-clear-confirm', handler);
  }, []);
  ...
  const doClear = async () => {
    ...
    try {
      ...
      const res = await mikoFetch(`/api/memories?agentId=${aid}`, { method: 'DELETE' });
