...
describe("login item settings", () => {
  it("uses a dedicated login-start argument on Windows so startup can stay hidden", async () => {
    ...
    expect(getLoginItemOptions("win32", "C:\\Program Files\\Miko\\Miko.exe")).toEqual({
      path: "C:\\Program Files\\Miko\\Miko.exe",
      args: [START_AT_LOGIN_ARG],
    });
    expect(wasLaunchedAtLogin({
      platform: "win32",
      argv: ["Miko.exe", START_AT_LOGIN_ARG],
      loginItemSettings: {},
    })).toBe(true);
  });
  ...
  it("reports Linux as unsupported without touching Electron login item APIs", async () => {
    ...
    expect(getAutoLaunchStatus({ app, platform: "linux", argv: [], execPath: "/opt/Miko/miko" })).toEqual({
      supported: false,
      openAtLogin: false,
      openedAtLogin: false,
      status: "unsupported",
  ...
  it("writes and re-reads Windows login item settings with the same path and args", async () => {
    ...
    const status = setAutoLaunchEnabled({
      ...
      platform: "win32",
      argv: [],
      execPath: "C:\\Miko\\Miko.exe",
      enabled: true,
    });

    expect(app.setLoginItemSettings).toHaveBeenCalledWith({
      openAtLogin: true,
      path: "C:\\Miko\\Miko.exe",
      args: [START_AT_LOGIN_ARG],
    });
