...
function mockPermissionDefault(mode = 'ask') {
  ...
  beforeEach(() => {
    ...
    (globalThis.window as unknown as { miko?: unknown }).miko = {};
    installStoreMethods();
    mockFetch.mockReset();
  ...
  it('continues a deleted-agent session by creating and switching to the returned primary-agent session', async () => {
    ...
    const newPath = '/tmp/agents/miko/sessions/new.jsonl';
    mockFetch.mockImplementation(async (url: string) => {
      if (url === '/api/sessions/continue-deleted-agent') {
        return jsonResponse({ ok: true, path: newPath, agentId: 'miko', agentName: 'Miko' });
      }
      if (url === '/api/sessions') {
        return jsonResponse([{ path: newPath, agentId: 'miko', agentName: 'Miko', cwd: '/tmp/work' }]);
      }
      if (url === '/api/sessions/switch') {
        return jsonResponse({
          ok: true,
  ...
  describe('createNewSession cwd draft', () => {
    ...
    it('invalidates an in-flight session switch so the new-session desk stays on the agent home folder', async () => {
      (mockState as Record<string, unknown>).currentSessionPath = '/session/miko.jsonl';
      (mockState as Record<string, unknown>).deskBasePath = '/workspace/Desktop/project-miko';
      (mockState as Record<string, unknown>).homeFolder = '/workspace/Desktop/project-miko';
      ...
      expect(mockState.pendingNewSession).toBe(true);
      expect(mockState.selectedFolder).toBe('/workspace/Desktop/project-miko');
      expect(mockState.deskBasePath).toBe('/workspace/Desktop/project-miko');
    ...
    it('uses the runtime new-session permission default instead of the old active session mode', async () => {
      ...
      const permissionEvent = dispatchedEvents.filter(e => e.type === 'miko-plan-mode').at(-1);
    ...
    it('consumes home draft on activation without copying text into the new session draft', async () => {
      const homeDoc = { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: "This feature is available in English only." }] }] };
      Object.assign(mockState, {
        pendingNewSession: true,
        ...
        attachedFiles: [{
          fileId: 'sf_home',
          path: '/tmp/miko/session-files/home.png',
          name: 'home.png',
          isDirectory: false,
        }],
      });
    ...
    it('posts one new-session request at send time using the current pending draft', async () => {
      ...
      mockFetch.mockImplementation(async (url: string, opts?: RequestInit) => {
        ...
        if (url === '/api/sessions/switch') {
          return jsonResponse({ ok: true, sessionId: 'sess_fresh', agentId: 'miko', workspaceFolders: [] });
        }
        throw new Error(`unexpected fetch: ${url}`);
      });
    ...
    it('does not let a late detached create response activate a replacement pending draft (#2078 ABA)', async () => {
      ...
      expect(ref).toEqual({ sessionId: 'sess_a', sessionPath: '/session/a.jsonl', agentId: 'miko' });
    ...
    it('carries an explicit project id from the new-session draft into session creation', async () => {
      ...
      await createNewSession({ projectId: 'project-miko', cwd: '/workspace/project-miko' });
      mockFetch.mockResolvedValueOnce(jsonResponse({
        ok: true,
        path: '/session/new.jsonl',
        sessionId: 'sess_project',
        agentId: 'miko',
        cwd: '/workspace/project-miko',
