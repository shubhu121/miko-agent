...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "miko-vite-keyset-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(() => {
  ...
  delete process.env.MIKO_SIGN_KEYSET;
  vi.resetModules();
});
...
describe("vite.config.main.js MIKO_SIGN_KEYSET build-time substitution", () => {
  ...
  it("aliases the pinned keyset module to the override file when MIKO_SIGN_KEYSET is set", async () => {
    ...
    process.env.MIKO_SIGN_KEYSET = overridePath;
  ...
  it("hard-errors at config load when MIKO_SIGN_KEYSET points at a missing file", async () => {
    process.env.MIKO_SIGN_KEYSET = "/nonexistent/override-keyset.json";
    await expect(loadConfig()).rejects.toThrow(/MIKO_SIGN_KEYSET/);
  });
});
